...
===========================================================
ADVANCED GOOGLE SHEETS SERVICE FOR TADAWUL FAST BRIDGE — v5.0.0
...
- ✅ High-Performance JSON (`orjson`) fallback for blazing fast grid serialization
- ✅ Memory-optimized state models using `@dataclass(slots=True)`
- ✅ OpenTelemetry Tracing integration covering Sheets API calls and backend chunks
- ✅ Prometheus Metrics for granular operation success/latency observability
- ✅ Full Jitter Exponential Backoff for Google Sheets API and Backend retries
    ...
    for attempt in range(_CONFIG.sheets_api_retries):
        try:
            ...
            if retryable and attempt < _CONFIG.sheets_api_retries - 1:
                ...
                sleep_time = min(15.0, base_sleep + random.uniform(0, base_sleep * 0.2))
                logger.warning(
            ...
            for attempt in range(_CONFIG.backend_retries + 1):
                try:
                    ...
                    with urllib.request.urlopen(req, timeout=_CONFIG.backend_timeout_sec, context=self._ssl_ctx) as resp:
                        ...
                        if 200 <= status < 300:
                            ...
                            if status in (429, 500, 502, 503, 504) and attempt < _CONFIG.backend_retries:
                                ...
                                sleep_time = min(30.0, float(retry_after)) if retry_after else min(30.0, base_sleep + random.uniform(0, base_sleep * 0.2))
                    ...
                    if attempt < _CONFIG.backend_retries:
                        base_sleep = _CONFIG.backend_retry_sleep * (2 ** attempt)
                        sleep_time = min(30.0, base_sleep + random.uniform(0, base_sleep * 0.2))
