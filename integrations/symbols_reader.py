# integrations/google_sheets_service.py  (FULL REPLACEMENT) — v3.17.0
"""
integrations/google_sheets_service.py
------------------------------------------------------------
Google Sheets helper for Tadawul Fast Bridge — v3.17.0 (PROD SAFE + ALIGNED + SAFE-MODE)
...
Key upgrades in v3.17.0
- ✅ Integrated symbols_reader logic: handles Arabic digits, invisible chars, and .SR normalization.
- ✅ Enhanced Header Alias Map: supports more ROI, Forecast, Technical, and Score variants.
- ✅ Market-Aware Normalization: properly splits and formats KSA vs Global tickers before backend calls.
- ✅ Standardized Payload structure: ensures 'symbols' and 'tickers' lists are identical for backend compat.
"""
...
# --- Symbol Reader Integration ---
_SPLIT_TICKERS_FN = None
try:
    from integrations.symbols_reader import split_tickers_by_market as _stbm  # type: ignore
    if callable(_stbm):
        _SPLIT_TICKERS_FN = _stbm
except Exception:
    pass
...
# =============================================================================
# Header aliasing (ROI + forecast key fix)
# =============================================================================
_HEADER_ALIAS_MAP: Dict[str, str] = {
    ...
    "opportunscore": "Opportunity Score",
    # Technicals
...
def _normalize_tickers(tickers: Sequence[str]) -> List[str]:
    # Integrated symbols_reader normalization logic
    # Handles Arabic digits, invisible chars, and .SR normalization.
    try:
        from integrations.symbols_reader import _clean_text, _normalize_ksa_symbol, _normalize_global_symbol
