...
================================================================================
SCORING ENGINE CONTRACT TESTS — v2.1.0 (HARDENED / CI-FRIENDLY / NO-PRINT)
================================================================================
Emad Bahbah – Tadawul Fast Bridge

Why v2.1.0 (your “not wired / no green signal” fix)
- ✅ Still stdlib unittest (no network, no prints, CI-friendly)
- ✅ Clearer failure diagnostics (surface import errors + missing contract fields)
- ✅ More tolerant dict extraction (pydantic v1/v2, dataclasses, objects)
- ✅ Safer numeric assertions (handles ints/floats/strings)
- ✅ Keeps your negative ROI clamp contract intact

IMPORTANT (WIRING)
- Pytest default collects: test_*.py
- This file is *_test.py, so pytest will NOT collect it unless you:
    A) add pytest.ini with python_files = test_*.py *_test.py   (recommended), OR
    B) rename the file to test_scoring_engine_contract.py
"""
...
class TestScoringEngineContract(unittest.TestCase):
    ...
    def test_compute_scores_minimal_input_no_crash(self) -> None:
        ...
        for k in ("value_score", "quality_score", "momentum_score", "risk_score", "opportunity_score", "overall_score"):
            ...
            self.assertTrue(_is_number(d[k]), msg=f"Non-numeric score for {k}: {d.get(k)!r}")
            v = _to_float(d[k])
    ...
    def test_negative_roi_clamp_is_supported(self) -> None:
        ...
        - within [-65, +65] default bounds (or env-adjusted for CI)
        """
        payload = {
            "symbol": "TEST.NEG",
            "price": 100.0,
    ...
    def test_missing_price_does_not_crash(self) -> None:
        ...
        # scoring_errors should exist per contract, even if empty
    ...
    def test_enrich_with_scores_returns_expected_fields(self) -> None:
        ...
        for k in ("overall_score", "risk_score", "recommendation", "forecast_price_12m", "expected_roi_12m"):
            self.assertIn(k, out, msg=f"Missing key in enrich output: {k}")

        for k in ("expected_return_12m", "expected_price_12m", "confidence_score"):
            self.assertIn(k, out, msg=f"Missing compatibility alias: {k}")
...
def main() -> int:
    ...
    try:
        ...
        verbosity = 2 if _is_number(os.getenv("TFB_TEST_VERBOSE")) and int(float(os.getenv("TFB_TEST_VERBOSE") or "0")) > 0 else 1
