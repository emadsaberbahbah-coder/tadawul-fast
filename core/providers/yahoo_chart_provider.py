...
def _infer_exchange(symbol: str, info: Any = None, meta: Optional[Dict[str, Any]] = None) -> Optional[str]:
    ...
    if s.endswith("=F"):  return "Yahoo Futures"
    if s.endswith("=X"):  return "Yahoo FX"
    return None
...
def _infer_country(symbol: str, info: Any = None) -> Optional[str]:
    if isinstance(info, dict):
        ...
        if c: return c
    if symbol.upper().endswith(".SR"): return "Saudi Arabia"
    return None


def _history_df_extract(hist: Any) -> Dict[str, Any]:
    bundle: Dict[str, Any] = {"closes": [], "opens": [], "highs": [], "lows": [], "volumes": [], "last_row": None, "last_dt": None}
    if hist is None: return bundle
    try:
        if getattr(hist, "empty", None) is True: return bundle
    except Exception:
        pass
    try:
        if hasattr(hist, "iloc"):
            try: bundle["last_row"] = hist.iloc[-1]
            except Exception: bundle["last_row"] = None
        try:
            idx = hist.index[-1]
        ...
        return {"closes": [], "opens": [], "highs": [], "lows": [], "volumes": [], "last_row": None, "last_dt": None}
    return bundle
...
def _history_df_to_list(hist: Any) -> List[Dict[str, Any]]:
    ...
    try:
        if getattr(hist, "empty", None) is True: return []
    except Exception:
        pass
...
def _row_get(row: Any, key: str) -> Any:
    try:
        ...
        if hasattr(row, "get"): return row.get(key)
        return row[key]
    except Exception:
        return None
...
class YahooChartProvider:
    ...
    def _blocking_fetch_history(self, symbol: str, period: str, interval: str) -> Dict[str, Any]:
        ...
        for attempt in range(max(1, int(self.retry_attempts))):
            try:
                ...
                last_err = e
                time.sleep(random.uniform(0.0, min(5.0, 0.5 * (2 ** attempt))))
    ...
    def _blocking_fetch(self, symbol: str) -> Dict[str, Any]:
        if not _HAS_YFINANCE or yf is None:
            return {"symbol": symbol, "error": "yfinance_not_installed", "provider": PROVIDER_NAME, "provider_version": PROVIDER_VERSION, "data_sources": [PROVIDER_NAME]}
        ...
        for attempt in range(max(1, int(self.retry_attempts))):
            try:
                ...
                hist = None
                history_bundle = {"closes": [], "opens": [], "highs": [], "lows": [], "volumes": [], "last_row": None, "last_dt": None}
                try:
                ...
                if (
                    ...
                    out["week_52_position_pct"] = float(
                        (out["current_price"] - out["week_52_low"]) /
                        (out["week_52_high"]  - out["week_52_low"])
                    )

                # Light fundamentals from Yahoo info (dtype=pct fields as fractions)
                # FIX v6.9.0: use _first_fraction() not _first_percentish()
                if isinstance(info, dict):
                    ...
                    out["pe_ttm"]             = _first_number(info.get("trailingPE"))
                    ...
                    out["dividend_yield"]     = _first_fraction(info.get("dividendYield"), info.get("trailingAnnualDividendYield"))
                    out["payout_ratio"]       = _first_fraction(info.get("payoutRatio"))
                    out["revenue_growth_yoy"] = _first_fraction(info.get("revenueGrowth"))
                    out["gross_margin"]       = _first_fraction(info.get("grossMargins"))
                    out["operating_margin"]   = _first_fraction(info.get("operatingMargins"))
                    out["profit_margin"]      = _first_fraction(info.get("profitMargins"))

                # Indicators + Forecasts + Risk Stats (already correct fractions)
                if closes:
                    rsi = _rsi_14(closes)
                    if rsi is not None:
                    ...
                    if vol30 is not None: out["volatility_30d"] = float(vol30)
                    if vol90 is not None: out["volatility_90d"] = float(vol90)

                    dd1y = _max_drawdown_1y(closes)
                    v95  = _var_95_1d(closes)
                    sh   = _sharpe_1y(closes)
                    if dd1y is not None: out["max_drawdown_1y"] = float(dd1y)
                    if v95  is not None: out["var_95_1d"]       = float(v95)
                    if sh   is not None: out["sharpe_1y"]       = float(sh)

                    # FIX v6.9.0: _simple_forecast returns ROI as fraction now
                ...
                filled = sum(1 for k in ("current_price","previous_close","open_price","day_high","day_low","volume","week_52_high","week_52_low","avg_volume_10d","avg_volume_30d") if out.get(k) is not None)
    ...
    async def fetch_enriched_quote_patch(self, symbol: str, debug: bool = False, *args: Any, **kwargs: Any) -> Dict[str, Any]:
        if not _configured():
            return {"symbol": symbol, "requested_symbol": symbol, "error": "provider_disabled_or_yfinance_missing", "provider": PROVIDER_NAME, "provider_version": PROVIDER_VERSION, "data_sources": [PROVIDER_NAME]}
        sym = normalize_symbol(symbol)
        if not sym:
            return {"symbol": symbol, "requested_symbol": symbol, "error": "invalid_symbol", "provider": PROVIDER_NAME, "provider_version": PROVIDER_VERSION, "data_sources": [PROVIDER_NAME]}
        if not await self._cb.allow():
            return {"symbol": sym, "requested_symbol": symbol, "error": "circuit_open", "provider": PROVIDER_NAME, "provider_version": PROVIDER_VERSION, "data_sources": [PROVIDER_NAME]}
        ...
        async def _do_fetch() -> Dict[str, Any]:
            async with self._sem:
                ...
                try:
                    ...
                    if ok: await self._cb.record_success()
                    else:  await self._cb.record_failure()
                    ...
                    m.request_duration.labels(symbol=sym, op="quote").observe(max(0.0, time.time() - t0))
                    return {"symbol": sym, "requested_symbol": symbol, "error": f"exception: {e}", "provider": PROVIDER_NAME, "provider_version": PROVIDER_VERSION, "data_sources": [PROVIDER_NAME]}
    ...
    async def fetch_batch(self, symbols: List[str], debug: bool = False) -> Dict[str, Dict[str, Any]]:
        ...
        for s, r in zip(syms, res):
            if isinstance(r, dict):
                ...
                out[s] = {"symbol": s, "requested_symbol": s, "error": str(r), "provider": PROVIDER_NAME, "provider_version": PROVIDER_VERSION, "data_sources": [PROVIDER_NAME]}
        return out

    # ENH v6.9.0: canonical batch method name for engine compatibility
    async def get_enriched_quotes_batch(
        self, symbols: List[str], *, mode: str = ""
    ) -> Dict[str, Dict[str, Any]]:
        return await self.fetch_batch(symbols)
...
async def fetch_enriched_quotes_batch(
    ...
    """ENH v6.9.0: Canonical batch fetch for engine compatibility."""
    return await (await aget_provider()).get_enriched_quotes_batch(symbols, mode=mode)
...
def get_provider():     return provider
def build_provider():   return provider
def create_provider():  return provider
def provider_factory(): return provider
