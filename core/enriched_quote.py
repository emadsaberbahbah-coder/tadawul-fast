...
===============================================================================
TFB Enriched Quote Core -- v3.3.0
===============================================================================
...
--------------------------
FIX: _FALLBACK_INSTRUMENT_KEYS aligned to schema_registry v3.0.0 canonical keys.
  v3.2.0 used non-canonical aliases that do not match the schema registry or
  data_engine_v2.INSTRUMENT_CANONICAL_KEYS. These caused the last-resort fallback
  to generate misaligned column headers / keys when neither schema_registry nor
  data_engine_v2 static contracts were importable.

  Fixed aliases:
    "high_52w"         -> "week_52_high"        (canonical schema key)
    "low_52w"          -> "week_52_low"          (canonical schema key)
    "position_52w_pct" -> "week_52_position_pct" (canonical schema key)
    "as_of_utc"        -> "last_updated_utc"     (canonical schema key)
    "source"           -> "data_provider"        (canonical schema key)

FIX: _FALLBACK_INSIGHTS_KEYS aligned to canonical Insights_Analysis schema.
  data_engine_v2.INSIGHTS_KEYS = [section, item, metric, value, notes, source,
  sort_order]. v3.2.0 used "priority" (non-canonical) and "as_of_utc" instead
  of "source" and "sort_order".

FIX: schema_registry and page_catalog imports hardened with multi-path fallback.
  v3.2.0 had single import paths that failed when modules were not at the
  core.sheets.* location. v3.3.0 tries:
...
Preserved from v3.2.0 (no other behavioral changes):
- rows->matrix conversion, engine auto-resolution, special-page dispatch,
  instrument quote fallback, sync/async safety, JSON-safe envelope.
"""
...
# -----------------------------------------------------------------------------
# Optional page catalog (FIX v3.3.0: multi-path import)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# Optional schema registry (FIX v3.3.0: multi-path import)
# -----------------------------------------------------------------------------
...
# FIX v3.3.0: key names aligned to schema_registry v3.0.0 and
# data_engine_v2.INSTRUMENT_CANONICAL_KEYS.
# Previous: "high_52w", "low_52w", "position_52w_pct", "as_of_utc", "source"
# Fixed:    "week_52_high", "week_52_low", "week_52_position_pct",
#           "last_updated_utc", "data_provider"
_FALLBACK_INSTRUMENT_KEYS: List[str] = [
    ...
    "day_high",
    "day_low",
    "week_52_high",           # FIX v3.3.0: was "high_52w"
    "week_52_low",            # FIX v3.3.0: was "low_52w"
    "price_change",
    "percent_change",
    "week_52_position_pct",   # FIX v3.3.0: was "position_52w_pct"
    "volume",
    "avg_volume_10d",
    "avg_volume_30d",
    "market_cap",
    ...
    "rank_overall",
    "confidence_bucket",
    "data_provider",          # FIX v3.3.0: was "source"
    "last_updated_utc",       # FIX v3.3.0: was "as_of_utc"
    "last_updated_riyadh",
    "notes",
    "primary_provider",
...
# FIX v3.3.0: aligned to data_engine_v2.INSIGHTS_KEYS =
# [section, item, metric, value, notes, source, sort_order].
# Previous: "priority" (non-canonical) and "as_of_utc" instead of
#           "source" and "sort_order".
_FALLBACK_INSIGHTS_KEYS = [
    ...
    "value",
    "notes",
    "source",      # FIX v3.3.0: was "as_of_utc"
    "sort_order",  # FIX v3.3.0: was "priority"
]
...
def _json_safe(value: Any) -> Any:
    ...
    if is_dataclass(value):
        try:    return _json_safe(asdict(value))
        except: return str(value)
...
def _extract_settings_dict(settings: Any) -> Dict[str, Any]:
    ...
    if isinstance(settings, Mapping): return dict(settings)
    try:
        d = vars(settings)
        return d if isinstance(d, dict) else {}
    except Exception:
...
def _normalize_symbols(value: Any) -> List[str]:
    ...
    if isinstance(value, str):
        ...
        for part in raw.split(","): _push(part)
        return out
    if isinstance(value, (list, tuple, set)):
        for item in value: _push(item)
        return out
    _push(value)
    return out
...
def _route_family(page: str) -> str:
    ...
    if page == "Top_10_Investments":  return "top10"
    if page == "Insights_Analysis":   return "insights"
    if page == "Data_Dictionary":     return "dictionary"
    return "instrument"


def _is_instrument_page(page: str) -> bool:
    try:    return bool(_catalog_is_instrument_page(page))
    except: return page in INSTRUMENT_PAGES
...
def _row_from_any(value: Any) -> Optional[Dict[str, Any]]:
    ...
    if isinstance(value, Mapping): return {str(k): v for k, v in value.items()}
    d = _as_dict(value)
...
def _extract_rows_like(payload: Any) -> List[Dict[str, Any]]:
    ...
    if isinstance(payload, Mapping):
        ...
        for key in ("payload", "result"):
            ...
            if nested_rows: return nested_rows
...
def _matrix_to_rows(matrix: Sequence[Sequence[Any]], keys: Sequence[str]) -> List[Dict[str, Any]]:
    ...
    key_list: List[str]            = [str(k) for k in keys]
    for row in matrix:
...
def _method_variants(
    *,
    ...
    common = {
        "page": page, "sheet": page, "sheet_name": page, "name": page, "tab": page,
        "body": dict(body), "request": request, "settings": settings,
        "limit": limit, "mode": mode,
        "schema_only": schema_only, "headers_only": headers_only, "table_mode": table_mode,
    }
    variants = [
...
def _row_richness(row: Optional[Dict[str, Any]]) -> int:
    ...
    important = (
        "symbol", "ticker", "name", "company_name", "current_price",
        "exchange", "currency", "country", "sector", "industry", "market_cap",
        "volume", "overall_score", "opportunity_score", "recommendation",
        "recommendation_reason", "selection_reason", "metric", "value", "header", "key",
    )
    score = 0
    for key in important:
        val = row.get(key)
        if val is None: continue
        if isinstance(val, str) and not _strip(val): continue
        score += 1
    return score
...
def _merge_payload_dicts(base: Optional[Dict[str, Any]], addon: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    out = dict(base or {})
    if not isinstance(addon, dict): return out
    for k, v in addon.items():
        if v is None: continue
        if isinstance(v, str) and not _strip(v): continue
        out[k] = v
    return out
...
def _safe_status(status: Any, rows: Sequence[Mapping[str, Any]], headers: Sequence[str], error: Optional[str], warnings: Sequence[str]) -> str:
    ...
    if st in {"success", "ok"}:    return "success"
    if st in {"error", "failed", "fail"}: return "error"
    if rows or headers:
...
async def _resolve_engine(engine: Any, request: Any = None) -> Any:
    ...
    if app_engine is not None: return app_engine
    ...
    for module_name in _ENGINE_MODULE_CANDIDATES:
        ...
        for name in _ENGINE_FACTORY_NAMES:
            ...
            try:
                result = await _call_variants(fn, [{"request": request}, {}], timeout_sec=DEFAULT_TIMEOUT_SEC)
...
async def _call_special_builder(
    *,
    ...
    def _variants() -> List[Dict[str, Any]]:
        base = {
            "engine": engine, "request": request, "settings": settings,
            "body": merged_body, "criteria": criteria,
            "page": page, "sheet": page, "mode": mode, "limit": limit,
        }
        return [
            {**base},
            {**base, "payload": merged_body},
            {"request": request, "settings": settings, "body": merged_body, "criteria": criteria, "limit": limit, "mode": mode},
            {"engine": engine, "body": merged_body, "criteria": criteria, "limit": limit, "mode": mode},
            {"body": merged_body, "criteria": criteria, "limit": limit, "mode": mode},
            {"payload": merged_body, "limit": limit, "mode": mode},
            {},
        ]
    ...
    if page == "Top_10_Investments":
        candidates = [
            ("core.analysis.top10_selector", (
                ...
                "build_top10_investments_rows", "select_top10", "select_top10_symbols",
            )),
            ("core.investment_advisor_engine", ("run_investment_advisor_engine", "run_advisor_engine")),
        ...
        candidates = [
            ("core.sheets.data_dictionary", (
                "build_data_dictionary_rows", "get_data_dictionary_rows", "generate_data_dictionary",
            )),
        ]
...
async def _engine_sheet_rows(
    *,
    ...
    variants = _method_variants(
        page=page, body=body, symbols=symbols, limit=limit, mode=mode,
...
async def _engine_quotes(
    *,
    ...
    variants  = _method_variants(
        page=page, body=body, symbols=symbols, limit=limit, mode=mode,
        request=request, settings=settings,
        schema_only=False, headers_only=False, table_mode=False,
    )
    ...
    def _get(sym: str) -> Dict[str, Any]:
        for cand in _symbol_candidates(sym):
            if cand in quote_map: return quote_map[cand]
        return {}
        ...
        if isinstance(payload, Mapping):
            ...
            if rows:
                for idx, row in enumerate(rows):
                    ...
                    if not sym and idx < len(symbols): sym = str(symbols[idx])
                    if sym: _put(sym, row)
            else:
                for k, v in payload.items():
                    if isinstance(v, Mapping):
                        rd = dict(v)
                        if _looks_like_row(rd) or _strip(k): _put(k, rd)
...
async def build_enriched_quote_payload(
    ...
    **extra: Any,
    ...
    page_norm    = _normalize_page(
        page
        or merged.get("page") or merged.get("sheet") or merged.get("sheet_name")
        or merged.get("tab")  or merged.get("name")
        or DEFAULT_PAGE
    )
    ...
    if page_norm in SPECIAL_PAGES:
        if schema_only_out or headers_only_out:
            ...
            return _envelope(
                page=page_norm, route_family=route_family,
                headers=headers, keys=keys, display_headers=display_headers, columns=columns,
                rows=[], include_matrix=include_matrix_out,
                request_id=req_id, started_at=started_at, mode=mode_out,
        ...
        try:
            special_payload = await _call_special_builder(
                page=page_norm, engine=engine_obj, request=request, settings=settings,
                body=merged, limit=limit_out, mode=mode_out, timeout_sec=DEFAULT_TIMEOUT_SEC,
            )
        except Exception as exc:
        ...
        if not rows:
            eng_rows, eng_meta, eng_status, eng_error = await _engine_sheet_rows(
                engine=engine_obj, page=page_norm, body=merged, symbols=resolved_symbols,
                request=request, settings=settings, mode=mode_out, limit=limit_out,
                schema_only=False, headers_only=False, table_mode=True,
    ...
    # Instrument quote mode
        ...
        quote_rows, quote_meta, quote_status, quote_error = await _engine_quotes(
            engine=engine_obj, page=page_norm, symbols=resolved_symbols[:limit_out],
            request=request, settings=settings, body=merged, mode=mode_out,
            limit=limit_out, timeout_sec=DEFAULT_TIMEOUT_SEC,
        )
        rows        = quote_rows
        ...
        if (not rows) or all(_is_sparse_row(r) for r in rows):
            sheet_rows, sheet_meta, sheet_status, sheet_error = await _engine_sheet_rows(
                engine=engine_obj, page=page_norm, body=merged, symbols=resolved_symbols,
                request=request, settings=settings, mode="live_sheet", limit=limit_out,
                schema_only=False, headers_only=False, table_mode=True,
                timeout_sec=DEFAULT_TIMEOUT_SEC,
            )
            if sheet_rows:
                ...
                if sheet_status: status_out = sheet_status
    ...
    # Table / page mode
        ...
        sheet_rows, sheet_meta, sheet_status, sheet_error = await _engine_sheet_rows(
            engine=engine_obj, page=page_norm, body=merged, symbols=resolved_symbols,
            request=request, settings=settings, mode=mode_out, limit=limit_out,
            schema_only=schema_only_out, headers_only=headers_only_out,
    ...
    headers, keys, display_headers, columns = _derive_headers_keys(
        page=page_norm, rows=rows, source_meta=source_meta,
    )
    ...
    if not rows and not (schema_only_out or headers_only_out):
        warnings.append("no_rows_with_error" if error_out else "no_rows_returned")
    ...
    return _envelope(
        page=page_norm, route_family=route_family,
        headers=headers, keys=keys, display_headers=display_headers, columns=columns,
        rows=rows, include_matrix=include_matrix_out,
        request_id=req_id, started_at=started_at, mode=mode_out,
        status=status_final, error=error_out, warnings=warnings,
        meta_extra={
