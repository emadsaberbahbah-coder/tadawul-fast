...
v5.72.0 DATA-QUALITY MERGE (May 16, 2026)
--------------------------------------------------------------------
v5.73.0 is the controlled successor to v5.70.0. The earlier v5.72.0
build was rejected because it had been authored on top of the v5.47.2
snapshot and silently regressed v5.69.0 / v5.70.0 / v5.71.0 work
(blank Confidence Score / Data Provider / Last Updated columns,
ghost Status, divergent recommendation classifier, ACCUMULATE and
AVOID in final output, rank fallback to opportunity_score). v5.73.0
starts from the approved v5.70.0 base and applies the v5.71-equivalent
architectural deltas plus the genuinely good v5.72.0 data-quality
ideas, rebuilt against the current internals rather than the v5.47.2
ones.

v5.73.0 CHANGES vs v5.70.0
  [v5.71-equivalent architectural deltas]
  - core.scoring delegation: `_recommendation`, `_risk_bucket`, and
    `_confidence_bucket` are imported from `core.scoring` (v5.3.0
    canonical scoring layer). If the import fails, the engine emits
    a conservative HOLD with recommendation_source="scoring_unavailable"
    rather than silently resurrecting an old local ladder.
  - 8-tier vocabulary collapsed to the canonical 6-tier enum:
    STRONG_BUY / BUY / HOLD / REDUCE / SELL / STRONG_SELL. ACCUMULATE
    collapses to BUY; AVOID collapses to STRONG_SELL.
  - provider_rating: any upstream provider recommendation is captured
    into the new `provider_rating` field. The final `recommendation`
    comes from core.scoring by default. The provider value is allowed
    to override only when env TFB_TRUST_PROVIDER_RECO=true.
  - recommendation_source emitted on every row:
      "engine"              — normal core.scoring decision
      "provider_override"   — TFB_TRUST_PROVIDER_RECO=true AND a
                              provider_rating was present
      "empty_row"           — empty-row guard fired
      "scoring_unavailable" — core.scoring import or call failed
  - recommendation_reason is always rewritten by the final decision
    source; no upstream reason is preserved as final.
  - _apply_rank_overall: opportunity_score fallback removed. Rows
    without overall_score remain unranked and receive a
    `rank_skipped_no_overall_score` warning tag.
  - _top10_criteria_snapshot: replaced the manual allowlist with a
    full JSON serialization capped at TFB_CRITERIA_SNAPSHOT_MAX_CHARS
    (default 2000). Truncation, when it happens, is marked with a
    `[truncated at N chars]` suffix.
  - Schema-aware cache key helper `_make_cache_key(symbol, page,
    provider_profile, schema_version)` added. The quote cache and
    singleflight key now include `_SCHEMA_VERSION`
    (= f"instrument:{len(INSTRUMENT_CANONICAL_KEYS)}:{__version__}")
    so a row cached under a previous schema version cannot leak into
    the new projection.

  [v5.72.0 cherry-picks, rebuilt against v5.70.0 internals]
  - _SUFFIX_TO_LOCALE: added MENA / non-US suffixes (.KW, .QA, .QE,
    .AE, .DFM, .ADX, .EG, .EGX, .TA, .TASE) so Kuwait, Qatar, UAE,
    Egypt, and Israel tickers no longer default to USA / USD /
    NASDAQ-NYSE. The .KSE entry is unchanged — its remapping is
    deferred to a coordinated core.symbols.normalize release.
  - Empty-row guard: `_is_empty_data_row` and `_mark_row_as_empty`
    short-circuit recommendation classification on rows where the
    provider returned no usable data. The row receives
    recommendation=HOLD, recommendation_source="empty_row", a
    canonical reason string, nulled scores, and the
    `empty_row_no_provider_data` warning tag. This preserves the
    canonical 6-value enum rather than introducing an out-of-enum NA.
  - Active outlier sanitization: `_apply_v572_sanitization` nulls
    extreme P/E, P/B, debt-to-equity, EV/EBITDA values, corrupt
    52-week bounds, and cross-currency revenue/market_cap mismatches
    BEFORE they enter scoring. Each null appends a
    `sanitized:<field>_<reason>` tag to the row's warnings. Toggle
    via env TFB_SANITIZATION_ENABLED (default true); the legacy
    TFB_DISABLE_V572_SANITIZATION is also honored for back-compat.

  [Schema policy for this patch]
  - Per the locked Q5 answer: do not expand
    _INSTRUMENT_CANONICAL_REQUIRED_KEYS in this build. v5.73.0 emits
    provider_rating, recommendation_source, scoring_errors,
    scoring_schema_version, opportunity_source, reco_priority into
    the row dict where available, but the enforced canonical sheet
    schema stays at 97 columns. Full column expansion is a
    coordinated Apps Script + schemas.py + page-module release
    handled separately.

  [Deferred / out of scope]
  - The `_as_pct_points` heuristic is preserved as-is. v5.73.0 adds
    `_fraction_to_points` as new strict-conversion infrastructure for
    engine-owned fraction-contract fields, but does NOT change any
    existing call site. v5.70.0's emit-path already stores
    percent_change in FRACTION form correctly (line ~3380's
    `abs(pct) > 1.5` branch divides by 100, the opposite of the
    v5.47.2 regression observed in the rejected v5.72.0 file).
  - .KSE suffix remapping deferred to core.symbols.normalize.
...
# =============================================================================
# v5.73.0 — core.scoring integration (canonical recommendation / bucket source)
# =============================================================================
# Per the locked Q1 answer: core.scoring v5.3.0 is the canonical scoring
# layer. The engine delegates to its `_recommendation`, `_risk_bucket`,
# and `_confidence_bucket` functions. Import is guarded; if it fails the
# engine falls back to a conservative HOLD with
# recommendation_source="scoring_unavailable" (NOT the old local ladder).
try:
...
# Canonical 6-tier vocabulary. Used for output validation only — the
# authoritative producer is core.scoring._recommendation.
_V573_RECOMMENDATION_ENUM = frozenset({
    "STRONG_BUY", "BUY", "HOLD", "REDUCE", "SELL", "STRONG_SELL",
})

# v5.73.0: collapse mapping from the legacy 8-tier vocabulary.
# ACCUMULATE was directionally positive but weaker than BUY -> BUY.
# AVOID was more bearish than STRONG_SELL in the legacy ladder ->
# STRONG_SELL (closest allowed enum). Applied only when a stray
# legacy value reaches the canonicalizer; the active classifier no
# longer emits ACCUMULATE / AVOID.
...
def _v573_trust_provider_reco() -> bool:
    """Return True only if env TFB_TRUST_PROVIDER_RECO is explicitly truthy.

    Default is False per the locked Q2 answer. When False, an upstream
    provider recommendation is captured into `provider_rating` only; the
    final `recommendation` comes from core.scoring.
    """
...
def _v573_collapse_to_canonical_enum(value: Any) -> str:
    """Map any input to the canonical 6-tier enum, or empty string.

    Legacy ACCUMULATE -> BUY, AVOID -> STRONG_SELL. Anything that is
    already in the canonical enum passes through. Unknown values become
    empty string (caller decides what to do).
    """
    if value is None:
        return ""
    ...
    if s in _V573_RECOMMENDATION_ENUM:
        return s
    # Try reco_normalize for vocabulary variants ("Strong Buy", etc.)
...
def _classify_recommendation_8tier(row: Dict[str, Any]) -> None:
    ...
    # -- Step 2: provider rating capture ----------------------------
...
# v5.73.0 — recompute schema version now that INSTRUMENT_CANONICAL_KEYS is
# defined. The cache key helper uses this to isolate cached rows produced
# under previous schema shapes.
...
def _fraction_to_points(value: Any) -> Optional[float]:
    """v5.73.0 — strict fraction -> points converter.

    For engine-owned fields that the v5.67.0 unit contract guarantees
    are stored as FRACTIONS (percent_change, upside_pct, max_drawdown_1y,
    var_95_1d, expected_roi_1m, expected_roi_3m, expected_roi_12m), use
    this helper instead of `_as_pct_points`. Unlike `_as_pct_points`,
    this helper makes no `abs(v) <= 1.5` heuristic guess — it trusts
    the contract and always multiplies by 100. That avoids the
    HCLTECH.NSE / EXE.US-style misclassification observed when
    legitimate small-fraction values like 0.00774 sit inside the
    [-1.5, 1.5] band that the heuristic interprets as fraction-form.

    Returns None for non-numeric / None inputs.

    Per the locked Q6 answer: this helper is available infrastructure
    for future use. v5.73.0 does NOT change any existing
    `_as_pct_points` call site; `_as_pct_points` remains the tolerant
    helper for provider fundamentals (margins, dividend yield) whose
    unit format is provider-dependent and ambiguous.
    """
    v = _as_float(value)
    if v is None:
        return None
...
# v5.73.0 — schema-versioned cache-key helper.
# `_SCHEMA_VERSION` is defined ABOVE this point, immediately after
# `INSTRUMENT_CANONICAL_KEYS`, so by the time `_make_cache_key` is called
# the value already reflects the canonical schema length.
...
def _make_cache_key(
    ...
    *,
    ...
    """v5.73.0 — schema-aware quote cache / singleflight key helper.

    Includes symbol, page, provider_profile, and schema_version so a
    row cached under a previous schema cannot leak into the new
    projection. `mode` distinguishes "cache" (use_cache=True) from
    "live" (use_cache=False) when keying the singleflight.

    All components are coerced through `_safe_str` and lower-cased
    only on the suffix where collisions matter; the symbol is left
    case-preserved because `normalize_symbol` is the canonicalizer.
    """
...
# v5.73.0 — empty-row guard (rebuilt against v5.70.0 internals).
#
# Per the locked Q2/Q8 contract: when a provider returns no usable
# data for a row, suppress recommendation classification and emit a
# conservative HOLD with `recommendation_source="empty_row"`. This
# preserves the canonical 6-value enum (no out-of-enum "NA").
...
def _mark_row_as_empty(row: Dict[str, Any]) -> None:
    """Apply the canonical empty-row contract to `row` in place.

    Sets recommendation=HOLD, recommendation_source="empty_row",
    a canonical reason string, nulls scores and rank, and appends
    `empty_row_no_provider_data` to the warnings string.
    """
    if not isinstance(row, dict):
        return
    ...
    row["recommendation_priority"] = 4  # canonical priority for HOLD
...
def _v573_append_warning(row: Dict[str, Any], tag: str) -> None:
    """Idempotently append a tag to the row's `warnings` string.

    Warnings are stored as "; "-joined strings per the v5.67.0 canonical
    shape. Appends only if the tag isn't already present.
    """
...
# v5.73.0 — active outlier sanitization (rebuilt against v5.70.0
# internals; the previous v5.72.0 implementation was on a v5.47.2
# base and is not reused).
#
# Toggles:
#   TFB_SANITIZATION_ENABLED       — default "true". The primary switch.
#   TFB_DISABLE_V572_SANITIZATION  — legacy. If "true", disables
#                                    sanitization regardless of the
#                                    primary switch (back-compat).

# Per-field plausibility bounds. Values outside these bounds are nulled
# (set to None) before scoring; the row's warnings get a
# `sanitized:<field>_out_of_range` tag.
...
def _v573_sanitization_enabled() -> bool:
    """Honor both the primary switch and the legacy disable flag."""
...
def _sanitize_extreme_outliers(row: Dict[str, Any]) -> int:
    """Null financial-ratio values outside `_V573_SANITIZATION_BOUNDS`.

    Returns the number of fields nulled. Appends a
    `sanitized:<field>_out_of_range` warning for each null.
    """
    if not isinstance(row, dict):
        return 0
    nulled = 0
...
def _sanitize_corrupt_52w_bounds(row: Dict[str, Any]) -> int:
    """Null impossible 52-week high/low pairs.

    Cases handled:
      - week_52_high <= 0 or week_52_low < 0
      - week_52_low > week_52_high (inverted)
      - high >= 1000x low (likely scale mismatch / corrupt data)
    """
    if not isinstance(row, dict):
        return 0
    nulled = 0
...
def _sanitize_cross_currency_revenue(row: Dict[str, Any]) -> int:
    """Detect implausible market_cap vs revenue magnitudes.

    Appends warnings rather than nulling — these are signals, not
    auto-corrections. The engine v5.72.0 force-override path may
    repair the underlying locale once core.symbols.normalize is
    consulted; until then this surfaces the inconsistency.
    """
    if not isinstance(row, dict):
        return 0
    flagged = 0
    ...
    # Very large market_cap on a USD-labeled foreign ticker (heuristic)
...
def _apply_v572_sanitization(row: Dict[str, Any]) -> Dict[str, int]:
    """Run all sanitization passes on `row`. Returns a counter dict.

    No-op when sanitization is disabled by env. Safe to call on any
    row shape; non-dict inputs return an empty counter.
    """
    if not isinstance(row, dict):
        return {}
...
_SUFFIX_TO_LOCALE: Dict[str, Tuple[str, str, str]] = {
    ...
    ".NM":    ("NASDAQ", "USD", "USA"),
    ".NG":    ("NASDAQ", "USD", "USA"),
    # v5.73.0 — MENA / additional non-US suffixes. Locked Q4 answer:
    # .KSE remains mapped to Saudi/SAR (unchanged); Kuwait is reached
    # via .KW.
...
def _apply_rank_overall(rows: List[Dict[str, Any]]) -> None:
    """v5.73.0 — rank rows by overall_score ONLY.

    Per the locked Q4/audit answer: the opportunity_score fallback that
    v5.47.2 / v5.69 / v5.70 retained is removed. Rows without
    overall_score remain unranked (rank_overall left untouched) and
    receive a `rank_skipped_no_overall_score` warning tag so audit can
    surface them.
    """
...
def _top10_criteria_snapshot(criteria: Dict[str, Any]) -> str:
    """v5.73.0 — full JSON serialization with character cap.

    Per the locked audit Q7: replaces the manual allowlist that
    silently dropped fields. Serializes the full criteria object via
    json.dumps(sort_keys=True, default=str) and caps at
    `TFB_CRITERIA_SNAPSHOT_MAX_CHARS` (default 2000). When the snapshot
    is truncated, a `... [truncated at N chars]` suffix is appended
    so audit can detect truncation.
    """
    ...
    try:
        ...
        max_chars = 2000
    if max_chars < 100:
        max_chars = 100  # never truncate below something useful
    try:
        import json
    ...
    if len(s) <= max_chars:
        return s
    # Reserve room for the truncation marker so the total length stays
    # at or below max_chars. The marker depends on len(s) (the value of
    # `n` substituted in), so compute it from the actual length.
