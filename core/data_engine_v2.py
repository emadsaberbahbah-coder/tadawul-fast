...
# Core Schemas
try:
    from core.schemas import UnifiedQuote
...
class DataEngineV4:
    ...
    async def _get_enriched_quote_impl(self, symbol: str, use_cache: bool = True) -> UnifiedQuote:
        ...
        if hasattr(quote, "latency_ms"):
            quote.latency_ms = 0.0 # Handled dynamically by outer wrapper
            
        if hasattr(quote, "data_sources"):
            quote.data_sources = merged.get("data_sources", [])
            
        if hasattr(quote, "provider_latency"):
