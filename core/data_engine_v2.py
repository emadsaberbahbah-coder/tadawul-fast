...
WHY v5.96.1 - CROSS-CHECK FIX: BODY/WICK LABEL PRECEDENCE (ALL v5.96.0 logic
              ...
              verdict change)
-------------------------------------------------------------------------------
A self cross-check of the v5.94.0-v5.96.0 structural backbone found one real
defect: _cs_body_wick_read evaluated the INDECISIVE branch BEFORE the DECISIVE
branches, so a window with a decisive MAJORITY plus a couple of small-body
(doji) bars -- e.g. 3 long-body up bars + 2 doji of 5 -- was mislabelled
INDECISIVE. Because the fusion reads that label, the bug wrongly DRAINED
conviction from a genuine trend (a DECISIVE_UP that should ADD +1 instead
SUBTRACTED -1, a 2-point swing that could downgrade STRONG->plain or
BULLISH->WEAK). Note this only affected the structural STRENGTH (and the
bodywick label / logs / future Phase 2b); st_signal direction was unaffected
since all *_BULLISH map to BULLISH. v5.96.1 evaluates the DECISIVE majority
first, so it wins; INDECISIVE now fires only when there is no decisive majority.
No other behaviour changes. Still env-gated default-OFF (byte-identical to
v5.93.0 with TFB_CANDLE_STRUCTURE off), still internal fields / no column, still
decision-neutral.

KNOWN/DISCLOSED (from the same cross-check, NOT defects): candle_structure is
computed only when the history-technicals patch runs this cycle (gap-aware
trigger; needs >=20 bars) -> the st_signal surface covers symbols whose history
was (re)fetched and falls back to the legacy chain otherwise; swing structure
spans the full ~252-bar history while volume/body/gap use recent 20/5/20 sub-
windows (intentional); a fresh 1-swing trend reaches STRONG only with BOTH
volume and body confirmation (established trends need one) -- a deliberate
gradient.

WHY v5.96.0 - STRUCTURAL READ SURFACED ON st_signal (Strategy #1, Phase 2a; ALL
              v5.95.0 logic carried; new gate default-OFF -> byte-identical)
-------------------------------------------------------------------------------
Phase 1 (v5.94.0/v5.95.0) computes the structural backbone but only LOGS it.
Phase 2a makes it VISIBLE where it is already semantically at home: the in-schema
st_signal short-term-signal field, which already blended candlestick_signal.

  _apply_analyst_trend_block's st_signal derivation now consults the structural
  bias FIRST when TFB_CANDLE_STRUCTURE_STSIGNAL is on: a clear *_BULLISH /
  *_BEARISH structural read drives st_signal (structure as the backbone,
  replacing the RSI/candlestick scalar shortcut for symbols that carry a
  structural read); RANGE / INSUFFICIENT / no-structure rows fall through to the
  legacy RSI-extreme -> candlestick -> 1M-momentum chain UNCHANGED.

WHY THIS IS SAFE (survives an adversarial audit):
  * Runs inside _apply_analyst_trend_block, which executes at the SAME final
    boundary on every path (_strict_project_row, get_page_rows, the enriched-
    quote cache/external-rows merges) BEFORE the 115-key projection -- so
    candle_structure (an INTERNAL field) is still on the row to read, and
    st_signal (one of the 115) survives the projection.
  * DECISION-NEUTRAL: st_signal is a presentation field derived AFTER the full
    scoring / investability-gate / ranking chain, so this changes NO score,
    verdict, recommendation, reliability or Top_10 rank. Vocabulary-preserving
    (BULLISH/BEARISH/NEUTRAL/OVERBOUGHT/OVERSOLD as before). Adds NO column.
  * Two-key dependency + fail-safe: the structural branch only fires when BOTH
    TFB_CANDLE_STRUCTURE (which populates candle_structure) and
    TFB_CANDLE_STRUCTURE_STSIGNAL are on AND a clear bias is present; if the
    field is absent the branch sets nothing and the legacy chain runs -> a safe
    no-op. With TFB_CANDLE_STRUCTURE_STSIGNAL off, st_signal is byte-identical
    to v5.95.0.
  * REVERSIBLE: TFB_CANDLE_STRUCTURE_STSIGNAL in {0,false,off,unset} restores
    the exact v5.95.0 st_signal derivation. Default OFF.

The richer full decomposition (trend / swings / volume / body / gap in one
place) is Phase 2b on the Insights disclosure panel; the momentum_score/RSI
replacement that MOVES verdicts remains Phase 3, env-gated and validated before
enabling.

WHY v5.95.0 - STRUCTURAL BACKBONE RESHAPED TO THE FULL STRATEGY #1 SHAPE
              (audit fix; ALL v5.94.0 logic carried; still env-gated default-OFF)
-------------------------------------------------------------------------------
A Strategy-alignment audit of the v5.94.0 backbone found it implemented only
THREE of the four structural elements Strategy #1 names. v5.95.0 closes the gap
so analyze_candle_structure covers all four AND fuses them auditably:

  (1) swing structure  -- DEEPENED: _cs_struct_run reports the multi-swing run
      length (consecutive HH/HL), so an established 3-swing trend is no longer
      read identically to a 1-swing one (v5.94.0 compared only the last pair).
  (2) body/wick geometry -- ADDED (was MISSING in v5.94.0): _cs_body_wick_read
      reads recent body_ratio + shadows via _cs_bar_geom into a CONVICTION
      signal DECISIVE_UP/DECISIVE_DOWN/INDECISIVE/UPPER_REJECTION/
      LOWER_REJECTION -- the Strategy's element #2, distinct from named-pattern
      labelling.
  (3) volume confirmation -- unchanged.
  (4) gap detection -- now FUSED into the bias (was report-only): a gap with
      the trend adds conviction, a gap against it subtracts.

FUSION (auditable, NOT a black box): trend is the primary axis; volume, body/
wick and gap each contribute +1 when they agree with the trend and -1 when they
contradict it, giving a bounded conviction score the label is read from --
STRONG_/plain/WEAK_ BULLISH/BEARISH or NEUTRAL. WEAK now means "a real trend
contradicted by its own bars/volume/gap" (the early-warning / thesis-decay read
the Strategy wants). Every element is ALSO surfaced as its own
candle_structure_* field (now incl. candle_structure_bodywick) so the composite
can always be decomposed.

UNCHANGED FROM v5.94.0: still computed on the SAME already-fetched OHLCV bars
(zero new I/O); still INTERNAL candle_structure_* fields the 115-key projection
STRIPS (NO schema column, My_Portfolio 122 / market 115 untouched); still NO
score/verdict/recommendation/reliability change; named patterns preserved as
labeled context on top; pure + fail-open. REVERSIBLE: TFB_CANDLE_STRUCTURE in
{0,false,off,unset} -> history patch + every downstream value byte-identical to
v5.93.0. Default OFF. The momentum_score/RSI replacement and the Insights /
st_signal surfacing remain the next env-gated phases.

WHY v5.94.0 - STRUCTURAL CANDLESTICK BACKBONE (Strategy #1, Phase 1; ALL v5.93.0
              logic carried verbatim; env-gated default-OFF -> byte-identical)
-------------------------------------------------------------------------------
The project Strategy requires candlestick analysis whose BACKBONE is structural
-- multi-day swing structure (higher-highs/higher-lows), volume confirmation,
and gap detection -- with named patterns demoted to labeled context on top. The
engine already detected named patterns (PHASE-JJ: Doji/Hammer/Engulfing/Star/
Marubozu) but had NO swing structure (only a first-vs-last close trend), NEVER
used the per-bar volume it already coerces, and detected NO gaps. v5.94.0 adds
that backbone:

  analyze_candle_structure(rows) computes, on the SAME already-fetched OHLCV
  history bars the named detectors use (so ZERO new fetch / edge cost):
    * swing structure -- _cs_struct_pivots finds swing pivot highs/lows
      (k bars each side), _cs_swing_structure classifies the last two of each
      as HH/HL/LH/LL and reads UPTREND (HH+HL) / DOWNTREND (LH+LL) / RANGE;
    * volume confirmation -- latest bar's volume vs its ~20-bar average
      -> CONFIRMED_UP / CONFIRMED_DOWN / WEAK / NEUTRAL;
    * gap detection -- most-recent open-vs-prior-close gap (UP/DOWN/NONE),
      its signed percent, and a window gap count;
  fused into a structural_bias (STRONG_/WEAK_ BULLISH/BEARISH / NEUTRAL) and a
  human-readable summary.

WIRING + SAFETY (why this survives an adversarial audit):
  * Called once inside _compute_history_patch_from_rows, right after the named
    detectors, ONLY when TFB_CANDLE_STRUCTURE is enabled. Merged as INTERNAL
    candle_structure_* fields -- the 115-key strict projection STRIPS them
    exactly like trust_level, so this adds NO schema column and the My_Portfolio
    122 / market 115 contracts are untouched.
  * NO score / verdict / recommendation / reliability change. The named-pattern
    fields (candlestick_*) are PRESERVED verbatim. This phase MEASURES and
    DISCLOSES structure; it does NOT yet replace the momentum_score/RSI shortcut
    (that, and the Insights / st_signal surfacing, are the next env-gated phases,
    each individually reversible and validated before enabling).
  * Pure + fail-open: any error in the structural read returns the INSUFFICIENT
    empty dict and can never break the history patch.
  * REVERSIBLE: TFB_CANDLE_STRUCTURE in {0,false,off,unset} -> the history patch,
    every downstream score/verdict, and the projected rows are byte-identical to
    v5.93.0. Default OFF.

WHY v5.93.0 - GHOST-TICKER / NAME-UNRESOLVED DISCLOSURE TAG (visibility, not a
              decision change; ALL v5.92.0 logic carried verbatim)
-------------------------------------------------------------------------------
The trust system already BARS a priced-but-nameless ghost from decisions:
v5.88.0 bands data_quality into trust_level + tags low_data_trust + withholds
LOW rows from Rank (Overall); v5.89.0 _row_is_degenerate forces LOW for a
nameless row with a degenerate 52W range (the 5023.SR case); v5.90.0 carries
trust_level through projection so the ranker actually sees it. What was missing
was a PRECISE, audit-surfaceable signal: low_data_trust is over-broad (it fires
for any low-DQ / blocked / no-price row), so it cannot cleanly drive a
"ghost ticker" alert. v5.93.0 adds a distinct name_unresolved warning tag.

_apply_investability_gate now appends name_unresolved to a PRICED row whose
display name never resolved -- blank, or still equal to its own symbol (the
ghost signature: 5023.SR rendered with name == "5023.SR"). _row_name_is_unresolved
is pure / fail-safe; has_price (the gate local) scopes the tag to the ghost case
(priced but nameless) and away from a no-data empty row (already tagged
empty_row_no_provider_data). The tag is DISCLOSURE-ONLY -- it NEVER demotes;
the trust system still governs ranking -- and is deliberately broader than the
v5.89.0 demotion trigger (which also requires a degenerate range), because
disclosing a missing name is safe even for a real-but-unnamed instrument.
audit_data_quality.py v4.8.0 maps name_unresolved -> a GHOST_TICKER issue that
rolls into the Insights Data Quality Alerts section.

Tag-only: NO schema (115 / My_Portfolio 122 unchanged), recommendation, verdict,
reliability, or scoring change. name_unresolved is substring-safe for the
reliability scan (no cap/forecast/target/roi/drop/reject), so a repeat gate pass
is byte-identical. Reversible: TFB_NAME_UNRESOLVED_TAG in {0,false,off} appends
no tag -> warnings byte-identical to v5.92.0; default ON.

WHY v5.92.0 - MY_PORTFOLIO POSITION MATH IS AUTHORITATIVE, NOT FILL-ONLY (Fix AH)
-------------------------------------------------------------------------------
Fix AF (v5.85.4) gave the external-rows path that serves My_Portfolio a
_compute_position_math() that finally multiplied the manual Position Qty x Avg
Cost / current_price into the four derived columns. But it wrote them FILL-ONLY
(each guarded "... is None"), as did the engine-fetch twin in
_canonicalize_provider_row. Correct for a first-ever fill; wrong on every
refresh after. position_cost is price-independent (qty*avg_cost) so it stayed
right -- but the three PRICE-DEPENDENT fields (position_value, unrealized_pl,
unrealized_pl_pct) were computed ONCE and never recomputed when current_price
moved. The external-rows merge preserves the sheet's stored values, so each run
re-ingested the prior run's position_value, fill-only skipped past the correct
formula, and the holding stopped reconciling: Value - Cost no longer equalled
the stored P/L and the stored P/L% no longer matched cost basis (it tracked
whatever price the field was last written at, not today's).

THE FIX (Fix AH): _compute_position_math now recomputes the price-dependent
fields AUTHORITATIVELY -- every run, overriding any stored value -- whenever the
inputs are present. Each step is still guarded on its OWN inputs, so a missing
qty / avg_cost / price leaves that field untouched (never blanks, never raises),
and the recompute cascades (value, cost, then pl = value - cost, then
pct = pl / cost * 100) so the four can never disagree. position_cost is
recomputed too, purely for internal consistency; its value does not change
because it never depended on price. The provider twin in
_canonicalize_provider_row is left fill-only: it runs on provider patches that
carry no manual qty/cost, so its position math is a no-op there and cannot go
stale the same way.

SCOPE / LIMITS: _compute_position_math runs only from _compute_portfolio_fields,
and every recompute is gated on position_qty -- a field only My_Portfolio
holdings carry -- so no market or opportunity row is touched on any call path.
This does NOT repair a bad INPUT: a transposed Avg Cost (e.g. 250 entered for a
~3.50 stock) still yields the exact arithmetic of that wrong input. It
guarantees only that the four derived fields are always the cost-basis math of
whatever qty/cost the row holds and the live price -- i.e. they reconcile.

REVERSIBLE: TFB_POSITION_MATH_AUTHORITATIVE in {0,false,no,n,off,f,disabled,
disable} collapses the override back to the v5.91.0 "... is None" guard, so
My_Portfolio rows are byte-identical to v5.91.0. Default ON. Every other page is
byte-identical to v5.91.0 regardless of the switch (no non-portfolio row carries
position_qty).

WHY v5.91.0 - RELIABILITY CALIBRATION ADJUST (self-correction loop "Adjust" step)
-------------------------------------------------------------------------------
THE LOOP: the engine STATES a forecast_reliability_score and an investability
verdict, and these WHY-blocks have said all along that the number "remains an
unbacktested ESTIMATE -- only a forward-return backtest can [ground it]."
scripts/track_performance.py (v6.8.0) now MEASURES the gap (realized win-rate
vs stated confidence, per bucket, with Wilson intervals and a minimum-sample
gate). v5.91.0 adds the engine-side complement: a switch to APPLY that
measured calibration to the displayed reliability -- the "Adjust" step --
without the engine ever reading a sheet.

THE FIX (env TFB_CALIBRATION_ADJUST, default OFF -> forecast_reliability_score
byte-identical to v5.90.0):
  * _apply_reliability_calibration multiplies the reliability by the OFFLINE-
    computed factor for this row's investability bucket. Factors are supplied
    as a simple CSV, e.g.  TFB_CALIBRATION_FACTORS="INVESTABLE:0.85,WATCHLIST:
    0.95"  (exactly the per-bucket factors the calibrator prints). Read at
    call time; NEVER from a live sheet -> zero edge-path I/O, no Render-timeout
    exposure. Empty/unparseable factors, or the master switch off, makes the
    whole block a no-op.
  * DECISION-NEUTRAL FOR THE GATE VERDICT, but NOT system-wide -- read this
    before enabling. The hook runs AFTER the entire verdict chain (incl. the
    strict-gate reliability floor at _strict_invest_reliability_min), so the
    engine's own INVESTABLE/WATCHLIST/BLOCKED verdict is decided on the RAW
    score (and _top10_row_is_eligible does not read reliability either).
    HOWEVER, forecast_reliability_score is a PUBLISHED field and the DOWNSTREAM
    Top_10 builder (opportunity_builder) floors on it (~70, per the v5.83.3
    note), so a haircut that drops a name below ~70 will TIGHTEN Top_10
    selection even though the gate verdict is unchanged. That is the intended
    "Adjust" effect -- but validate it against a Top_10 dry-run before enabling
    in production. Feeding the calibrated score back into the gate VERDICT
    (demote on calibration) is a separate, default-OFF follow-on, NOT here.
  * DOUBLE-CALIBRATION CAUTION (operational): while the haircut is ON,
    track_performance records the CALIBRATED forecast_reliability_score as the
    entry reliability, so a later --calibrate would compute factors on already-
    adjusted numbers (they compound). The engine exposes no raw-reliability
    column, so to re-derive clean factors run a measurement window with the
    haircut OFF (or account for the compounding). Moot until factors exist.
  * CONSERVATIVE by default: TFB_CALIBRATION_DOWNWARD_ONLY (default ON) clamps
    an applied factor to <= 1.0, so the hook only ever HAIRCUTS an over-claimed
    reliability, never inflates it (the honest-uncertainty direction). The
    calibrator already shrinks factors toward 1.0 on thin samples, so a factor
    of ~1.0 is a no-op -- nothing auto-tunes on noise.
  * Adds NO column (schema 115); a touched row is tagged reliability_calibrated
    in warnings. TAG-SUBSTRING SAFETY: the reliability penalties scan warnings
    for cap/forecast/target/roi and provider_target+drop/reject; the tag
    contains NONE of those, so a SECOND gate pass over a tagged row computes
    the identical reliability (the calibration itself is idempotent given the
    same factor). Reversible: TFB_CALIBRATION_ADJUST unset restores v5.90.0
    exactly.

DOCTRINE (project-wide): detection gates the decision; correction is DISCLOSED,
never silent; nothing auto-tunes on noise; every adjustment is reversible and
attributable. This hook is the engine half of that loop -- dormant until the
operator validates the calibrator's factors and chooses to feed them in.
...
def _intrinsic_display_cap_enabled() -> bool:
    """v5.87.0 (Fix AG): master switch for the intrinsic-value DISPLAY ceiling
    (default OFF -> intrinsic_value / upside_pct byte-identical to v5.86.0). Set
    TFB_INTRINSIC_DISPLAY_CAP to 1/true/on to clamp the displayed fair value (and
    the Top_10 valuation upside the builder derives from it) at
    TFB_INTRINSIC_DISPLAY_MAX_PCT above current price."""
    raw = (os.getenv("TFB_INTRINSIC_DISPLAY_CAP") or "").strip().lower()
...
def _cap_intrinsic_display(row: Dict[str, Any]) -> None:
    ...
    (cap/forecast/target/roi/drop/reject) so a repeat gate pass is byte-identical.
    Note: this caps the intrinsic-value basis only; an implausible analyst
    target_price (a separate field) remains the Top_10 builder's job (its v1.0.4
...
# =============================================================================
# v5.91.0 RELIABILITY CALIBRATION ADJUST (self-correction loop "Adjust" step)
# -----------------------------------------------------------------------------
# Engine half of the calibration loop: APPLY the offline-measured calibration
# factor (from scripts/track_performance.py --calibrate) to the DISPLAYED
# forecast_reliability_score. Default OFF; decision-neutral; schema-safe; fed
# from env (no live sheet read -> zero edge-path I/O). See the v5.91.0 WHY block.
# =============================================================================
_CALIBRATION_TAG = "reliability_calibrated"  # substring-safe (no cap/forecast/
#                                              target/roi/drop/reject)
_CALIBRATION_FACTOR_MIN: float = 0.1
_CALIBRATION_FACTOR_MAX: float = 2.0


def _calibration_adjust_enabled() -> bool:
    """v5.91.0: master switch for the reliability-calibration ADJUST hook
    (default OFF). When off, forecast_reliability_score is byte-identical to
    v5.90.0. Set TFB_CALIBRATION_ADJUST to 1/true/on to enable."""
    raw = (os.getenv("TFB_CALIBRATION_ADJUST") or "").strip().lower()
    return raw in {"1", "true", "yes", "y", "on", "enabled", "enable"}


def _calibration_downward_only() -> bool:
    """v5.91.0: when ON (default), an applied calibration factor is clamped to
    <= 1.0 so the hook only ever HAIRCUTS an over-claimed reliability, never
    inflates it (the conservative, honest-uncertainty direction). Set
    TFB_CALIBRATION_DOWNWARD_ONLY=0/false/off to allow upward adjustment."""
    raw = (os.getenv("TFB_CALIBRATION_DOWNWARD_ONLY") or "").strip().lower()
    return raw not in {"0", "false", "no", "n", "off", "disabled", "disable"}


def _calibration_factors() -> Dict[str, float]:
    """v5.91.0: per-investability-bucket calibration multipliers, supplied
    OFFLINE (computed by scripts/track_performance.py --calibrate against
    matured outcomes) and pasted as a simple CSV, e.g.
    TFB_CALIBRATION_FACTORS="INVESTABLE:0.85,WATCHLIST:0.95". Read at call
    time; clamped to [MIN, MAX]. Empty/unparseable -> {} (the hook is then a
    no-op even when the master switch is on). NEVER read from a live sheet."""
    raw = (os.getenv("TFB_CALIBRATION_FACTORS") or "").strip()
    if not raw:
        return {}
    out: Dict[str, float] = {}
    for part in raw.split(","):
        ...
        try:
            ...
            continue
        if key and fv > 0.0:
            out[key] = max(_CALIBRATION_FACTOR_MIN, min(_CALIBRATION_FACTOR_MAX, fv))
    return out


def _apply_reliability_calibration(
    rel: float, status: str
) -> Tuple[float, Optional[float]]:
    """v5.91.0: apply the offline calibration factor for this row's
    investability bucket to the (already-decided) reliability score. Returns
    (adjusted_rel, factor_applied) or (rel, None) when no factor applies.

    DECISION-NEUTRAL: callers invoke this AFTER the verdict chain, so the
    INVESTABLE/WATCHLIST/BLOCKED verdict (incl. the strict-gate reliability
    floor) was already decided on the RAW score; only the DISPLAYED number is
    calibrated. Idempotent given the same factor. Schema-safe (no new column)."""
    factors = _calibration_factors()
    if not factors:
        return rel, None
    factor = factors.get((status or "").upper())
    if factor is None:
        return rel, None
    if _calibration_downward_only() and factor > 1.0:
        factor = 1.0
    if factor == 1.0:
        return rel, None  # no-op (shrunk-to-1 on thin data, or downward-clamped)
    adj = round(max(0.0, min(100.0, rel * factor)), 1)
    if adj == rel:
        return rel, None
    return adj, factor
...
# =============================================================================
# v5.88.0 DATA-TRUST LEVEL (Engineering Audit Phase 0 -- trust source-of-truth)
# -----------------------------------------------------------------------------
# WHY: the engine DETECTS thin/sparse coverage (the "Live quote sparse" warning;
# a low data_quality_score) but historically still emitted a CONFIDENT ranked
# output -- a sparse name like 5023.SR received a market-page Rank (Overall)
# regardless of how little the engine actually knew about it. The unifying
# Phase-0 rule ("detection must gate the decision") is closed here at the
# source: an explicit trust_level is BANDED from the completeness signal the
# gate already computes, and a LOW-trust row is withheld from ranking.
#
# DESIGN (deliberately minimal, schema-safe):
#   * trust_level is DERIVED, not re-computed -- it bands the EXISTING
#     data_quality_score (weighted completeness) plus the gate's BLOCKED verdict
#     and the has-usable-price flag. No new provider calls, no parse risk.
#   * It is an INTERNAL field, NOT a schema column (INSTRUMENT_CANONICAL_KEYS
#     stays 115). It drives _apply_rank_overall's trust skip, tags LOW rows in
#     warnings (visible on the sheet), and is available to the Top_10 builder /
#     API. Promoting it to a visible "Trust Level" column is a deliberate
#     follow-up (engine + schema_registry + 00_Config, coordinated) once the
#     band distribution is validated -- NOT done here, to avoid the schema-
#     projection failure class.
#   * FRESHNESS is intentionally NOT an input: the engine stamps
#     last_updated_utc at fetch (so it is ~now at emit and a non-differentiator
#     engine-side). Quote staleness is the CONSUMER's concern and is already
#     enforced by opportunity_builder / portfolio_actions' local trust gates.
#   * Consumers need NO re-wiring for the Top_10 path: opportunity_builder's
#     existing data_quality MAJOR gate (dq >= 80) already excludes the same
#     low-completeness names; trust_level makes the verdict explicit and adds
#     the market-page ranking drop the consumers could not apply.
#
# Reversible: TFB_TRUST_LEVEL=0 -> no trust_level is written and the ranking
# applies no trust skip, so the projected rows (and ranks) are byte-identical
# to v5.87.0. LOW only ever demotes (drops from rank); it never promotes a row.
#   TFB_TRUST_LEVEL      default ON  (0/false/off to disable)
#   TFB_TRUST_DQ_HIGH    default 80.0 (data_quality_score at/above -> HIGH)
#   TFB_TRUST_DQ_LOW     default 55.0 (data_quality_score below   -> LOW)
#
# v5.89.0 DEGENERATE-FIELD DEMOTION (Phase 0 -- completeness != trustworthiness)
# -----------------------------------------------------------------------------
# data_quality_score weights price completeness heavily, so a row that HAS a
# price but is otherwise a synthetic placeholder -- blank name, a near-zero-width
# 52-week range (5023.SR: 102.00<->102.30), a momentum-only scoring fallback --
# still clears the LOW band (5023.SR dq 63.2 -> MEDIUM) and kept a market-page
# rank under v5.88.0. v5.89.0 adds a structural-degeneracy check
# (_row_is_degenerate) that forces such a row to LOW regardless of dq, so it is
# withheld from Rank (Overall) and tagged low_data_trust like any other LOW row.
# This is the precise gap the v5.82.0 thin-history guard (Fix W) leaves open:
# Fix W demotes a MISSING 52W range (w_hi/w_lo non-positive), but a DEGENERATE
# positive range (hi ~= lo) passes its `hi > 0 and lo > 0` test untouched. Two
# deliberately conservative conditions (see _row_is_degenerate): (1) the scorer's
# own momentum_only_fallback flag, standalone and asset-class-safe; OR (2) a
# degenerate 52W range AND a blank name TOGETHER -- both halves required so the
# trigger spares a named pegged / low-vol instrument and a real-but-unnamed one.
# Reversible: TFB_TRUST_DEGENERATE_GUARD=0 -> trust_level (and ranks)
# byte-identical to v5.88.0. LOW still only ever demotes.
#   TFB_TRUST_DEGENERATE_GUARD       default ON   (0/false/off to disable)
#   TFB_TRUST_DEGENERATE_RANGE_PCT   default 0.01 (52W width/price below -> degenerate)
# =============================================================================
_TRUST_DQ_HIGH_DEFAULT: float = 80.0
_TRUST_DQ_LOW_DEFAULT: float = 55.0
_TRUST_DEGENERATE_RANGE_PCT_DEFAULT: float = 0.01

TRUST_LEVEL_LOW = "LOW"
TRUST_LEVEL_MEDIUM = "MEDIUM"
TRUST_LEVEL_HIGH = "HIGH"

# v5.88.0: appended to warnings on a LOW-trust row. Avoids the reliability-scan
# substrings (cap/forecast/target/roi/drop/reject) so a second gate pass over a
# tagged row stays byte-identical.
_TRUST_LOW_TAG = "low_data_trust"

# v5.93.0: appended to warnings on a PRICED-but-NAMELESS row -- the ghost-ticker
# signature: a usable price but no resolved display name (e.g. 5023.SR priced
# 101.95 with name == symbol). DISCLOSURE tag, NOT a demotion: the v5.88.0-
# v5.90.0 trust system already withholds such a row from Rank (Overall); this
# only turns the condition into a NAMED, audit-surfaceable signal that
# audit_data_quality.py maps to a GHOST_TICKER issue + Insights Data Quality
# Alert. Deliberately broader than _row_is_degenerate's DEMOTION trigger (which
# also requires a degenerate 52W range): disclosing a missing name is safe even
...
def _name_unresolved_tag_enabled() -> bool:
    """v5.93.0: sub-switch for the priced-but-nameless DISCLOSURE tag (default
    ON). When OFF, _apply_investability_gate appends no name_unresolved tag, so
    the row's warnings are byte-identical to v5.92.0. Decision-neutral either
    way -- the trust system, not this tag, governs ranking. Set
    TFB_NAME_UNRESOLVED_TAG to 0/false/off."""
    raw = (os.getenv("TFB_NAME_UNRESOLVED_TAG") or "").strip().lower()
    if raw in {"0", "false", "no", "n", "off", "f", "disabled", "disable"}:
        return False
    return True


def _trust_level_enabled() -> bool:
    """v5.88.0: master switch for the data-trust level + its ranking drop
    (default ON). When OFF, _apply_investability_gate writes no trust_level and
    _apply_rank_overall applies no trust-based skip, so the projected rows and
    ranks are byte-identical to v5.87.0. Set TFB_TRUST_LEVEL to 0/false/off."""
    raw = (os.getenv("TFB_TRUST_LEVEL") or "").strip().lower()
    if raw in {"0", "false", "no", "n", "off", "f", "disabled", "disable"}:
        return False
    return True


def _trust_dq_high() -> float:
    """v5.88.0: data_quality_score (0-100 pts) at/above which trust_level is
    HIGH (default 80, floored 60). Read at call time."""
    v = _get_env_float("TFB_TRUST_DQ_HIGH", _TRUST_DQ_HIGH_DEFAULT)
    return v if (v is not None and v >= 60.0) else _TRUST_DQ_HIGH_DEFAULT


def _trust_dq_low() -> float:
    """v5.88.0: data_quality_score (0-100 pts) below which trust_level is LOW
    (default 55, floored 20). Read at call time and kept strictly below the
    HIGH band so the three bands never collapse."""
    v = _get_env_float("TFB_TRUST_DQ_LOW", _TRUST_DQ_LOW_DEFAULT)
    if v is None or v < 20.0:
        v = _TRUST_DQ_LOW_DEFAULT
...
def _derive_trust_level(dq: Optional[float], status: str,
    ...
    trust verdict. LOW when the gate already BLOCKED the row, when no usable
    current price exists (the sparse / 'Live quote sparse' case), or when
    completeness is below the LOW band; HIGH when completeness is at/above the
    HIGH band with a usable price and a non-blocked verdict; MEDIUM otherwise.
    Pure banding of values already computed in _apply_investability_gate -- no
    provider calls, no freshness parsing (see the WHY block above).

    v5.89.0: also LOW when `degenerate` is True -- a structurally broken row
    (blank name / degenerate 52W range / momentum-only scoring fallback) that
    nonetheless clears the completeness band (see _row_is_degenerate). It sits
    with the other LOW conditions so it composes with them. The caller computes
    `degenerate` only when the guard is enabled, so with the guard OFF this is
    byte-identical to v5.88.0."""
    if status == "BLOCKED" or not has_price:
        return TRUST_LEVEL_LOW
    if degenerate:
        return TRUST_LEVEL_LOW
    if dq is not None and dq < _trust_dq_low():
        return TRUST_LEVEL_LOW
    if dq is not None and dq >= _trust_dq_high():
        return TRUST_LEVEL_HIGH
    return TRUST_LEVEL_MEDIUM


def _trust_degenerate_guard_enabled() -> bool:
    """v5.89.0: sub-switch for the structural-degeneracy demotion (default ON).
    When OFF, _row_is_degenerate always returns False so trust_level is
    byte-identical to v5.88.0. Set TFB_TRUST_DEGENERATE_GUARD to 0/false/off."""
    raw = (os.getenv("TFB_TRUST_DEGENERATE_GUARD") or "").strip().lower()
    if raw in {"0", "false", "no", "n", "off", "f", "disabled", "disable"}:
        return False
    return True


def _trust_degenerate_range_pct() -> float:
    """v5.89.0: 52-week range WIDTH as a fraction of the reference price below
    which the range is treated as degenerate (no real annual history). Default
    0.01 (1%), floored at 0 and capped at 0.10 so it can never demote a normal
    book; read at call time."""
    v = _get_env_float("TFB_TRUST_DEGENERATE_RANGE_PCT",
                       _TRUST_DEGENERATE_RANGE_PCT_DEFAULT)
    if v is None or v < 0.0:
        return _TRUST_DEGENERATE_RANGE_PCT_DEFAULT
    return v if v <= 0.10 else _TRUST_DEGENERATE_RANGE_PCT_DEFAULT


def _row_is_degenerate(row: Dict[str, Any]) -> bool:
    """v5.89.0: True when a row is STRUCTURALLY degenerate -- complete enough to
    clear the data_quality band yet not a real, evaluable instrument. Demotes on
    EITHER of two deliberately conservative conditions:

      1. a momentum-only scoring fallback -- the scorer ITSELF reported it lacked
         the inputs for its full model (opportunity_source / recommendation_source
         / scoring_recommendation_source == 'momentum_only_fallback'). An explicit,
         asset-class-agnostic "could not evaluate" signal; standalone.

      2. a near-zero-width 52-week range AND a blank name TOGETHER -- the synthetic
         placeholder signature (5023.SR: nameless, 102.00<->102.30). BOTH halves
         are required so the trigger cannot fire on a *named* pegged / low-vol
         instrument (a real FX peg keeps its name) nor on a *real* instrument that
         is merely missing its display name (its 52W range is real). Width is
         (hi - lo) / ref below the degenerate threshold, hi/lo both present and
         positive -- the v5.82.0 thin-history guard only catches a MISSING range.

    Both signals hold for 5023.SR, so it is demoted under either condition.
    Returns False when the guard is disabled (TFB_TRUST_DEGENERATE_GUARD=0)."""
    if not _trust_degenerate_guard_enabled():
        return False
    # 1) the scorer's own fallback flag -- standalone, asset-class-safe.
    for _k in ("opportunity_source", "recommendation_source",
               "scoring_recommendation_source"):
        if _safe_str(row.get(_k)).strip().lower() == "momentum_only_fallback":
            return True
    # 2) synthetic-placeholder signature: degenerate 52W range AND blank name
    #    (both required -- spares named pegs and real-but-unnamed instruments).
    if not _safe_str(row.get("name")).strip():
        w_hi = _as_float(row.get("week_52_high"))
        w_lo = _as_float(row.get("week_52_low"))
        if (w_hi is not None and w_lo is not None and
                w_hi > 0.0 and w_lo > 0.0 and w_hi >= w_lo):
            ref = _as_float(row.get("current_price"))
            if ref is None or ref <= 0.0:
                ref = w_hi
            if ref > 0.0 and ((w_hi - w_lo) / ref) < _trust_degenerate_range_pct():
                return True
    return False


def _row_name_is_unresolved(row: Dict[str, Any]) -> bool:
    """v5.93.0: True when a row has NO resolved display name -- either blank or
    still equal to its own symbol (the ghost signature; 5023.SR surfaced as
    name == "5023.SR"). The symbol comparison is normalized + case-insensitive
    and also tested against normalize_symbol(symbol), so a raw-vs-normalized
    echo is caught. Pure / fail-safe (never raises). Whether the resulting tag
    is EMITTED is gated by _name_unresolved_tag_enabled() and scoped to PRICED
    rows at the call site, so a no-data empty row (already tagged
    empty_row_no_provider_data) is not double-flagged."""
    name = _safe_str(row.get("name")).strip()
    if not name:
        return True
    sym = _safe_str(
        row.get("symbol") or row.get("ticker") or row.get("requested_symbol")
    ).strip()
    if not sym:
        return False
    if name.upper() == sym.upper():
        return True
    try:
        nsym = _safe_str(normalize_symbol(sym)).strip()
    except Exception:
        nsym = ""
    if nsym and name.upper() == nsym.upper():
        return True
    return False
...
def _apply_investability_gate(row: Dict[str, Any]) -> None:
    ...
    # -- v5.88.0: DATA-TRUST LEVEL (Phase 0 trust source-of-truth) ------------
    # Band the completeness signal just computed (dq) together with the final
    # verdict (status) and the usable-price flag into an explicit LOW/MEDIUM/
    # HIGH trust_level, and tag LOW rows in warnings. trust_level is an INTERNAL
    # field (NOT a schema column -- INSTRUMENT_CANONICAL_KEYS stays 115); it
    # drives _apply_rank_overall's trust skip and is available to the Top_10
    # builder / API. Reversible: TFB_TRUST_LEVEL=0 -> trust_level unset, no
    # ranking effect, projected rows byte-identical to v5.87.0.
    if _trust_level_enabled():
        # v5.89.0: structural-degeneracy demotion -- a priced-but-synthetic row
        # (blank name / degenerate 52W range / momentum-only fallback) is forced
        # to LOW even when its data_quality_score would otherwise band MED/HIGH.
        _degenerate = _row_is_degenerate(row)
        trust_level = _derive_trust_level(dq, status, has_price, _degenerate)
    ...
    # identical to v5.92.0). Independent of the trust master above: a PRICED row
    # whose display name never resolved (blank or == symbol) gets a NAMED
    # warning so audit_data_quality.py can surface a GHOST_TICKER Insights Data
    # Quality Alert. has_price (gate local, set above) scopes it to the ghost
    # case (priced but nameless, e.g. 5023.SR) and away from a no-data empty row
    # (already empty_row_no_provider_data). DISCLOSURE-ONLY -- the trust system,
    # not this tag, governs ranking; _v573_append_warning de-dupes so a second
    # gate pass is idempotent.
    if _name_unresolved_tag_enabled() and has_price and _row_name_is_unresolved(row):
        _v573_append_warning(row, _NAME_UNRESOLVED_TAG)

    # -- v5.91.0: RELIABILITY CALIBRATION ADJUST (Phase 0 -- self-correction
    # loop "Adjust" step; default OFF -> forecast_reliability_score byte-
    # identical to v5.90.0). Applies the OFFLINE-computed calibration factor
    # for this row's investability bucket (see _apply_reliability_calibration)
    # to the DISPLAYED reliability. DECISION-NEUTRAL: this runs AFTER the entire
    # verdict chain (incl. the strict-gate reliability floor), so the verdict
    # was decided on the RAW score; only the shown number is calibrated. The
    # factor is fed via TFB_CALIBRATION_FACTORS (no live sheet read -> zero
    # edge-path I/O); empty factors or the switch off make this a no-op. Adds
    # NO column (schema 115); a touched row is tagged reliability_calibrated
    # (substring-safe). Feeding the calibrated score BACK into the gate verdict
    # is a deliberate, separately-gated follow-on, NOT done here.
    if _calibration_adjust_enabled():
...
# =============================================================================
# v5.94.0 PHASE-JJ-2 — STRUCTURAL CANDLESTICK BACKBONE (Strategy #1, Phase 1)
# -----------------------------------------------------------------------------
# Swing structure (pivot higher-highs/higher-lows), volume confirmation, and gap
# detection -- the structural backbone the project's Strategy requires, with the
# named patterns above (detect_candlestick_patterns) DEMOTED to labeled context
# on top. Computed on the SAME already-fetched OHLCV history bars that the named
# detectors use (zero new I/O, no new edge cost). Returns INTERNAL
# candle_structure_* fields that the 115-key projection STRIPS exactly like
# trust_level -- so NO schema column is added and NO score / verdict /
# recommendation changes. Reuses _cs_coerce_bars (full OHLCV) + _cs_bar_geom.
# PURE + FAIL-OPEN: any error returns the empty structural dict; it can never
# break the history patch. Surfaced/used only behind TFB_CANDLE_STRUCTURE
# (default OFF -> the patch is byte-identical to v5.93.0).
# =============================================================================
_CS_STRUCT_PIVOT_K = 2              # bars each side that define a swing pivot
_CS_STRUCT_MIN_BARS = 8            # min bars before structure is meaningful
_CS_STRUCT_SWING_EPS = 0.001       # 0.1% deadband so equal pivots aren't "higher"
_CS_STRUCT_VOL_WINDOW = 20          # bars for the average-volume baseline
_CS_STRUCT_VOL_CONFIRM_RATIO = 1.20  # latest vol >= 1.20x avg -> confirmed
_CS_STRUCT_VOL_WEAK_RATIO = 0.70     # latest vol <= 0.70x avg -> weak move
_CS_STRUCT_GAP_PCT = 0.01           # 1% open-vs-prev-close -> a gap
_CS_STRUCT_GAP_WINDOW = 20          # bars to scan for the most-recent gap


def _cs_struct_pivots(
    bars: List[Dict[str, float]], k: int
) -> Tuple[List[Tuple[int, float]], List[Tuple[int, float]]]:
    """Swing pivot highs/lows. Index i is a pivot high if bars[i]['high'] is
    >= every high in [i-k, i+k] AND strictly above at least one neighbour on
    each side (so a flat shelf is not a pivot on every bar); pivot low is the
    mirror. Returns (highs, lows) as ordered [(idx, price), ...]."""
    ...
    for i in range(k, n - k):
        ...
        is_ph = True
        is_pl = True
        left_lower_h = right_lower_h = False
        left_higher_l = right_higher_l = False
        for j in range(i - k, i + k + 1):
            ...
            if bars[j]["low"] < lo:
                is_pl = False
            if j < i and bars[j]["high"] < hi:
                left_lower_h = True
            if j > i and bars[j]["high"] < hi:
                right_lower_h = True
            if j < i and bars[j]["low"] > lo:
                left_higher_l = True
            if j > i and bars[j]["low"] > lo:
                right_higher_l = True
        if is_ph and left_lower_h and right_lower_h:
            highs.append((i, hi))
        if is_pl and left_higher_l and right_higher_l:
            lows.append((i, lo))
    return highs, lows


def _cs_struct_seq(points: List[Tuple[int, float]], eps: float) -> str:
    """Classify the last two pivots: 'H' (higher), 'L' (lower), '=' (within
    eps), or '' (fewer than two pivots)."""
    if len(points) < 2:
        return ""
    prev = points[-2][1]
    last = points[-1][1]
...
def _cs_struct_run(points: List[Tuple[int, float]], eps: float) -> Tuple[str, int]:
    """v5.95.0 multi-swing depth: the direction of the most recent pivot step
    ('H'/'L'/'='/'') AND the number of CONSECUTIVE same-direction steps ending
    at the latest pivot (>=1 once there are two pivots). Reuses _cs_struct_seq
    for each pairwise comparison, so a 3-swing uptrend reads deeper than a
    1-swing one."""
    last_dir = _cs_struct_seq(points, eps)
    if last_dir == "":
        return "", 0
    run = 1
    i = len(points) - 1
    while i >= 2:
        if _cs_struct_seq(points[:i], eps) == last_dir:
            run += 1
            i -= 1
        else:
            break
...
def _cs_swing_structure(bars: List[Dict[str, float]]) -> Dict[str, Any]:
    """UPTREND iff higher-high AND higher-low; DOWNTREND iff lower-high AND
    lower-low; everything else (mixed / insufficient pivots) is RANGE.
    swing_run carries the multi-swing depth (how many consecutive swings agree
    with the trend) so the bias can distinguish a freshly-formed trend from an
    established one."""
    highs, lows = _cs_struct_pivots(bars, _CS_STRUCT_PIVOT_K)
    h_dir, h_run = _cs_struct_run(highs, _CS_STRUCT_SWING_EPS)
    l_dir, l_run = _cs_struct_run(lows, _CS_STRUCT_SWING_EPS)
    if h_dir == "H" and l_dir == "H":
        structure = "UPTREND"
    elif h_dir == "L" and l_dir == "L":
        structure = "DOWNTREND"
    else:
        structure = "RANGE"
    high_label = {"H": "HH", "L": "LH", "=": "EH"}
    low_label = {"H": "HL", "L": "LL", "=": "EL"}
    swings: List[str] = []
    if h_dir in high_label:
        swings.append(high_label[h_dir])
    if l_dir in low_label:
        swings.append(low_label[l_dir])
    if h_dir and l_dir:
        swing_run = min(h_run, l_run)
    else:
        swing_run = max(h_run, l_run)
    return {
        "structure": structure,
        "swings": ",".join(swings),
        "swing_run": swing_run,
        "n_pivot_highs": len(highs),
        "n_pivot_lows": len(lows),
    }


def _cs_volume_confirmation(bars: List[Dict[str, float]]) -> Dict[str, Any]:
    """Is the latest bar's move backed by volume? CONFIRMED_UP / CONFIRMED_DOWN
    when latest volume >= confirm-ratio x average and the bar is up / down;
    WEAK when latest volume <= weak-ratio x average; else NEUTRAL."""
    if len(bars) < 3:
        return {"volume_signal": "NEUTRAL", "vol_ratio": 0.0}
    window = bars[-_CS_STRUCT_VOL_WINDOW:] if len(bars) >= _CS_STRUCT_VOL_WINDOW else bars
    vols = [b["volume"] for b in window if b.get("volume", 0.0) > 0]
    if len(vols) < 2:
        return {"volume_signal": "NO_DATA", "vol_ratio": 0.0}
    avg = sum(vols) / len(vols)
    last = bars[-1]
    last_vol = last.get("volume", 0.0)
    if avg <= 0 or last_vol <= 0:
        return {"volume_signal": "NO_DATA", "vol_ratio": 0.0}
    ratio = last_vol / avg
    if ratio >= _CS_STRUCT_VOL_CONFIRM_RATIO and last["close"] > last["open"]:
        sig = "CONFIRMED_UP"
    elif ratio >= _CS_STRUCT_VOL_CONFIRM_RATIO and last["close"] < last["open"]:
        sig = "CONFIRMED_DOWN"
    elif ratio <= _CS_STRUCT_VOL_WEAK_RATIO:
        sig = "WEAK"
    else:
        sig = "NEUTRAL"
    return {"volume_signal": sig, "vol_ratio": round(ratio, 3)}


def _cs_gap_detection(bars: List[Dict[str, float]]) -> Dict[str, Any]:
    """Most-recent open-vs-prior-close gap in the window: UP / DOWN / NONE,
    its signed percent, and the count of gaps in the window."""
    if len(bars) < 2:
        return {"gap": "NONE", "gap_pct": 0.0, "gap_count": 0}
    window = bars[-_CS_STRUCT_GAP_WINDOW:] if len(bars) >= _CS_STRUCT_GAP_WINDOW else bars
    gap_count = 0
    recent_gap = "NONE"
    recent_pct = 0.0
    for i in range(1, len(window)):
        ...
        if prev_close <= 0:
            continue
        chg = (curr_open - prev_close) / prev_close
        if chg >= _CS_STRUCT_GAP_PCT:
            gap_count += 1
            recent_gap, recent_pct = "UP", chg
        elif chg <= -_CS_STRUCT_GAP_PCT:
            gap_count += 1
            recent_gap, recent_pct = "DOWN", chg
    return {
        "gap": recent_gap,
        "gap_pct": round(recent_pct * 100.0, 2),
        "gap_count": gap_count,
    }


_CS_STRUCT_BW_WINDOW = 5            # recent bars for the body/wick conviction read
_CS_STRUCT_BW_DECISIVE_BODY = 0.60  # body_ratio >= 0.60 -> a decisive (long-body) bar
_CS_STRUCT_BW_INDECISIVE_BODY = 0.25  # body_ratio <= 0.25 -> indecision / doji-ish
_CS_STRUCT_BW_WICK_REJECT = 1.5     # shadow >= 1.5x body -> a rejection wick


def _cs_body_wick_read(bars: List[Dict[str, float]]) -> Dict[str, Any]:
    """v5.95.0 body/wick GEOMETRY as a structural CONVICTION read over the recent
    window (Strategy element #2; distinct from named-pattern detection, which
    labels single bars). DECISIVE_UP / DECISIVE_DOWN when recent bodies are long
    and directionally aligned (conviction); INDECISIVE when bodies are small
    (doji-ish / stalling); UPPER_REJECTION / LOWER_REJECTION when long opposing
    wicks dominate (sellers/buyers rejecting an extreme). Reuses _cs_bar_geom."""
    if len(bars) < 2:
        return {"bodywick": "NEUTRAL", "decisive_ratio": 0.0}
    window = bars[-_CS_STRUCT_BW_WINDOW:] if len(bars) >= _CS_STRUCT_BW_WINDOW else bars
    geoms = [_cs_bar_geom(b) for b in window]
    n = len(geoms)
    decisive_up = sum(
        1 for g in geoms if g["body_ratio"] >= _CS_STRUCT_BW_DECISIVE_BODY and g["is_bullish"]
    )
    decisive_down = sum(
        1 for g in geoms if g["body_ratio"] >= _CS_STRUCT_BW_DECISIVE_BODY and g["is_bearish"]
    )
    indecisive = sum(1 for g in geoms if g["body_ratio"] <= _CS_STRUCT_BW_INDECISIVE_BODY)
    upper_rej = sum(
        1 for g in geoms if g["body"] > 0 and g["upper_shadow"] >= _CS_STRUCT_BW_WICK_REJECT * g["body"]
    )
    lower_rej = sum(
        1 for g in geoms if g["body"] > 0 and g["lower_shadow"] >= _CS_STRUCT_BW_WICK_REJECT * g["body"]
    )
    half = max(2, (n + 1) // 2)
    # v5.96.1 (cross-check fix): a DECISIVE majority must win over INDECISIVE.
    # The prior order checked INDECISIVE first, so a window with a decisive
    # majority plus a couple of doji bars (e.g. 3 long-body up + 2 doji of 5)
    # was mislabelled INDECISIVE and -- via the fusion -- wrongly drained
    # conviction from a real trend. Decisive is now evaluated first.
    if decisive_up >= half and decisive_up > decisive_down:
        sig = "DECISIVE_UP"
    elif decisive_down >= half and decisive_down > decisive_up:
        sig = "DECISIVE_DOWN"
    elif indecisive >= max(2, n // 2):
        sig = "INDECISIVE"
    elif upper_rej >= 2 and upper_rej > lower_rej:
        sig = "UPPER_REJECTION"
    elif lower_rej >= 2 and lower_rej > upper_rej:
        sig = "LOWER_REJECTION"
    else:
        sig = "NEUTRAL"
    dr = round((decisive_up + decisive_down) / n, 3) if n else 0.0
    return {"bodywick": sig, "decisive_ratio": dr}


def analyze_candle_structure(rows: Any) -> Dict[str, Any]:
    """v5.94.0 structural backbone aggregator. Returns INTERNAL
    candle_structure_* fields (NOT in the 115 schema; stripped at projection).
    PURE + FAIL-OPEN: any error returns the INSUFFICIENT empty dict."""
    empty = {
        "candle_structure": "INSUFFICIENT",
        "candle_structure_trend": "",
        "candle_structure_swings": "",
        "candle_structure_volume": "",
        "candle_structure_bodywick": "",
        "candle_structure_gap": "",
        "candle_structure_summary": "",
    }
    try:
        bars = _cs_coerce_bars(rows)
        if len(bars) < _CS_STRUCT_MIN_BARS:
            return dict(empty)
        swing = _cs_swing_structure(bars)
        vol = _cs_volume_confirmation(bars)
        bw = _cs_body_wick_read(bars)
        gap = _cs_gap_detection(bars)
        structure = swing["structure"]
        vsig = vol["volume_signal"]
        bwsig = bw["bodywick"]
        gsig = gap["gap"]
        swing_run = int(swing.get("swing_run", 1) or 1)
        # ----------------------------------------------------------------
        # Four-element structural fusion (Strategy #1). FULLY AUDITABLE and
        # rule-based -- NOT a black box. The trend (swing structure) is the
        # primary axis; volume confirmation, body/wick conviction and gaps
        # each add +1 when they AGREE with the trend direction and -1 when
        # they CONTRADICT it, producing a bounded conviction score the label
        # is read from. STRONG when confirmations stack; WEAK when a real
        # trend is contradicted by its own bars/volume/gap (the early-warning
        # / thesis-decay signal). Every element is ALSO surfaced as its own
        # candle_structure_* field, so the composite can always be decomposed.
        # ----------------------------------------------------------------
        if structure == "RANGE":
            bias = "NEUTRAL"
        else:
            up = (structure == "UPTREND")
            score = 1 if swing_run >= 2 else 0  # established (multi-swing) trend
            if (up and vsig == "CONFIRMED_UP") or ((not up) and vsig == "CONFIRMED_DOWN"):
                score += 1
            elif vsig == "WEAK":
                score -= 1
            if (up and bwsig == "DECISIVE_UP") or ((not up) and bwsig == "DECISIVE_DOWN"):
                score += 1
            elif (
                bwsig == "INDECISIVE"
                or (up and bwsig == "UPPER_REJECTION")
                or ((not up) and bwsig == "LOWER_REJECTION")
            ):
                score -= 1
            if (up and gsig == "UP") or ((not up) and gsig == "DOWN"):
                score += 1
            elif (up and gsig == "DOWN") or ((not up) and gsig == "UP"):
                score -= 1
            if score >= 2:
                bias = "STRONG_BULLISH" if up else "STRONG_BEARISH"
            elif score <= -1:
                bias = "WEAK_BULLISH" if up else "WEAK_BEARISH"
            else:
                bias = "BULLISH" if up else "BEARISH"
        gap_txt = ""
        if gsig != "NONE":
            gap_txt = " | gap %s %.1f%%" % (gsig, gap["gap_pct"])
        summary = "%s (%s x%d) vol=%s body=%s%s" % (
            structure, swing["swings"] or "n/a", swing_run, vsig, bwsig, gap_txt
        )
        return {
            "candle_structure": bias,
            "candle_structure_trend": structure,
            "candle_structure_swings": swing["swings"],
            "candle_structure_volume": vsig,
            "candle_structure_bodywick": bwsig,
            "candle_structure_gap": gsig if gsig != "NONE" else "",
            "candle_structure_summary": summary,
        }
    except Exception:
        return dict(empty)


def _candle_structure_enabled() -> bool:
    """v5.94.0: master switch for the structural candlestick backbone
    (default OFF). When OFF, _compute_history_patch_from_rows merges no
    candle_structure_* fields, so the history patch -- and therefore every
    downstream score, verdict, recommendation and the 115-key projection --
    is byte-identical to v5.93.0. Set TFB_CANDLE_STRUCTURE to 1/true/on."""
    raw = (os.getenv("TFB_CANDLE_STRUCTURE") or "").strip().lower()
    return raw in {"1", "true", "yes", "y", "on", "t", "enabled", "enable"}


def _candle_structure_stsignal_enabled() -> bool:
    """v5.96.0 (Phase 2a): surface the structural backbone on the in-schema
    st_signal presentation field (default OFF). When OFF, st_signal is derived
    exactly as in v5.95.0 (RSI extremes -> candlestick_signal -> 1M momentum).
    When ON, a CLEAR structural bias (the *_BULLISH / *_BEARISH families) drives
    st_signal as the primary short-term signal -- structure replacing the
    RSI/candlestick scalar shortcut for the symbols that carry a structural read
    -- while RANGE / INSUFFICIENT / no-structure rows still fall through to the
    legacy chain UNCHANGED. Requires TFB_CANDLE_STRUCTURE to also be on (that is
    what populates candle_structure); if the field is absent this is a safe
    no-op. DECISION-NEUTRAL: st_signal is derived AFTER the scoring / gate /
    ranking chain, so this changes no score, verdict, recommendation or Top_10
    rank; vocabulary-preserving (BULLISH/BEARISH/NEUTRAL); adds NO column
    (st_signal is one of the 115). Set TFB_CANDLE_STRUCTURE_STSIGNAL to 1/true/on."""
    raw = (os.getenv("TFB_CANDLE_STRUCTURE_STSIGNAL") or "").strip().lower()
    return raw in {"1", "true", "yes", "y", "on", "t", "enabled", "enable"}


def _candle_structure_momentum_enabled() -> bool:
    """v5.97.0 (Phase 3): let the structural candlestick read DRIVE the
    momentum component of the score for DECISION symbols (default OFF). This is
    the only candle_structure phase that MOVES VERDICTS. When OFF, the engine
    stamps no _decision_symbol flag, so scoring.py's compute_momentum_score is
    byte-identical to v5.7.4 (RSI/price-based) and every score, recommendation
    and rank is byte-identical to v5.96.1. When ON, _get_enriched_quote_impl
    tags decision-symbol rows so scoring.py blends the structural bias into
    momentum (the Strategy's 'replace the momentum_score/RSI shortcut' for
    decision symbols), which then flows through the EXISTING single authoritative
    scorer -> overall_score -> recommendation (no duplicate derivation path).
    Requires TFB_CANDLE_STRUCTURE on (that populates candle_structure); a safe
    no-op when the field is absent. Validate with a Top_10 dry-run before
    enabling. Set TFB_CANDLE_STRUCTURE_MOMENTUM to 1/true/on."""
    raw = (os.getenv("TFB_CANDLE_STRUCTURE_MOMENTUM") or "").strip().lower()
    return raw in {"1", "true", "yes", "y", "on", "t", "enabled", "enable"}


# v5.97.0 (Phase 3): the decision-symbol universe the Strategy scopes the
# verdict-moving structural override to -- Top_10 + holdings + Market_Leaders.
# Membership is page-context based; canonicalized so name variants all match.
_DECISION_PAGE_CANON: frozenset = frozenset({
    "top_10_investments", "my_portfolio", "market_leaders",
})


def _is_decision_page(page: Any) -> bool:
    """v5.97.0 (Phase 3): True when the resolved page context is one the Strategy
    treats as a decision page (Top_10 / holdings / Market_Leaders). Only these
    rows receive the _decision_symbol tag, so the structural momentum override
    can never reach a non-decision market-page row."""
    try:
        canon = _canonicalize_sheet_name(_safe_str(page)).strip().lower()
    except Exception:
        return False
    return canon in _DECISION_PAGE_CANON


# =============================================================================
# FLOW / BUYERS-vs-SELLERS READ (Strategy item (a); env TFB_FLOW default OFF)
# -----------------------------------------------------------------------------
# A CURRENT-STATE supply/demand inference -- NOT order-flow prediction -- on the
# SAME already-fetched OHLCV history bars (ZERO new I/O), mirroring the Phase-1
# candle_structure backbone. Returns INTERNAL flow_* fields the 115-key
# projection STRIPS (adds NO column); pure + fail-open -> any error / no usable
# volume returns the INSUFFICIENT empty dict so scoring.compute_flow_score falls
# back and the overall score is byte-identical to "no flow". Three transparent,
# rule-based reads are fused:
#   * flow_pressure  : Chaikin Money Flow (CMF) over the window -- volume-weighted
#                      close-location-in-range, bounded ~[-1,+1]; >0 = buying
#                      pressure (accumulation), <0 = selling pressure (distribution)
#   * flow_ad_trend  : is money flow STRENGTHENING? 2nd-half CMF vs 1st-half CMF
#                      over the window -> RISING / FALLING / FLAT
#   * flow_inflection: where the last close sits in the window high-low range
#                      -> NEAR_SUPPORT / MID_RANGE / NEAR_RESISTANCE
# Fusion is the SAME shape as analyze_candle_structure: CMF is the primary axis;
# ad-trend and inflection each add +1 when they CONFIRM the CMF direction and -1
# when they CONTRADICT it, yielding STRONG_/plain/WEAK_ ACCUMULATION/DISTRIBUTION
# (or NEUTRAL). Every component is ALSO surfaced as its own flow_* field so the
# composite always decomposes. This is the "buyers vs sellers" factor the
# Strategy calls for -- a current-state inference, fully auditable, never a claim
# about the future.
# =============================================================================
_FLOW_WINDOW = 20                  # bars used for the money-flow read
_FLOW_MIN_BARS = 10                # min bars before flow is meaningful
_FLOW_CMF_STRONG = 0.15            # |CMF| at/above this = strong pressure
_FLOW_CMF_MILD = 0.05              # |CMF| at/above this = mild pressure (else NEUTRAL)
_FLOW_AD_TREND_EPS = 0.05          # half-window CMF delta to call RISING/FALLING
_FLOW_INFLECTION_LOW = 0.30        # close-position <= this = NEAR_SUPPORT
_FLOW_INFLECTION_HIGH = 0.70       # close-position >= this = NEAR_RESISTANCE


def _flow_enabled() -> bool:
    """v5.98.0: master switch for the flow / buyers-vs-sellers read (default
    OFF). When OFF, _compute_history_patch_from_rows merges no flow_* fields, so
    the history patch -- and every downstream score, verdict, recommendation and
    the 115-key projection -- is byte-identical to v5.97.0. Set TFB_FLOW to
    1/true/on. (This only populates the read; the scoring blend is the separate
    TFB_FLOW_SCORE gate, and the Insights panel is TFB_INSIGHTS_FLOW.)"""
    raw = (os.getenv("TFB_FLOW") or "").strip().lower()
    return raw in {"1", "true", "yes", "y", "on", "t", "enabled", "enable"}


def _flow_score_enabled() -> bool:
    """v5.98.0: whether the flow read is promoted to a weighted scoring factor
    (mirrors scoring.py's TFB_FLOW_SCORE). Read HERE only to decide whether to
    stamp the transient _decision_symbol tag, so flow scoring can reach decision
    rows even when the Phase-3 candle-momentum flag is off. Default OFF."""
    raw = (os.getenv("TFB_FLOW_SCORE") or "").strip().lower()
    return raw in {"1", "true", "yes", "y", "on", "t", "enabled", "enable"}


def _flow_cmf_window(mfm_vol: Any) -> Optional[float]:
    """Chaikin Money Flow over the supplied (mfm, volume) pairs:
    sum(mfm*vol)/sum(vol). Returns None when total volume is non-positive (no
    usable volume data) so the caller can fall back honestly."""
    num = 0.0
    den = 0.0
    for mfm, vol in mfm_vol:
        if vol and vol > 0:
            num += mfm * vol
            den += vol
    if den <= 0:
...
def analyze_flow(rows: Any) -> Dict[str, Any]:
    """v5.98.0 flow aggregator. Returns INTERNAL flow_* fields (NOT in the 115
    schema; stripped at projection). PURE + FAIL-OPEN: any error or absent
    volume returns the INSUFFICIENT empty dict. Reuses _cs_coerce_bars /
    _cs_bar_geom (the SAME bars the candle backbone uses) -- zero new I/O."""
    empty = {
        "flow": "INSUFFICIENT",
        "flow_pressure": "",
        "flow_ad_trend": "",
    ...
    try:
        bars = _cs_coerce_bars(rows)
        if len(bars) < _FLOW_MIN_BARS:
            return dict(empty)
        window = bars[-_FLOW_WINDOW:] if len(bars) >= _FLOW_WINDOW else bars
        # Money Flow Multiplier per bar: ((close-low)-(high-close))/range -- the
        # volume-weighted close location within the bar's range, clamped [-1,+1].
        mfm_vol: List[Tuple[float, float]] = []
        for b in window:
            g = _cs_bar_geom(b)
            rng = g["range"]
            mfm = ((g["close"] - g["low"]) - (g["high"] - g["close"])) / rng
            if mfm > 1.0:
                mfm = 1.0
            elif mfm < -1.0:
                mfm = -1.0
            mfm_vol.append((mfm, b.get("volume", 0.0)))
        cmf = _flow_cmf_window(mfm_vol)
        if cmf is None:
            return dict(empty)   # no usable volume -> no flow read (honest)
        # Money-flow TREND: compare 2nd-half CMF vs 1st-half CMF over the window.
        half = max(1, len(mfm_vol) // 2)
        cmf_first = _flow_cmf_window(mfm_vol[:half])
        cmf_second = _flow_cmf_window(mfm_vol[half:])
        if cmf_first is None or cmf_second is None:
            ad_trend = "FLAT"
        else:
            d = cmf_second - cmf_first
            if d >= _FLOW_AD_TREND_EPS:
                ad_trend = "RISING"
            elif d <= -_FLOW_AD_TREND_EPS:
                ad_trend = "FALLING"
            else:
                ad_trend = "FLAT"
        # Inflection: last close position within the window high-low range.
        hi = max(b["high"] for b in window)
        lo = min(b["low"] for b in window)
        span = hi - lo
        if span <= 0:
            inflection = "MID_RANGE"
        else:
            pos = (window[-1]["close"] - lo) / span
            if pos <= _FLOW_INFLECTION_LOW:
                inflection = "NEAR_SUPPORT"
            elif pos >= _FLOW_INFLECTION_HIGH:
                inflection = "NEAR_RESISTANCE"
            else:
                inflection = "MID_RANGE"
        # ----------------------------------------------------------------
        # Fusion (SAME shape as analyze_candle_structure). CMF is the primary
        # axis; ad-trend and inflection each CONFIRM (+1) or CONTRADICT (-1) the
        # CMF direction. FULLY AUDITABLE, rule-based -- NOT a black box.
        # ----------------------------------------------------------------
        if abs(cmf) < _FLOW_CMF_MILD:
            bias = "NEUTRAL"
        else:
            acc = cmf > 0  # accumulation (net buying) direction
            score = 1 if abs(cmf) >= _FLOW_CMF_STRONG else 0
            if (acc and ad_trend == "RISING") or ((not acc) and ad_trend == "FALLING"):
                score += 1
            elif (acc and ad_trend == "FALLING") or ((not acc) and ad_trend == "RISING"):
                score -= 1
            if (acc and inflection == "NEAR_SUPPORT") or ((not acc) and inflection == "NEAR_RESISTANCE"):
                score += 1
            elif (acc and inflection == "NEAR_RESISTANCE") or ((not acc) and inflection == "NEAR_SUPPORT"):
                score -= 1
            if score >= 2:
                bias = "STRONG_ACCUMULATION" if acc else "STRONG_DISTRIBUTION"
            elif score <= -1:
                bias = "WEAK_ACCUMULATION" if acc else "WEAK_DISTRIBUTION"
            else:
                bias = "ACCUMULATION" if acc else "DISTRIBUTION"
        summary = "%s cmf=%+.3f trend=%s pos=%s" % (bias, cmf, ad_trend, inflection)
        return {
            "flow": bias,
            "flow_pressure": round(cmf, 3),
            "flow_ad_trend": ad_trend,
            "flow_inflection": inflection,
...
def _position_math_authoritative_enabled() -> bool:
    ...
    unrealized_pl_pct) on every run instead of fill-only, so a stale prior-run
    value cannot persist against a newer current_price. When OFF, the writes in
    _compute_position_math collapse back to the v5.91.0 "... is None" fill-only
    guard, so My_Portfolio rows are byte-identical to v5.91.0. Set
    TFB_POSITION_MATH_AUTHORITATIVE to 0/false/off to disable."""
    raw = (os.getenv("TFB_POSITION_MATH_AUTHORITATIVE") or "").strip().lower()
    if raw in {"0", "false", "no", "n", "off", "f", "disabled", "disable"}:
        return False
    return True
...
def _compute_position_math(row: Dict[str, Any]) -> None:
    ...
    # v5.92.0 (Fix AH): recompute the price-dependent derived fields every run
    # (overriding a stale prior-run value), not fill-only. Each line stays
    # guarded on its own inputs; when the gate is OFF, "_auth or ... is None"
    # collapses to the v5.91.0 "... is None" fill-only guard byte-for-byte.
    _auth = _position_math_authoritative_enabled()
    if qty is not None and price is not None and (_auth or _as_float(row.get("position_value")) is None):
        row["position_value"] = round(qty * price, 6)
    if qty is not None and avg_cost is not None and (_auth or _as_float(row.get("position_cost")) is None):
        row["position_cost"] = round(qty * avg_cost, 6)
    pos_val = _as_float(row.get("position_value"))
    pos_cost = _as_float(row.get("position_cost"))
    if pos_val is not None and pos_cost is not None and (_auth or _as_float(row.get("unrealized_pl")) is None):
        row["unrealized_pl"] = round(pos_val - pos_cost, 6)
    upl = _as_float(row.get("unrealized_pl"))
    if upl is not None and pos_cost not in (None, 0) and (_auth or _as_float(row.get("unrealized_pl_pct")) is None):
...
def _apply_analyst_trend_block(row: Dict[str, Any]) -> None:
    ...
    # 8) ST Signal. v5.96.0 (Phase 2a): the structural backbone is the PRIMARY
    #    short-term signal (Strategy: structure replaces the RSI/candlestick
    #    scalar shortcut). A CLEAR structural bias drives st_signal; RANGE /
    #    INSUFFICIENT / gate-off / no-structure rows fall through to the legacy
    #    RSI-extreme -> candlestick -> 1M-momentum chain UNCHANGED. Gated +
    #    fail-safe; vocabulary-preserving (BULLISH/BEARISH/NEUTRAL); decision-
    #    neutral (st_signal is presentation, derived AFTER scoring); NO column.
    st = ""
    if _candle_structure_stsignal_enabled():
        _cs_bias = _safe_str(row.get("candle_structure")).upper()
        if _cs_bias in ("STRONG_BULLISH", "BULLISH", "WEAK_BULLISH"):
            st = "BULLISH"
        elif _cs_bias in ("STRONG_BEARISH", "BEARISH", "WEAK_BEARISH"):
            st = "BEARISH"
    if not st:
        rsi = _as_float(row.get("rsi_14"))
        if rsi is not None:
...
def _project_rows_with_trust_carry(keys: Sequence[str], rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """v5.90.0: project each row onto the canonical keys via _strict_project_row,
    but CARRY the gate's INTERNAL trust_level onto the projected row so the
    subsequent _apply_rank_overall can still withhold a LOW-trust row from
    Rank (Overall).

    Root cause this fixes: _strict_project_row returns {k: ... for k in keys}
    and trust_level is NOT one of the canonical keys (schema 115), so it was
    stripped on the line BEFORE _apply_rank_overall read it -- silently killing
    the v5.88.0 LOW-trust rank skip on every market page AND the Top_10 path.
    The live [v5.89.0 TRUST] band log read low=0/med=0/high=0 for exactly this
    reason: by the time the ranker counted bands, every projected row had
    already lost trust_level. The carried field is TRANSIENT; callers MUST pop
    it after ranking (see _pop_trust_carry) so the row still matches the
    canonical 115-key contract -- trust_level stays INTERNAL, schema unchanged.

    Gated implicitly by the same TFB_TRUST_LEVEL master switch as the skip: when
    trust is OFF the gate writes no trust_level, so "trust_level" in r is False,
    nothing is carried, and the projected rows (and ranks) are byte-identical to
    v5.89.0. When trust is ON, a LOW row is now actually dropped from rank -- the
    intended v5.88.0 behavior that this projection-order bug had suppressed.
    """
    out: List[Dict[str, Any]] = []
    for r in rows:
        pr = _strict_project_row(keys, r)
        if isinstance(pr, dict) and isinstance(r, dict) and "trust_level" in r:
            pr["trust_level"] = r.get("trust_level")
        out.append(pr)
    return out


def _pop_trust_carry(rows: List[Dict[str, Any]]) -> None:
    """v5.90.0: remove the transient trust_level carried by
    _project_rows_with_trust_carry once ranking has consumed it, so the emitted
    rows match the canonical 115-key contract (trust_level is INTERNAL only --
    adds NO column). Idempotent / fail-open: a row without the key is left
    untouched."""
    for r in rows:
        if isinstance(r, dict):
...
def _apply_rank_overall(rows: List[Dict[str, Any]]) -> None:
    ...
    """
    _trust_on = _trust_level_enabled()
    dropped_low_trust = 0
    ...
    for i, row in enumerate(rows):
        ...
        # fires for rows the gate marked trust_level=LOW; a row without a
        # trust_level (gate disabled or not yet run) ranks normally (fail-open).
        if _trust_on and isinstance(row, dict) and \
                _safe_str(row.get("trust_level")).upper() == TRUST_LEVEL_LOW:
            _v573_append_warning(row, "rank_skipped_low_trust")
            dropped_low_trust += 1
            continue
        score = _as_float(row.get("overall_score"))
        if score is None:
    ...
    if logger.isEnabledFor(logging.INFO):
        try:
            ...
            skipped_no_score = total - scored_count - dropped_low_trust
            logger.info(
                "[v%s RANK] total=%d scored=%d skipped_no_score=%d dropped_low_trust=%d",
                __version__, total, scored_count, skipped_no_score,
                dropped_low_trust,
            )
            # v5.88.0 TRUST observability -- band counts across the batch,
            # emitted once per call alongside RANK and gated on the trust flag.
            if _trust_on:
                _tl = [_safe_str(r.get("trust_level")).upper()
                       for r in rows if isinstance(r, dict)]
                logger.info(
                    "[v%s TRUST] total=%d low=%d med=%d high=%d dropped_from_rank=%d",
                    __version__, total,
                    _tl.count(TRUST_LEVEL_LOW), _tl.count(TRUST_LEVEL_MEDIUM),
                    _tl.count(TRUST_LEVEL_HIGH), dropped_low_trust,
                )
        except Exception:
            pass
...
class DataEngineV5:
    ...
    def _compute_history_patch_from_rows(self, rows: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for k, v in patches.items():
            patch[k] = v
        # v5.94.0 PHASE-1: structural candlestick backbone on the SAME already-
        # fetched bars (zero new I/O). Merges INTERNAL candle_structure_* fields
        # the 115-key projection strips -> NO schema column, NO score/verdict/
        # recommendation change. Env-gated default-OFF + fail-open, so when OFF
        # the patch is byte-identical to v5.93.0. Named patterns above are kept
        # as labeled context on top of this backbone.
        if _candle_structure_enabled():
            try:
                _cs_struct = analyze_candle_structure(rows)
                ...
                logger.debug(
                    ...
                    _cs_struct.get("candle_structure_volume"),
                    _cs_struct.get("candle_structure_bodywick"),
                    _cs_struct.get("candle_structure_gap"),
                )
            except Exception:
                pass
        # v5.98.0 FLOW: buyers-vs-sellers / supply-demand read on the SAME
        # already-fetched bars (zero new I/O). Merges INTERNAL flow_* fields the
        # 115-key projection strips -> NO schema column, NO score/verdict change
        # by itself (the scoring blend is the separate TFB_FLOW_SCORE gate).
        # Env-gated default-OFF + fail-open: when OFF the patch is byte-identical
        # to v5.97.0.
        if _flow_enabled():
            try:
                _flow_read = analyze_flow(rows)
                ...
                logger.debug(
                    ...
                    _flow_read.get("flow_inflection"),
                )
            except Exception:
                pass
        return patch
    ...
    async def _get_enriched_quote_impl(self, symbol: str, page: str = "") -> Dict[str, Any]:
        ...
        async def factory() -> Dict[str, Any]:
            ...
            if not _is_empty_data_row(merged):
                ...
                # page-scoped -> when OFF (or non-decision page) no tag is set and
                # scoring is byte-identical to v5.96.1. The tag is a transient,
                # non-canonical key -> stripped by _strict_project_row (no column).
                # v5.98.0: ALSO stamp when the flow scoring factor is enabled, so
                # the flow blend can reach decision rows independently of the
                # Phase-3 candle-momentum flag (either gate -> same single tag).
                if (_candle_structure_momentum_enabled() or _flow_score_enabled()) and _is_decision_page(page_ctx):
    ...
    async def get_sheet_rows(
        ...
        **kwargs: Any,
        ...
        if target_sheet == "Top_10_Investments":
            ...
            # v5.90.0 (trust-carry): keep trust_level alive across projection so
            # _apply_rank_overall can drop a LOW-trust row, then pop the transient.
            rows = _project_rows_with_trust_carry(keys, rows)
        ...
        # v5.90.0 (trust-carry): project WITH a transient trust_level so the
        # v5.88.0 LOW-trust rank skip actually fires. _strict_project_row maps
        # onto the 115 canonical keys (trust_level is not one), which stripped
        # the field one line before _apply_rank_overall read it -- the live
        # [TRUST] band log read low=0/med=0/high=0 for exactly this reason. The
        # carry is popped after ranking so the row matches the 115-key contract.
        rows = _project_rows_with_trust_carry(keys, rows)
    ...
    def health(self) -> Dict[str, Any]:
        return {
            ...
            "valuation_model": {
                "version": "v5.98.0",  # v5.98.0 FLOW / BUYERS-vs-SELLERS FACTOR (Strategy item (a); three env gates, ALL default OFF -> with TFB_FLOW unset the history patch, every score, verdict, recommendation, rank and the 115-key projection are byte-identical to v5.97.0): analyze_flow() computes a CURRENT-STATE supply/demand inference (NOT order-flow prediction) on the SAME already-fetched OHLCV bars (ZERO new I/O) -- Chaikin Money Flow (flow_pressure, volume-weighted close-location-in-range, bounded ~[-1,+1]), money-flow trend (flow_ad_trend RISING/FALLING/FLAT from 2nd-half vs 1st-half CMF) and inflection (flow_inflection NEAR_SUPPORT/MID_RANGE/NEAR_RESISTANCE) -- fused the SAME auditable rule-based way as analyze_candle_structure (CMF primary; ad-trend + inflection each +/-1 on confirm/contradict) into STRONG_/plain/WEAK_ ACCUMULATION/DISTRIBUTION or NEUTRAL. Merged as INTERNAL flow_* fields the 115 projection STRIPS (NO column); pure + fail-open (absent volume / error -> INSUFFICIENT). Three independent gates mirroring the candle phases: TFB_FLOW (populate the read), TFB_FLOW_SCORE (scoring.py v5.9.0 promotes flow to a FIRST-CLASS weighted factor w_flow -- default 0.10, renormalizing the other weights -- blended into overall_score for decision symbols only; the engine extends the _decision_symbol stamp to fire on TFB_FLOW_SCORE too), and TFB_INSIGHTS_FLOW (insights_builder v8.6.0 surfaces a Flow panel). The buyers-vs-sellers factor the Strategy calls for: honestly weighted, fully auditable, never a future claim. Validate with a Top_10 dry-run before enabling. Reversible: TFB_FLOW unset -> v5.97.0 exactly. v5.97.0 STRUCTURAL MOMENTUM -- VERDICT-MOVING (Strategy #1, Phase 3; env TFB_CANDLE_STRUCTURE_MOMENTUM default OFF -> momentum_score/overall_score/recommendation/rank byte-identical to v5.96.1): _get_enriched_quote_impl stamps a transient _decision_symbol flag on rows whose resolved page is a decision page (_is_decision_page: Top_10_Investments / My_Portfolio / Market_Leaders) BEFORE scoring; scoring.py v5.8.0's compute_momentum_score then BLENDS the structural candle read (candle_structure bias -> 0-100 via the structure->momentum map, weight TFB_CANDLE_STRUCTURE_MOMENTUM_WEIGHT default 0.6) into the RSI/price momentum for those rows only -- the Strategy's 'replace the momentum_score/RSI shortcut for decision symbols'. Flows through the EXISTING single authoritative scorer (compute_scores) -> overall_score -> recommendation; NO duplicate derivation path, NO new column (the flag is non-canonical -> stripped by _strict_project_row), pure + fail-open (absent candle_structure or non-decision page -> exact v5.7.4 RSI momentum). The ONLY candle phase that moves verdicts; validate with a Top_10 dry-run before enabling. Reversible: TFB_CANDLE_STRUCTURE_MOMENTUM unset -> v5.96.1 exactly. v5.96.1 CROSS-CHECK FIX: _cs_body_wick_read now evaluates the DECISIVE majority BEFORE INDECISIVE (a decisive majority + a couple of doji bars was mislabelled INDECISIVE, wrongly draining fusion conviction from a real trend; structural STRENGTH only -- st_signal direction unaffected). All v5.96.0 logic carried; still env-gated/no-column/decision-neutral. v5.96.0 STRUCTURAL READ SURFACED ON st_signal (Strategy #1 Phase 2a; env TFB_CANDLE_STRUCTURE_STSIGNAL default OFF -> st_signal byte-identical to v5.95.0): _apply_analyst_trend_block consults the structural bias FIRST when on -- a clear *_BULLISH/*_BEARISH candle_structure read drives st_signal (structure replacing the RSI/candlestick scalar shortcut), RANGE/INSUFFICIENT/no-structure fall through to the legacy RSI-extreme->candlestick->1M-momentum chain. Runs at the same final boundary before the 115-projection so candle_structure (internal) is readable and st_signal (in-schema) survives; DECISION-NEUTRAL (st_signal is presentation, derived after scoring -> no score/verdict/reco/rank change), vocabulary-preserving, NO column; two-key dependency + fail-safe no-op when the field is absent. Reversible: TFB_CANDLE_STRUCTURE_STSIGNAL unset restores v5.95.0 st_signal exactly. v5.95.0 STRUCTURAL BACKBONE RESHAPED TO FULL STRATEGY #1 SHAPE (audit fix; env TFB_CANDLE_STRUCTURE default OFF -> history patch/115-schema/verdict byte-identical to v5.93.0): analyze_candle_structure now covers ALL FOUR Strategy #1 elements -- swing structure DEEPENED with multi-swing run length (_cs_struct_run), body/wick geometry ADDED as a conviction read (_cs_body_wick_read via _cs_bar_geom: DECISIVE_UP/DECISIVE_DOWN/INDECISIVE/UPPER_REJECTION/LOWER_REJECTION), volume confirmation, and gap detection now FUSED into the bias -- combined through a transparent rule-based conviction score (trend primary; volume/body/gap +/-1 on agree/contradict) yielding STRONG_/plain/WEAK_ BULLISH/BEARISH/NEUTRAL, every element also surfaced as its own internal candle_structure_* field (incl. candle_structure_bodywick) so the composite decomposes. Still computed on already-fetched OHLCV bars (zero new I/O), INTERNAL fields stripped at projection (NO column), NO score/verdict change, named patterns preserved as labeled context, pure + fail-open. v5.94.0 STRUCTURAL CANDLESTICK BACKBONE (Strategy #1, Phase 1; env TFB_CANDLE_STRUCTURE default OFF -> history patch / 115-schema / verdict / recommendation byte-identical to v5.93.0): analyze_candle_structure() computes swing structure (pivot HH/HL/LH/LL -> UPTREND/DOWNTREND/RANGE), volume confirmation (latest bar vs ~20-bar avg -> CONFIRMED_UP/CONFIRMED_DOWN/WEAK/NEUTRAL) and open-vs-prior-close gap detection on the SAME already-fetched OHLCV history bars inside _compute_history_patch_from_rows (ZERO new I/O / edge cost), merged as INTERNAL candle_structure_* fields the 115-key projection STRIPS (adds NO column; named patterns from detect_candlestick_patterns preserved as labeled context on top of the backbone). Pure + fail-open: any error returns the INSUFFICIENT empty dict and never breaks the patch. This is the backbone the project Strategy requires (replaces nothing yet -- the momentum_score/RSI replacement + Insights/st_signal surfacing are the next env-gated phases). v5.93.0 GHOST-TICKER / NAME-UNRESOLVED DISCLOSURE TAG (visibility-only; ALL v5.92.0 logic verbatim): _apply_investability_gate appends a distinct name_unresolved warning to a PRICED row whose display name never resolved (blank or == symbol -- the ghost signature, 5023.SR rendered name == "5023.SR"), via pure/fail-safe _row_name_is_unresolved, gated by has_price so a no-data empty row (already empty_row_no_provider_data) is not double-flagged. DISCLOSURE-ONLY -- never demotes; the v5.88.0-v5.90.0 trust system still governs ranking (this just gives the already-barred ghost a NAMED, audit-surfaceable signal that audit_data_quality.py v4.8.0 maps to a GHOST_TICKER issue + Insights Data Quality Alert). Broader than the v5.89.0 demotion trigger on purpose (no degenerate-range requirement): disclosing a missing name is safe even for a real-but-unnamed instrument. Tag-only: NO schema (115 / MP 122), recommendation, verdict, reliability, or scoring change; name_unresolved is reliability-scan substring-safe so a repeat gate pass is byte-identical. Reversible: TFB_NAME_UNRESOLVED_TAG in {0,false,off} -> warnings byte-identical to v5.92.0; default ON. v5.92.0 MY_PORTFOLIO POSITION MATH AUTHORITATIVE (Fix AH): _compute_position_math recomputes the price-dependent derived fields (position_value/unrealized_pl/unrealized_pl_pct) on EVERY run instead of fill-only, so a stale prior-run value can no longer persist against a newer current_price (the holding now reconciles: Value-Cost==P/L and P/L/Cost*100==P/L%); position_cost is price-independent so it is unchanged; each recompute stays guarded on its inputs (never blanks, never raises) and runs My_Portfolio-only (gated on position_qty). Reversible: TFB_POSITION_MATH_AUTHORITATIVE in {0,false,off} restores the v5.91.0 fill-only writes byte-for-byte; default ON. v5.91.0 RELIABILITY CALIBRATION ADJUST (Phase 0 self-correction loop "Adjust" step; env TFB_CALIBRATION_ADJUST default OFF -> forecast_reliability_score byte-identical to v5.90.0): _apply_reliability_calibration multiplies the DISPLAYED reliability by the offline-computed per-investability-bucket factor read from TFB_CALIBRATION_FACTORS (simple CSV, e.g. "INVESTABLE:0.85,WATCHLIST:0.95", produced by scripts/track_performance.py --calibrate; NO live sheet read -> zero edge-path I/O). DECISION-NEUTRAL: runs AFTER the full verdict chain so the strict-gate reliability floor saw the RAW score; only the shown number is calibrated (BUT forecast_reliability_score is published, so the downstream Top_10 builder/opportunity_builder ~70 reliability floor WILL see the calibrated value -- the intended Adjust effect; validate with a Top_10 dry-run before enabling, and re-derive factors with the haircut OFF to avoid double-calibration). DOWNWARD-ONLY by default (TFB_CALIBRATION_DOWNWARD_ONLY caps an applied factor <=1.0 so it only haircuts an over-claim, never inflates); no-op on empty factors or factor~=1.0 (the calibrator shrinks toward 1.0 on thin samples, so nothing auto-tunes on noise). Adds NO column (schema 115); a touched row is tagged reliability_calibrated (substring-safe: no cap/forecast/target/roi/drop/reject, so a second gate pass is identical); idempotent given the same factor. Reversible: TFB_CALIBRATION_ADJUST unset restores v5.90.0 exactly. v5.90.0 TRUST-CARRY THROUGH PROJECTION (Phase 0 fix; gated by the existing TFB_TRUST_LEVEL master -> OFF = byte-identical to v5.89.0): _project_rows_with_trust_carry copies the gate's INTERNAL trust_level onto the projected row so _apply_rank_overall can actually WITHHOLD a LOW-trust row from Rank (Overall), then _pop_trust_carry removes the transient so the row still matches the canonical 115-key contract. Root cause: _strict_project_row maps onto the 115 keys and trust_level is not one of them, so it was stripped one line BEFORE the ranker read it -- the v5.88.0 LOW-trust skip was structurally dead on EVERY market page AND Top_10 (the live [v5.89.0 TRUST] band log read low=0/med=0/high=0 because every projected row had already lost trust_level). Applies to both rank call-sites in get_sheet_rows (market-page branch + Top_10 branch). Adds NO column (schema 115); LOW still only ever demotes. v5.89.0 DEGENERATE-FIELD TRUST DEMOTION (Phase 0; env TFB_TRUST_DEGENERATE_GUARD default ON -> OFF = trust_level/ranks byte-identical to v5.88.0): _row_is_degenerate forces trust_level=LOW for a priced-but-synthetic row on EITHER (1) the scorer's own momentum_only_fallback flag (standalone, asset-class-safe) OR (2) a near-zero-width 52W range (hi~=lo, TFB_TRUST_DEGENERATE_RANGE_PCT=0.01) AND a blank name together (both required, so a named peg / a real-but-unnamed instrument is spared) -- closing the gap where 5023.SR (dq 63.2 -> MEDIUM, nameless, 102.00<->102.30) still held a market-page rank (the v5.82.0 thin-history guard caught only a MISSING range, not a degenerate positive one); trust_level stays INTERNAL (schema 115), LOW only ever demotes. v5.88.0 DATA-TRUST LEVEL (Engineering Audit Phase 0 -- trust source-of-truth; env-gated TFB_TRUST_LEVEL default ON -> with it OFF the projected rows AND ranks are byte-identical to v5.87.0): _apply_investability_gate bands the EXISTING data_quality_score (plus the BLOCKED verdict and the usable-price flag) into an explicit trust_level LOW/MEDIUM/HIGH (thresholds TFB_TRUST_DQ_HIGH=80 / TFB_TRUST_DQ_LOW=55), tags a LOW row with low_data_trust in warnings, and _apply_rank_overall WITHHOLDS a LOW-trust row from Rank (Overall) (rank_skipped_low_trust) so a sparse name (5023.SR) can no longer occupy a market-page rank. trust_level is an INTERNAL field -- adds NO column (schema 115); freshness is intentionally excluded (the engine stamps last_updated_utc at fetch, so staleness is the consumer's concern, enforced by opportunity_builder / portfolio_actions); OB's existing data_quality dq>=80 gate already aligns the Top_10 path, so no consumer re-wiring is required for this deploy. LOW only ever demotes (drops from rank), never promotes. New [v5.88.0 TRUST] band-count log line + a dropped_low_trust field on the [RANK] line. v5.87.0 INTRINSIC-VALUE DISPLAY CEILING (Fix AG, env-gated TFB_INTRINSIC_DISPLAY_CAP default OFF -> intrinsic_value/upside_pct byte-identical to v5.86.0): _cap_intrinsic_display clamps the DISPLAYED fair value (and the Top_10 valuation upside the builder derives from it) to TFB_INTRINSIC_DISPLAY_MAX_PCT (default 35, floored 30) above current price, killing the sector-multiple model's systematic ~2x-price 'fair value' (COL.MC intrinsic 11.98 vs price 5.71 = 109.8%). DISPLAY-ONLY by construction: it runs AFTER _phase_ii_quality_forecast has already read intrinsic_value (so forecast_price_*/expected_roi_*/recommendations are untouched) and the 30 floor keeps the three score/label consumers byte-identical (valuation_score contribution clamped +25; value_view CHEAP at >=30; top-factors >=15). Clamp-only, fail-open, adds NO column (schema 115); tag intrinsic_ceiling_applied avoids the reliability-scan substrings. Caps the intrinsic basis only; an implausible analyst target_price stays the Top_10 builder's v1.0.4 Valuation Sanity gate. v5.86.0 RESILIENT TOP10 FETCH + INSIGHTS EQUITY UNIVERSE (env TFB_TOP10_RESILIENT_FETCH / TFB_INSIGHTS_EQUITY_UNIVERSE). v5.85.4 MY_PORTFOLIO POSITION MATH ON THE EXTERNAL-ROWS PATH (Fix AF): new _compute_position_math fills the four derived position columns (position_cost = qty*avg_cost, position_value = qty*current_price, unrealized_pl, unrealized_pl_pct in percent points) from the sheet's manual Position Qty + Avg Cost; called per-row at the TOP of _compute_portfolio_fields before the weight pass. Root cause: the position math lived only in _canonicalize_provider_row, which runs on provider patches (no manual qty/cost) and only on the engine-fetch path, while My_Portfolio is served via the EXTERNAL-ROWS path -- so qty/cost were preserved on the sheet row but never multiplied out, leaving Position Cost/Value/Unrealized P/L blank on every holding AND starving the weight columns (position_value blank -> total_mv 0 -> actual_weight/weight_gap blank). Fill-only + fail-open; formulas/units identical to _canonicalize_provider_row so both paths agree; runs only for My_Portfolio so every other page is byte-identical to v5.85.3. Adds NO column (My_Portfolio stays 122). v5.85.3 REGISTRY LOOKUP ACTUALLY FINDS THE REGISTRY (Fix AE: _registry_sheet_lookup gains the registry's real API name get_sheet_spec, and _schema_columns_from_any reads tuple-of-ColumnSpec specs -- the engine had silently projected with its static legacy contract since the registry integration was written, which the v2.15.0 market realignment exposed; engine projections now follow the live registry). v5.85.2 ANALYST/TREND FIELDS SURVIVE THE EXTERNAL-ROWS MERGE (Fix AD-3: the 8 analyst/trend keys join _V577_LIVE_OVERWRITE_FIELDS so the fresh quote overwrites the sheet's blank/NaN cells on the external-rows path, and the block's fill-only blank check now treats float NaN + junk strings as blank -- closes the served None-with-tag contradiction the live trace exposed). v5.85.1 ANALYST/TREND BLOCK AT THE FACTORY (Fix AD-2: _apply_analyst_trend_block now also runs at the END of the enriched-quote factory pre-cache AND on the cache-hit return, because the registry-first route layer projects get_enriched_quotes rows itself and bypassed the v5.85.0 boundaries -- post-deploy spot check showed the 8 columns as None; boundary calls kept, fill-only + idempotent). v5.85.0 HISTORY-GAP BACKFILL + ANALYST/TREND OUTPUT BLOCK (Fix AC + AD, both env-gated default ON, both fill-only -- NO recommendation, verdict, reliability, scoring, or schema change; with both off the output is byte-identical to v5.84.0): (AC, TFB_HISTORY_GAP_BACKFILL) the orchestrator history-technicals trigger is gap-aware (fires when ANY of rsi_14/volatility_30d/volatility_90d/max_drawdown_1y/avg_volume_10d/avg_volume_30d is missing, was all-four-missing) and _fetch_yahoo_chart_patch + _fetch_history_patch pickers gain the yahoo_chart_provider's REAL entry-point names (fetch_price_history/fetch_history/fetch_ohlc_history/fetch_prices -- the v5.77.14 name-mismatch class on the chart side), closing the 2026-06-10 audit's 100%-blank Volatility 90D + Avg Volume 10D/30D on Market_Leaders (76/76) and Commodities_FX (75/75); (AD, TFB_ANALYST_TREND_BLOCK) _apply_analyst_trend_block at the _strict_project_row/get_page_rows boundaries (AFTER the gate) derives the eight schema-defined-but-never-emitted presentation columns -- analyst_rating<-provider_rating, target_price<-target_mean_price, upside_downside_pct (FRACTION, matches upside_pct), trend_1m/3m/12m UP/FLAT/DOWN from expected_roi_*m (+/-2% deadband, forecast_price fallback), signal BUY/NEUTRAL/SELL from _RECO_DIRECTION, st_signal RSI-extremes/candlestick/1M-momentum -- derivation-only, no provider calls, fill-only, fail-open, alias keys emitted, tag analyst_trend_block_applied (reliability-scan substring-safe). v5.84.0 DECISION-GOVERNANCE TRANSPARENCY TAGS (Fix AA, three independently env-gated phases, ALL tag-only/sanitization -- NO recommendation, verdict, reliability, or schema change; with all three off the output is byte-identical to v5.83.3): (AA-1, TFB_DIVERGENCE_TAG default ON, threshold TFB_DIVERGENCE_ROI_MIN=0.25) a sell-family (TRIM-direction) row whose signed governing forecast ROI (_gate_governing_forecast_roi, the Fix S helper) is >= +25% gets warnings tag bearish_reco_high_modeled_upside -- the bearish mirror of Fix S, marking the 114 GM + 4 ML score/forecast contradictions from the 2026-06-10 audit for review without flipping any recommendation; (AA-2, TFB_CONF_CLUSTER_TAG default ON, list TFB_CONF_CLUSTER_VALUES=66.41,64.82,63.23,58.0) a row whose confidence_score sits exactly on a configured formula-attractor value gets warnings tag confidence_default_suspected, making the audit's 900x66.41 / 583x64.82 Global_Markets clusters visible per-row without demoting (Fix S2 standing decision preserved); (AA-3, TFB_FC_SOURCE_TYPE_GUARD default ON) a junk forecast_source literal (digits-only / true/false/none/null/nan -- the audit's 4 GM rows reading "1") is cleared to blank + tagged fc_source_invalid_cleared BEFORE fc_src is consumed, narrow by construction. All three tags avoid the reliability-scan substrings (cap/forecast/target/roi/drop/reject) so repeat gate passes are byte-identical. v5.83.3 FORECAST-RELIABILITY RECALIBRATION (Fix S2, env-gated TFB_RELIABILITY_RECALIBRATION, default OFF -> forecast_reliability_score byte-identical to v5.83.2): _apply_investability_gate's reliability base optionally blends data_quality (0.7*forecast_confidence + 0.3*data_quality_score) and the soft-display-cap penalty for a provider target compressed to the Phase-II ceiling (_capped_to_phase_ii_ceiling) drops 20->5, while a DROPPED/REJECTED-as-implausible target still takes its independent -15; corrects the flat -20 that branded the highest-upside real-analyst names (NVDA/META/MSFT 46.4/46.4/44.8) as least reliable and pushed them below the 70 floor into Top_10 backfill (with the blend at dq=100 they land ~71.5/71.5/70.4). Adds NO column (schema 115). v5.80.0 PORTFOLIO & INSIGHTS DECISION LAYER: My_Portfolio 115->122 (this page only; +Buy Date/Target Weight %/Actual Weight %/Weight Gap/Rebalance Action/Investor Decision/User Notes, mirroring the Top_10 115+3 pattern); _compute_portfolio_fields fills actual_weight/weight_gap (PERCENT POINTS) + action_flag (drift-only ADD/HOLD/REDUCE/SELL) + decision (blended SELL/REDUCE/ADD/HOLD from drift+reco+forecast-sign+risk), target_weight seeded from TFB_PORTFOLIO_TARGETS (1120=40/4013=30/7020=30) when blank, band TFB_PORTFOLIO_REBALANCE_BAND_PP=5.0pp, weak floor TFB_PORTFOLIO_WEAK_SCORE=60; Insights_Analysis rebuilt from live portfolio+market data (Portfolio Summary/Allocation vs Target/Market Opportunities/Data Quality) replacing the 2-row stub; Top_10 eligibility now also requires INVESTABLE and rejects negative-forecast buy-family (closing the Fix S/Fix J seam). Market pages stay 115. Pairs with 00_Config.gs v1.12.3. v5.79.7 NEGATIVE-FORECAST INVESTABILITY DEMOTION (Fix S): _apply_investability_gate demotes a BUY-family row (STRONG_BUY/BUY/ACCUMULATE) whose signed governing forecast ROI (12M preferred, 3M fallback, fraction units; derived from forecast_price vs current price when an expected_roi_* is absent) is strictly negative from INVESTABLE/INVEST to WATCHLIST/WATCH with block_reason "Negative forecast (expected ROI X.X%)"; the branch sits after the HOLD/moderate-DQ/incomplete-fundamentals branch and before the INVESTABLE else, so it ONLY ever demotes (never promotes/relaxes, adds NO column, schema 115); fail-open when no horizon is determinable; env TFB_GATE_BLOCK_NEGATIVE_ROI (default ON; 0/false/off restores the exact v5.79.6 verdict). v5.79.6 PROVIDER-NAME RESOLUTION (Fix R): ProviderRegistry canonicalizes a provider-alias map ('yahoo'/'yfinance' -> 'yahoo_chart') before lookup, the DEFAULT_PROVIDERS/DEFAULT_KSA_PROVIDERS/DEFAULT_GLOBAL_PROVIDERS lists name 'yahoo_chart' directly, and get() logs ONCE when a provider name fails to resolve. Root cause: the lists named 'yahoo' but the importable module is yahoo_chart_provider.py and the registry's candidate-suffix loop never tried _chart_provider, so get('yahoo') returned None and Yahoo was silently skipped on every page -- starving the KSA (.SR) and Commodities/FX (=F/=X) books of spot prices even though yahoo_chart.fetch_quote('2222.SR') returns 27.38/EXCELLENT. No schema/verdict/scoring change; schema 115. v5.79.5 GATE PRICE-COLUMN CONSISTENCY (Fix Q): _apply_investability_gate backfills current_price from the `price` alias before judging has_price, so an INVESTABLE/INVEST verdict can never sit next to a blank Current Price cell (closes the NBK.KW/GFH.KW/CPI.JSE/FOLD.US null-price-investable leak); a row with neither field still reads BLOCKED "Missing current price"; never overwrites an existing price, never changes a correct verdict; schema 115. v5.79.4 STRICT FINAL-APPROVAL TIER (Fix P, default OFF): _apply_investability_gate can DEMOTE a base-INVESTABLE/INVEST row to WATCHLIST/WATCH when it fails the audits' "Final rule" floors (dq>=80, forecast_reliability>=70, risk not HIGH, no unreviewed provider/engine conflict), writing "Strict gate: ..." into block_reason; never promotes/relaxes, adds NO column (schema 115), reversible via TFB_STRICT_INVEST_GATE (off) / TFB_STRICT_INVEST_DQ_MIN / TFB_STRICT_INVEST_RELIABILITY_MIN. Governance only, NOT prediction (backtest lives in scripts/track_performance.py). v5.79.3 PROVIDER-TARGET SOFT CAP (Fix O): _cap_provider_target_forecasts maps an over-ceiling provider target via a monotonic bounded soft compression (cap_abs + band*(1-exp(-excess/band)), default band 0.05) instead of a HARD clamp to +/-30%, so distinct out-of-band targets keep distinct ORDERED forecasts/ROIs and cross-sectional ranking no longer saturates; in-band targets untouched; env TFB_PROVIDER_TARGET_SOFT_CAP (default ON) / TFB_PROVIDER_TARGET_SOFT_CAP_BAND; warning tags + schema (115) unchanged. v5.79.2 GATE LABELING + DETECTION: (Fix M) final_decision_basis "Engine (provider conflict flagged)" instead of "(provider override)" since a conflict is flagged not acted on; (Fix N) fundamentals_apply also exempts rows whose industry is a fund-vehicle label (_GATE_FUNDAMENTALS_EXEMPT_INDUSTRIES, exact-match) catching ETFs mislabeled Equity. schema unchanged 115. v5.79.1 GATE REFINEMENTS: (Fix K) provider_engine_conflict now compares canonical DIRECTION via _provider_rating_direction (provider_rating is TEXT in prod, so numeric-only path left it permanently FALSE); (Fix L) D/E+FCF gate requirement + DQ weights now asset-class-aware via _GATE_FUNDAMENTALS_EXEMPT_TOKENS (ETF/fund/commodity/FX/index exempt; equities incl banks/REITs unchanged). schema unchanged 115. v5.79.0 EODHD FUNDAMENTALS FALLBACK: _apply_eodhd_fundamentals_fallback fills blank debt_to_equity/free_cash_flow_ttm (+ other still-missing fundamentals) from EODHD's fundamentals endpoint AFTER Yahoo, BEFORE scoring/gate, fill-only, one extra call only on gap rows (env TFB_EODHD_FUNDAMENTALS_FALLBACK); schema unchanged 115. v5.78.0 INVESTABILITY GATE (schema 107->115; pairs w/ 00_Config.gs v1.11.0): _apply_investability_gate emits data_quality_score/forecast_reliability_score/provider_engine_conflict/conflict_type/final_decision_basis/investability_status/final_action/block_reason. PRIOR: v5.77.23 on v5.77.22: (Fix H) _reconcile_recommendation_family() now also refreshes a RICH position_size_hint whose direction contradicts the final reco (e.g. ACCUMULATE + "hold existing; no new capital") while preserving consistent rich hints; (Fix I) _classify_recommendation_8tier() forces a neutral HOLD when current_price is missing (not actionable); (Fix J) Top 10 build paths exclude missing-price and REDUCE/SELL/STRONG_SELL/AVOID rows. v5.77.22 base: (F) 52W range + rolling volume in _compute_history_patch_from_rows, (G) subunit GBX/GBp/ZAC/ILA market-cap normalization. v5.77.21 base: (C/D/E) reco-family reconciliation hardening; v5.77.20 base: (A/B) reconciliation + provider-target cap
