...
================================================================================
INVESTMENT ADVISOR ENGINE -- v3.7.0
================================================================================
...
v3.7.0 changes vs v3.6.0
--------------------------
FIX: _SCHEMA_MODULE_CANDIDATES: added "schema_registry" repo-root fallback.
  v3.6.0 only tried core.sheets.*, core.schema_registry, core.page_catalog,
  core.schemas, core.schema. When none resolved, _load_schema_defaults fell to
  _SPECIAL_SCHEMA_DEFAULTS — which had wrong keys for Insights / Data_Dictionary.

FIX: _PAGE_CATALOG_MODULE_CANDIDATES: added "page_catalog" repo-root fallback.
  v3.6.0 tried core.sheets.page_catalog and core.page_catalog only.

FIX: _SPECIAL_SCHEMA_DEFAULTS[INSIGHTS_PAGE_NAME]: "Source Page"/"source_page"
  → "Source"/"source" (canonical schema_registry v3.0.0).
  Canonical Insights_Analysis keys: section, item, metric, value, notes,
  source, sort_order. v3.6.0 "source_page" caused schema projection to drop
  the source column whenever the live schema_registry was unavailable.

FIX: _SPECIAL_SCHEMA_DEFAULTS[DATA_DICTIONARY_PAGE_NAME]: aligned to canonical
  Data_Dictionary schema (schema_registry v3.0.0 / data_dictionary.py v3.3.0).
  Canonical: Sheet, Group, Header, Key, DType, Format, Required, Source, Notes
  (keys: sheet, group, header, key, dtype, fmt, required, source, notes).
  Previous (non-canonical): Section, Field, Description, Type, Example.

FIX: _build_insights_sheet_payload: all seven insight rows changed "source_page"
  field to "source" to match the updated schema projection keys.
  The internal source_page bookkeeping in _collect_candidate_rows is unchanged.

Preserved from v3.6.0 (no behavioral changes):
  canonical RECO constants, _normalize_reco fallback, existing-reco
  normalization in _score_and_rank_rows, _recommendation_from_scores using
  STRONG_BUY/BUY/HOLD/REDUCE/SELL throughout.
================================================================================
"""
...
try:
    from core.reco_normalize import (  # type: ignore
        ...
        RECO_REDUCE,
        RECO_SELL,
        normalize_recommendation as _normalize_reco,
    )
except Exception:
    ...
    def _normalize_reco(x: Any) -> str:  # type: ignore[misc]
        _map = {
            "STRONG_BUY": RECO_STRONG_BUY, "STRONG BUY": RECO_STRONG_BUY,
            "BUY": RECO_BUY, "ACCUMULATE": RECO_BUY, "ADD": RECO_BUY,
            "OUTPERFORM": RECO_BUY, "OVERWEIGHT": RECO_BUY,
            "HOLD": RECO_HOLD, "NEUTRAL": RECO_HOLD, "WATCH": RECO_HOLD,
            "REDUCE": RECO_REDUCE, "TRIM": RECO_REDUCE, "UNDERWEIGHT": RECO_REDUCE,
            "SELL": RECO_SELL, "AVOID": RECO_SELL, "EXIT": RECO_SELL,
            "UNDERPERFORM": RECO_SELL,
        }
        raw = str(x or "").strip().upper().replace("-", "_").replace(" ", "_")
        return _map.get(raw, RECO_HOLD)

logger = logging.getLogger("core.investment_advisor_engine")
logger.addHandler(logging.NullHandler())

INVESTMENT_ADVISOR_ENGINE_VERSION = "3.7.0"
...
_PASSTHROUGH_PAGES = {
    ...
    "My_Portfolio", INSIGHTS_PAGE_NAME, DATA_DICTIONARY_PAGE_NAME,
}

_DERIVED_OR_NON_SOURCE_PAGES = {
    "KSA_TADAWUL", "Advisor_Criteria", "AI_Opportunity_Report",
    INSIGHTS_PAGE_NAME, TOP10_PAGE_NAME, DATA_DICTIONARY_PAGE_NAME,
}

_ENGINE_SHEET_METHOD_CANDIDATES = (
    "get_sheet_rows", "get_page_rows", "sheet_rows", "build_sheet_rows", "get_rows",
)

# FIX v3.7.0: added "schema_registry" repo-root fallback
...
_SCHEMA_FN_CANDIDATES = (
    "get_sheet_spec", "get_page_spec", "get_schema_for_page",
    "sheet_spec", "build_sheet_spec",
)

# FIX v3.7.0: added "page_catalog" repo-root fallback
...
_PAGE_NORMALIZER_FN_CANDIDATES = (
    "normalize_page_name", "resolve_page_name", "canonical_page_name",
    "canonicalize_page_name", "normalize_sheet_name",
)
...
_TOP10_BUILDER_FN_CANDIDATES = (
    ...
    "build_top10_investments_rows", "build_top10",
)

_TOP10_BUILDER_OBJECT_CANDIDATES = (
    "top10_selector", "top10_builder", "selector", "builder", "service",
)

_SPECIAL_SCHEMA_DEFAULTS: Dict[str, Tuple[List[str], List[str]]] = {
    TOP10_PAGE_NAME: (
        [
            "Symbol", "Name", "Current Price", "Expected ROI 3M",
            "Forecast Confidence", "Risk Score", "Overall Score",
            "Recommendation", "Last Updated (Riyadh)",
            "Top10 Rank", "Selection Reason", "Criteria Snapshot",
        ],
        [
            "symbol", "name", "current_price", "expected_roi_3m",
            "forecast_confidence", "risk_score", "overall_score",
            "recommendation", "last_updated_riyadh",
            "top10_rank", "selection_reason", "criteria_snapshot",
        ],
    ),
    # FIX v3.7.0: "Source Page"/"source_page" → "Source"/"source"
    # Canonical Insights_Analysis schema (schema_registry v3.0.0):
    #   section, item, metric, value, notes, source, sort_order
    INSIGHTS_PAGE_NAME: (
        ["Section", "Item", "Metric", "Value", "Notes", "Source", "Sort Order"],
        ["section", "item", "metric", "value", "notes", "source", "sort_order"],
    ),
    # FIX v3.7.0: aligned to canonical Data_Dictionary schema.
    # Canonical (schema_registry v3.0.0 / data_dictionary.py v3.3.0):
...
def _is_signature_mismatch_typeerror(exc: TypeError) -> bool:
    ...
    return any(t in msg for t in (
        ...
        "required positional argument", "takes ", "got an unexpected keyword",
        "got multiple values for argument", "missing 1 required positional argument",
        "missing required positional argument", "but ", "was given",
    ))
...
def _get_request_path(request: Any) -> str:
    try:   return _s(getattr(getattr(request, "url", None), "path", ""))
    except: return ""
...
def _get_request_id(request: Any = None) -> str:
    try:
        if request is not None:
            ...
            if rid: return _s(rid)
            hdr = request.headers.get("X-Request-ID")
            if hdr: return _s(hdr)
    except Exception:
        pass
...
def _model_to_dict(obj: Any) -> Dict[str, Any]:
    ...
    try:
        if hasattr(obj, "__dict__"):
            d = vars(obj)
            if isinstance(d, dict): return dict(d)
    except Exception: pass
    return {"result": obj}
...
def _extract_matrix_candidate(payload: Any) -> Optional[List[List[Any]]]:
    ...
    if isinstance(nested, dict): return _extract_matrix_candidate(nested)
    return None
...
def _extract_headers_keys(payload: Any) -> Tuple[List[str], List[str]]:
    if not isinstance(payload, dict):
        ...
        if not isinstance(payload, dict): return [], []
    headers = (
        payload.get("display_headers") or payload.get("sheet_headers") or
        payload.get("column_headers")  or payload.get("headers") or []
    )
    ...
    if isinstance(nested, dict) and (not clean_headers or not clean_keys):
        nh, nk = _extract_headers_keys(nested)
        if not clean_headers: clean_headers = nh
        if not clean_keys:    clean_keys    = nk
...
def _has_tabular_shape(payload: Any) -> bool:
    ...
    for key in ("rows", "row_objects", "rowObjects", "rows_matrix", "matrix", "headers", "keys", "display_headers"):
        if isinstance(payload.get(key), list): return True
    return False
...
def _make_error_payload(
    ...
    page: str = TOP10_PAGE_NAME, operation: str = "run",
    ...
    meta = {
        ...
        "operation": operation, "target_page": page,
    }
    if isinstance(extra_meta, dict): meta.update(extra_meta)
    return {
        "status": "error", "page": page, "sheet": page,
        "route_family": _route_family_for_page(page),
        "headers": [], "display_headers": [], "keys": [],
        "rows": [], "row_objects": [], "items": [], "data": [],
        "rows_matrix": [], "count": 0, "detail": detail, "meta": meta,
    }
...
def _run_coro_sync(coro: Any) -> Dict[str, Any]:
    ...
    t.start(); t.join()
    if box["error"] is not None: raise box["error"]
    return box["result"]
...
def _fallback_normalize_page_name(value: Any) -> str:
    ...
    alias_map  = {
        "top10": TOP10_PAGE_NAME, "top_10": TOP10_PAGE_NAME,
        "top_10_investments": TOP10_PAGE_NAME, "top10_investments": TOP10_PAGE_NAME,
        "insights": INSIGHTS_PAGE_NAME, "insights_analysis": INSIGHTS_PAGE_NAME,
        "data_dictionary": DATA_DICTIONARY_PAGE_NAME, "dictionary": DATA_DICTIONARY_PAGE_NAME,
        "market_leaders": "Market_Leaders", "global_markets": "Global_Markets",
        "mutual_funds": "Mutual_Funds", "commodities_fx": "Commodities_FX",
        "my_portfolio": "My_Portfolio", "ai_opportunity_report": "AI_Opportunity_Report",
        "advisor_criteria": "Advisor_Criteria", "ksa_tadawul": "KSA_TADAWUL",
    }
...
def _normalize_page_name(value: Any) -> str:
    ...
    for mod_name in _PAGE_CATALOG_MODULE_CANDIDATES:
        ...
        for fn_name in _PAGE_NORMALIZER_FN_CANDIDATES:
            ...
            if not _callable(fn): continue
            for kwargs in ({"page": raw}, {"sheet": raw}, {"sheet_name": raw}, {"name": raw}, {}):
                try:
                    out = fn(**kwargs) if kwargs else fn(raw)
                except TypeError as exc:
                    if _is_signature_mismatch_typeerror(exc): continue
                    break
                except Exception:
                    continue
                if isinstance(out, str) and _s(out): return _s(out)
        ...
        for attr_name in ("PAGE_ALIASES", "PAGE_ALIAS_MAP", "ALIASES", "CANONICAL_PAGE_ALIASES"):
            ...
            if isinstance(aliases, dict):
                for probe in [raw, raw.lower(), raw.replace("-","_").replace(" ","_"), raw.upper()]:
                    val = aliases.get(probe)
                    if val: return _s(val)
...
def _ensure_top10_keys_present(keys: List[str], headers: List[str]) -> Tuple[List[str], List[str]]:
    extras = [("top10_rank","Top10 Rank"),("selection_reason","Selection Reason"),("criteria_snapshot","Criteria Snapshot")]
    out_keys = list(keys or []); out_headers = list(headers or [])
    for key, header in extras:
        if key not in out_keys:
            out_keys.append(key); out_headers.append(header)
...
def _extract_columns_from_spec(spec: Any) -> List[Tuple[str, str]]:
    ...
    if isinstance(raw_columns, list):
        for col in raw_columns:
            ...
            if key: columns.append((key, header or key))
    if not columns and isinstance(spec, dict):
        keys = spec.get("keys") or []; headers = spec.get("headers") or spec.get("display_headers") or []
        if isinstance(keys, list):
            for idx, key in enumerate(keys):
                k = _s(key); h = _s(headers[idx]) if isinstance(headers, list) and idx < len(headers) else k
                if k: columns.append((k, h or k))
    return columns
...
async def _load_schema_defaults(page_name: str = TOP10_PAGE_NAME) -> Tuple[List[str], List[str]]:
    ...
    for mod_name in _SCHEMA_MODULE_CANDIDATES:
        ...
        for fn_name in _SCHEMA_FN_CANDIDATES:
            fn = getattr(mod, fn_name, None)
            if not _callable(fn): continue
            for kwargs in ({"sheet": canonical_page}, {"page": canonical_page}, {"sheet_name": canonical_page}, {"name": canonical_page}):
                try:
                    spec = await _call_maybe_async(fn, **kwargs)
                    cols = _extract_columns_from_spec(spec)
                    if cols:
                        keys = [k for k,_ in cols]; headers = [h for _,h in cols]
    ...
    if fallback:
        ...
        headers = list(headers); keys = list(keys)
        if canonical_page == TOP10_PAGE_NAME:
            keys, headers = _ensure_top10_keys_present(keys, headers)
        return headers, keys
    return [], []
...
def _looks_like_engine(obj: Any) -> bool:
    ...
    for name in _ENGINE_SHEET_METHOD_CANDIDATES:
        try:
            if _callable(getattr(obj, name, None)): return True
...
async def _get_engine_from_request(request: Any) -> Any:
    try:
        app = getattr(request, "app", None); st = getattr(app, "state", None)
    ...
    for modpath in ("core.data_engine_v2", "core.data_engine"):
        try:
            ...
            if _callable(get_engine):
                eng = get_engine()
                if inspect.isawaitable(eng): eng = await eng
                return eng
        except Exception: continue
    return None
...
async def _extract_context(*args: Any, **kwargs: Any) -> Tuple[Any, Any, Any, Dict[str, Any]]:
    ...
    for key in ("payload", "body", "request_data", "params"):
        val = kwargs.get(key)
        if isinstance(val, dict): payload = _merge_dicts(payload, val)
    criteria = kwargs.get("criteria")
    if isinstance(criteria, dict):
        payload["criteria"] = _merge_dicts(
    ...
    for arg in args:
        ...
        if isinstance(arg, dict):
            if not payload:
                ...
                if "criteria" not in payload: payload["criteria"] = {}
...
def _flatten_criteria(payload: Dict[str, Any]) -> Dict[str, Any]:
    out: Dict[str, Any] = {}
    if isinstance(payload.get("criteria"), dict): out.update(payload["criteria"])
    if isinstance(payload.get("filters"),  dict): out.update(payload["filters"])
    for key in (
        "pages_selected","pages","selected_pages","sources",
        "page","sheet","sheet_name",
        "direct_symbols","symbols","symbol","tickers",
        "limit","top_n","offset",
        "investment_period_days","invest_period_days","horizon_days",
        "risk_level","risk_profile","confidence_level","confidence_bucket",
        "min_expected_roi","min_roi","min_confidence",
        "mode","advisor_data_mode",
        "debug","schema_only","headers_only","include_matrix","format",
        "invest_amount","allocation_strategy",
    ):
    ...
    normalized_pages = []
    for item in (
        _normalize_list(out.get("pages_selected")) or _normalize_list(out.get("pages")) or
        _normalize_list(out.get("selected_pages")) or _normalize_list(out.get("sources"))
    ):
...
def _normalize_sources(criteria: Dict[str, Any]) -> List[str]:
    pages = (
        _normalize_list(criteria.get("pages_selected")) or _normalize_list(criteria.get("pages")) or
        _normalize_list(criteria.get("selected_pages")) or _normalize_list(criteria.get("sources"))
    )
    page_single = (_s(criteria.get("page")) or _s(criteria.get("sheet")) or _s(criteria.get("sheet_name")))
    if page_single and not pages: pages = [page_single]
    pages = _dedupe_keep_order([_normalize_page_name(x) or _s(x) for x in pages])
    out: List[str] = []
    for item in pages:
        if item.upper() == "ALL": out.extend(_BASE_SOURCE_PAGES)
        else: out.append(item)
    out = _dedupe_keep_order(out)
    out = [p for p in out if p and p not in _DERIVED_OR_NON_SOURCE_PAGES]
    if not out: out = list(_BASE_SOURCE_PAGES)
    return out


def _normalize_direct_symbols(criteria: Dict[str, Any]) -> List[str]:
    return (
        _normalize_list(criteria.get("direct_symbols")) or _normalize_list(criteria.get("symbols")) or
        _normalize_list(criteria.get("tickers"))        or _normalize_list(criteria.get("symbol"))
    )

def _effective_limit(criteria: Dict[str, Any], default: int = 10) -> int:
    return max(1, min(200, _safe_int(criteria.get("limit") or criteria.get("top_n"), default)))
...
def _normalize_mode(criteria: Dict[str, Any]) -> str:
    ...
    if not mode or mode == "auto":            return "live_quotes"
    if mode in {"live", "quotes"}:            return "live_quotes"
    if mode in {"sheet", "live_sheet"}:       return "live_sheet"
    if mode in {"snapshot", "snapshots"}:     return "snapshot"
    return mode
...
def _normalize_target_page(criteria: Dict[str, Any]) -> str:
    ...
    return _normalize_page_name(target) or TOP10_PAGE_NAME

def _infer_operation(request: Any, criteria: Dict[str, Any]) -> str:
    path = _get_request_path(request).lower()
    if "sheet-rows" in path:                                                             return "sheet_rows"
    if "recommendations" in path:                                                        return "recommendations"
    if path.endswith("/run") or path.endswith("/advisor") or path.endswith("/investment-advisor"): return "run"
    if _s(criteria.get("format")).lower() == "rows":                                     return "sheet_rows"
    return "run"
...
async def _call_engine_sheet_rows(
    engine: Any, *, page: str, limit: int, mode: str, payload: Dict[str, Any],
) -> Dict[str, Any]:
    page = _normalize_page_name(page) or page
    if engine is None:
        return _make_error_payload("Data engine unavailable", page=page, operation="sheet_rows",
                                   extra_meta={"dispatch": "engine.missing"})
    fn = None; fn_name = ""
    for name in _ENGINE_SHEET_METHOD_CANDIDATES:
        candidate = getattr(engine, name, None)
        if _callable(candidate): fn = candidate; fn_name = name; break
    if fn is None:
        return _make_error_payload("Engine sheet-row handler not found", page=page, operation="sheet_rows",
    ...
    attempts = [
        ...
        {"page": page, "limit": limit}, {"sheet": page, "limit": limit},
        {"sheet_name": page, "limit": limit}, {"name": page, "limit": limit},
    ]
    out: Any = None
    for kwargs in attempts:
        try:
            out = await _call_maybe_async(fn, **kwargs); break
        except TypeError as exc:
            if _is_signature_mismatch_typeerror(exc): continue
            return _make_error_payload(f"Engine page call failed: {type(exc).__name__}: {exc}",
                                       page=page, operation="sheet_rows", extra_meta={"dispatch": f"engine.{fn_name}"})
        except Exception as e:
            return _make_error_payload(f"Engine page call failed: {type(e).__name__}: {e}",
                                       page=page, operation="sheet_rows", extra_meta={"dispatch": f"engine.{fn_name}"})
    if out is None:
        for args in ((page, limit), (page,), tuple()):
            try:
                out = await _call_maybe_async(fn, *args); break
            except TypeError as exc:
                if _is_signature_mismatch_typeerror(exc): continue
                return _make_error_payload(f"Engine page call failed: {type(exc).__name__}: {exc}",
                                           page=page, operation="sheet_rows", extra_meta={"dispatch": f"engine.{fn_name}"})
    ...
    if isinstance(out, dict):
        ...
        if out and isinstance(out[0], dict):
            ...
            keys = _derive_keys_from_rows(rows); headers = list(keys)
            result = {"status": "success" if rows else "partial", "page": page, "sheet": page,
                      "headers": headers, "display_headers": headers, "keys": keys,
                      "rows": rows, "row_objects": rows, "data": rows, "items": rows,
                      "rows_matrix": _rows_to_matrix(rows, keys) if keys else []}
        else:
            result = {"status": "partial", "page": page, "sheet": page,
                      "detail": "Engine returned non-dict list rows",
                      "rows": [], "row_objects": [], "rows_matrix": [], "headers": [], "keys": []}
    else:
    ...
    result["meta"] = meta
    result.setdefault("page", page); result.setdefault("sheet", page)
    return result


def _normalize_engine_page_payload(result: Dict[str, Any], *, page: str) -> Dict[str, Any]:
    payload = dict(result or {}); page = _normalize_page_name(page) or page
    headers, keys = _extract_headers_keys(payload)
    rows           = _extract_rows_candidate(payload)
    matrix         = _extract_matrix_candidate(payload)
    if not rows and matrix:
        if not keys: keys = list(headers)
        if not keys and matrix and isinstance(matrix[0], list):
            keys = [f"col_{i+1}" for i in range(len(matrix[0]))]
        if keys: rows = _matrix_to_rows(matrix, keys)
    if rows and not keys:   keys    = _derive_keys_from_rows(rows)
    if keys and not headers: headers = list(keys)
    if rows and not matrix and keys: matrix = _rows_to_matrix(rows, keys)
    if matrix is None: matrix = []
    status_value = _s(payload.get("status")) or ("success" if rows or matrix or headers or keys else "partial")
    ...
    return {
        "status": status_value, "page": _s(payload.get("page")) or page,
        "sheet": _s(payload.get("sheet")) or page, "route_family": _route_family_for_page(page),
        "headers": headers, "display_headers": headers, "sheet_headers": headers,
        "column_headers": headers, "keys": keys,
        "rows": rows, "row_objects": rows, "items": rows, "data": rows,
        "rows_matrix": matrix, "count": max(len(rows), len(matrix)),
...
def _rows_from_engine_result(result: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if matrix:
        ...
        if not keys: keys = headers
        if keys: return _matrix_to_rows(matrix, keys)
    return []

def _symbol_of(row: Dict[str, Any]) -> str:
    return (_s(row.get("symbol")) or _s(row.get("ticker")) or _s(row.get("code")) or
            _s(row.get("symbol_code")) or _s(row.get("instrument")))

def _name_of(row: Dict[str, Any]) -> str:
    return (_s(row.get("name")) or _s(row.get("company_name")) or
            _s(row.get("description")) or _s(row.get("short_name")))


async def _collect_candidate_rows(
    *, engine: Any, criteria: Dict[str, Any], mode: str,
    ...
    for page in pages:
        page_result = await _call_engine_sheet_rows(engine, page=page, limit=fetch_limit,
                                                    mode=mode, payload=criteria)
        ...
        if not page_rows:
            ...
            if detail: page_errors[page] = detail
        for row in page_rows:
            item = dict(row)
            item.setdefault("source_page", page)   # internal bookkeeping only
            sym = _symbol_of(item)
            if not sym and direct_symbols_upper: continue
            if direct_symbols_upper and sym.upper() not in direct_symbols_upper: continue
            key = sym.upper() if sym else f"__row_{len(collected)+1}"
            if key not in by_symbol:
                by_symbol[key] = item; collected.append(item)
            else:
    ...
    if direct_symbols_upper:
        ...
        for sym_upper in direct_symbol_lookup.keys():
            if sym_upper not in existing_upper:
                ...
                collected.append({"symbol": seed, "name": seed, "source_page": "direct_symbols"})

    return collected, {
        "pages_effective": pages, "page_counts": page_counts, "page_errors": page_errors,
        "candidate_rows": len(collected), "direct_symbols_count": len(direct_symbols_upper),
    }
...
def _extract_expected_roi_3m(row: Dict[str, Any]) -> float:
    for key in ("expected_roi_3m","expected_roi","roi_3m","expected_return_3m"):
        v = _maybe_ratio(row.get(key))
        if v is not None: return v
    cp = _extract_current_price(row); tp = _extract_target_price(row)
    if cp is not None and tp is not None and cp > 0: return (tp / cp) - 1.0
    roi1 = _maybe_ratio(row.get("expected_roi_1m"))
    if roi1 is not None: return roi1 * 3.0
    roi12 = _maybe_ratio(row.get("expected_roi_12m"))
    if roi12 is not None: return roi12 / 4.0
    upside = _maybe_ratio(row.get("upside_pct"))
    if upside is not None: return upside / 100.0 if abs(upside) > 1.0 else upside
    return 0.0
...
def _extract_confidence_score(row: Dict[str, Any]) -> float:
    for key in ("forecast_confidence","confidence_score","ai_confidence","confidence"):
        ...
        if isinstance(raw, str):
            ...
            if s in {"high","strong"}:     return 0.85
            if s in {"moderate","medium"}: return 0.60
            if s in {"low","weak"}:        return 0.35
            try:   raw = float(s)
            except: raw = None
        v = _maybe_ratio(raw)
        if v is not None:
            return _clamp(v / 100.0, 0.0, 1.0) if abs(v) > 1.0 else _clamp(v, 0.0, 1.0)
    return 0.55

def _extract_risk_score(row: Dict[str, Any]) -> float:
    v = _maybe_ratio(row.get("risk_score"))
    if v is not None: return _clamp(v * 100.0, 0.0, 100.0) if abs(v) <= 1.0 else _clamp(v, 0.0, 100.0)
    vol = _maybe_ratio(row.get("volatility_90d"))
    if vol is not None: return _clamp(abs(vol)*100.0, 0.0, 100.0) if abs(vol)<=1.0 else _clamp(abs(vol),0.0,100.0)
    dd = _maybe_ratio(row.get("max_drawdown_1y"))
    if dd is not None: return _clamp(abs(dd)*100.0, 0.0, 100.0) if abs(dd)<=1.0 else _clamp(abs(dd),0.0,100.0)
    beta = _maybe_ratio(row.get("beta"))
    if beta is not None: return _clamp(abs(beta) * 30.0, 0.0, 100.0)
    return 40.0
...
def _recommendation_from_scores(
    ...
    """
    FIX v3.6.0: Was returning non-canonical labels ("Strong Buy", "Avoid").
    Returns canonical 5-value vocabulary: STRONG_BUY/BUY/HOLD/REDUCE/SELL.
    """
...
def _canonical_selection_reason(row: Dict[str, Any]) -> Optional[str]:
    ...
    for label, key in (("overall","overall_score"),("opportunity","opportunity_score"),
                        ("value","value_score"),("quality","quality_score"),
                        ("momentum","momentum_score"),("growth","growth_score")):
        val = row.get(key)
        if isinstance(val, (int, float)): score_parts.append(f"{label}={round(float(val),2)}")
    roi_parts: List[str] = []
    for label, key in (("1M","expected_roi_1m"),("3M","expected_roi_3m"),("12M","expected_roi_12m")):
        val = row.get(key)
        if isinstance(val, (int, float)): roi_parts.append(f"{label} ROI={round(float(val)*100,2)}%")
...
def _score_and_rank_rows(
    ...
    for row in rows:
        ...
        cp = _extract_current_price(r)
        if cp is not None and r.get("current_price") in (None,"","None"): r["current_price"] = cp
        ...
        if risk_pref == "low":   safety   = _clamp(safety + 10.0,    0.0, 100.0)
        elif risk_pref == "high": roi_score = _clamp(roi_score + 7.5, 0.0, 100.0)
        overall_score = _clamp((roi_score * 0.45) + (conf_pct * 0.30) + (safety * 0.25), 0.0, 100.0)
        ...
        # FIX v3.6.0: normalize ANY existing recommendation to canonical vocabulary
        ...
        if _is_blank(r.get("selection_reason")): r["selection_reason"] = _canonical_selection_reason(r)
        if _is_blank(r.get("criteria_snapshot")): r["criteria_snapshot"] = criteria_snapshot
        scored.append(r)

    scored.sort(
        key=lambda x: (float(x.get("overall_score") or 0.0), float(x.get("expected_roi_3m") or 0.0),
...
def _normalize_top10_builder_rows(value: Any) -> List[Dict[str, Any]]:
    ...
    if isinstance(value, list):
        ...
        if value and isinstance(value[0], (list, tuple)):
            _, fallback_keys = _SPECIAL_SCHEMA_DEFAULTS[TOP10_PAGE_NAME]
...
async def _resolve_top10_builder(request: Any, engine: Any) -> Tuple[Any, str]:
    state_obj = await _get_state_builder(request,
        ("top10_selector","top10_builder","investment_top10_selector","investment_top10_builder"))
    if state_obj is not None:
        for fn_name in _TOP10_BUILDER_FN_CANDIDATES:
            fn = getattr(state_obj, fn_name, None)
            if _callable(fn): return fn, f"app.state.{type(state_obj).__name__}.{fn_name}"
        if _callable(state_obj): return state_obj, f"app.state.{type(state_obj).__name__}"
    if engine is not None:
        for fn_name in _TOP10_BUILDER_FN_CANDIDATES:
            fn = getattr(engine, fn_name, None)
            if _callable(fn): return fn, f"engine.{fn_name}"
    for mod_name in _TOP10_BUILDER_MODULE_CANDIDATES:
        try:   mod = importlib.import_module(mod_name)
        except: continue
        for fn_name in _TOP10_BUILDER_FN_CANDIDATES:
            fn = getattr(mod, fn_name, None)
            if _callable(fn): return fn, f"{mod_name}.{fn_name}"
        for obj_name in _TOP10_BUILDER_OBJECT_CANDIDATES:
            obj = getattr(mod, obj_name, None)
            if obj is None: continue
            if _callable(obj): return obj, f"{mod_name}.{obj_name}"
            for fn_name in _TOP10_BUILDER_FN_CANDIDATES:
                fn = getattr(obj, fn_name, None)
                if _callable(fn): return fn, f"{mod_name}.{obj_name}.{fn_name}"
    return None, ""


async def _build_top10_via_builder(
    *, request: Any, engine: Any, criteria: Dict[str, Any],
    limit: int, offset: int, mode: str,
) -> Dict[str, Any]:
    builder, dispatch = await _resolve_top10_builder(request, engine)
    if builder is None: return {}
    attempts = [
        {"request": request, "engine": engine, "criteria": criteria,
         "limit": limit, "offset": offset, "top_n": limit+offset, "mode": mode, "page": TOP10_PAGE_NAME},
        {"request": request, "engine": engine, "criteria": criteria,
         "limit": limit+offset, "page": TOP10_PAGE_NAME},
        {"request": request, "engine": engine, "criteria": criteria,
         "top_n": limit+offset, "page": TOP10_PAGE_NAME},
        {"request": request, "engine": engine, "body": criteria,
         "limit": limit+offset, "page": TOP10_PAGE_NAME},
        {"request": request, "engine": engine, "payload": criteria,
         "limit": limit+offset, "page": TOP10_PAGE_NAME},
        {"request": request, "engine": engine, "symbols": _normalize_direct_symbols(criteria),
         "criteria": criteria, "limit": limit+offset},
        {"criteria": criteria, "limit": limit+offset, "engine": engine},
    ]
    last_error = ""
    for kwargs in attempts:
        try:
            ...
            if rows:
                ...
                return {"rows": window, "meta": {"dispatch": dispatch, "builder_used": True}}
            if isinstance(out, dict) and _has_tabular_shape(out):
                payload = _normalize_engine_page_payload(out, page=TOP10_PAGE_NAME)
                pm = dict(payload.get("meta") if isinstance(payload.get("meta"), dict) else {})
                pm.update({"dispatch": dispatch, "builder_used": True})
                payload["meta"] = pm
                return payload
        except TypeError as exc:
            if _is_signature_mismatch_typeerror(exc): continue
    ...
    if last_error:
        return {"status": "error", "detail": f"Top10 builder failed: {last_error}",
...
async def _build_projected_sheet_payload(
    page_name: str, rows: List[Dict[str, Any]], *, meta: Dict[str, Any], detail: str = "",
) -> Dict[str, Any]:
    page_name = _normalize_page_name(page_name) or page_name
    headers, keys = await _load_schema_defaults(page_name)
    if not keys and rows: keys = _derive_keys_from_rows(rows)
    if not headers and keys: headers = list(keys)
    if page_name == TOP10_PAGE_NAME: keys, headers = _ensure_top10_keys_present(keys, headers)
    projected = [{k: row.get(k, None) for k in keys} for row in rows] if keys else [dict(r) for r in rows]
    matrix    = _rows_to_matrix(projected, keys) if keys else []
    return {
        "status": "success" if projected or headers or keys else "partial",
        "page": page_name, "sheet": page_name, "route_family": _route_family_for_page(page_name),
        "headers": headers, "display_headers": headers, "sheet_headers": headers,
        "column_headers": headers, "keys": keys,
        "rows": projected, "row_objects": projected, "data": projected, "items": projected,
        "rows_matrix": matrix, "count": len(projected), "detail": detail, "meta": meta,
    }


def _build_recommendations_payload(
    recommendations: List[Dict[str, Any]], *, meta: Dict[str, Any],
) -> Dict[str, Any]:
    return {
        "status": "success" if recommendations else "error",
        "page": TOP10_PAGE_NAME, "sheet": TOP10_PAGE_NAME, "route_family": "top10",
        "recommendations": recommendations, "rows": recommendations,
        "row_objects": recommendations, "data": recommendations, "items": recommendations,
...
async def _build_criteria_sheet_payload(criteria: Dict[str, Any], *, meta: Dict[str, Any]) -> Dict[str, Any]:
    row = {
        ...
        "top_n": _effective_limit(criteria), "offset": _effective_offset(criteria),
    ...
    return await _build_projected_sheet_payload(
        "Advisor_Criteria", [row], meta=meta,
...
async def _build_empty_schema_payload(page_name: str, *, meta: Dict[str, Any], detail: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "headers": headers, "display_headers": headers, "sheet_headers": headers,
        "column_headers": headers, "keys": keys,
        "rows": [], "row_objects": [], "data": [], "items": [],
        "rows_matrix": [], "count": 0, "detail": detail, "meta": meta,
    }


async def _build_insights_sheet_payload(
    recommendations: List[Dict[str, Any]], *, criteria: Dict[str, Any], meta: Dict[str, Any],
) -> Dict[str, Any]:
    ...
    # FIX v3.7.0: "source_page" → "source" to match canonical Insights_Analysis
    # schema (source key, not source_page). All seven rows updated.
    rows: List[Dict[str, Any]] = [
        {"section": "Summary",  "item": "Coverage",
         "metric": "Candidate Count",           "value": len(recommendations),
         "notes": f"Sources={','.join(_normalize_sources(criteria))}",
         "source": "advisor_engine", "sort_order": 1},
        {"section": "Summary",  "item": "Expected Return",
         "metric": "Average ROI 3M",            "value": round(avg_roi, 6),
         "notes": "Average across ranked recommendations",
         "source": "advisor_engine", "sort_order": 2},
        {"section": "Summary",  "item": "Confidence",
         "metric": "Average Forecast Confidence","value": round(avg_conf, 6),
         "notes": "Average confidence across ranked recommendations",
         "source": "advisor_engine", "sort_order": 3},
        {"section": "Risk",     "item": "Distribution",
         "metric": "Low Risk Count",            "value": low_risk,
         "notes": "Ranked recommendation distribution",
         "source": "advisor_engine", "sort_order": 4},
        {"section": "Risk",     "item": "Distribution",
         "metric": "Moderate Risk Count",       "value": med_risk,
         "notes": "Ranked recommendation distribution",
         "source": "advisor_engine", "sort_order": 5},
        {"section": "Risk",     "item": "Distribution",
         "metric": "High Risk Count",           "value": high_risk,
         "notes": "Ranked recommendation distribution",
         "source": "advisor_engine", "sort_order": 6},
    ]
    for idx, row in enumerate(recommendations[:3], start=1):
        rows.append({
            "section": "Top Ideas",
            "item":    _symbol_of(row) or _name_of(row) or f"Idea {idx}",
            "metric":  "Recommendation",
            "value":   _s(row.get("recommendation")) or "n/a",
            "notes":   _s(row.get("selection_reason")) or f"ROI3M={round(float(row.get('expected_roi_3m') or 0.0)*100.0,2)}%",
            # Fall back to internal source_page tracking field then default
            "source":  _s(row.get("source_page") or row.get("source")) or "advisor_engine",
            "sort_order": 10 + idx,
        })

    return await _build_projected_sheet_payload(
        INSIGHTS_PAGE_NAME, rows, meta=meta,
        detail="Insights fallback built from advisor scoring",
    )
...
async def _execute_engine_async(*args: Any, **kwargs: Any) -> Dict[str, Any]:
    request = None; target_page = TOP10_PAGE_NAME; operation = "run"
    try:
        ...
        base_meta: Dict[str, Any] = {
            "engine_version": INVESTMENT_ADVISOR_ENGINE_VERSION,
            "request_id": request_id, "generated_at_utc": _now_utc_iso(),
            "operation": operation, "target_page": target_page,
        ...
        if _coerce_bool(criteria.get("schema_only")) or _coerce_bool(criteria.get("headers_only")):
            ...
            if target_page == TOP10_PAGE_NAME: keys, headers = _ensure_top10_keys_present(keys, headers)
            return _jsonable_snapshot({
                "status": "success", "page": target_page, "sheet": target_page,
                "route_family": _route_family_for_page(target_page),
                "headers": headers, "display_headers": headers,
                "sheet_headers": headers, "column_headers": headers, "keys": keys,
                "rows": [], "row_objects": [], "items": [], "data": [],
                "rows_matrix": [], "count": 0,
        ...
        if operation == "sheet_rows" and target_page != TOP10_PAGE_NAME:
            direct_result  = await _call_engine_sheet_rows(engine, page=target_page,
                                                           limit=limit+offset, mode=mode, payload=criteria)
            direct_payload = _normalize_engine_page_payload(direct_result, page=target_page)
            direct_meta    = dict(direct_payload.get("meta") if isinstance(direct_payload.get("meta"), dict) else {})
            direct_payload["meta"] = {**direct_meta, **base_meta,
            ...
            if is_source_page and (direct_meaningful or direct_status not in {"error","failed"}):
                if offset and isinstance(direct_payload.get("rows"), list):
                    ...
                    direct_payload.update({"rows": rows, "row_objects": rows, "items": rows, "data": rows,
                                           "rows_matrix": _rows_to_matrix(rows, keys) if keys else [],
                                           "count": len(rows)})
                return _jsonable_snapshot(direct_payload)

            if target_page in {INSIGHTS_PAGE_NAME, DATA_DICTIONARY_PAGE_NAME,
                                "AI_Opportunity_Report", "Advisor_Criteria", "KSA_TADAWUL"}:
                base_meta.update({"direct_page_empty_or_weak": not direct_meaningful,
                                   "direct_page_status": direct_status, "direct_page_count": direct_count})
        ...
        if target_page == TOP10_PAGE_NAME or operation in {"recommendations","run"}:
            builder_result = await _build_top10_via_builder(request=request, engine=engine,
                                                            criteria=criteria, limit=limit, offset=offset, mode=mode)
            if isinstance(builder_result, dict):
                ...
                if builder_rows:
                    for idx, row in enumerate(builder_rows, start=1+offset):
                        ...
                        row.setdefault("criteria_snapshot", _json_compact(criteria))
                    bm = dict(base_meta)
                    bm.update(builder_result.get("meta") if isinstance(builder_result.get("meta"), dict) else {})
                    bm["recommendation_count"] = len(builder_rows)
                    if operation == "sheet_rows":
                        return _jsonable_snapshot(await _build_projected_sheet_payload(TOP10_PAGE_NAME, builder_rows, meta=bm))
                    return _jsonable_snapshot(_build_recommendations_payload(builder_rows, meta=bm))
                if _has_meaningful_tabular_content(builder_result):
                    po = _normalize_engine_page_payload(builder_result, page=TOP10_PAGE_NAME)
                    pm = dict(po.get("meta") if isinstance(po.get("meta"), dict) else {})
                    po["meta"] = {**base_meta, **pm}
                    if operation == "sheet_rows": return _jsonable_snapshot(po)
                if _s(builder_result.get("detail")):
                    base_meta["top10_builder_detail"] = _s(builder_result.get("detail"))

        candidate_rows, collect_meta = await _collect_candidate_rows(engine=engine, criteria=criteria, mode=mode)
        base_meta.update(collect_meta)

        if not candidate_rows:
            if operation == "sheet_rows" and target_page == "Advisor_Criteria":
                return _jsonable_snapshot(await _build_criteria_sheet_payload(
                    criteria, meta={**base_meta, "dispatch": "criteria_fallback"}))
            if operation == "sheet_rows" and target_page == DATA_DICTIONARY_PAGE_NAME:
                return _jsonable_snapshot(await _build_empty_schema_payload(
                    DATA_DICTIONARY_PAGE_NAME,
                    meta={**base_meta, "dispatch": "data_dictionary_schema_fallback"},
                    detail="Data_Dictionary schema fallback built by advisor engine"))
            return _jsonable_snapshot(_make_error_payload(
                "No candidate rows found for advisor execution", request=request,
                page=(target_page if operation=="sheet_rows" else TOP10_PAGE_NAME),
                operation=operation, extra_meta=base_meta))

        recommendations = _score_and_rank_rows(candidate_rows, criteria=criteria, limit=limit, offset=offset)
        meta: Dict[str, Any] = {**base_meta, "dispatch": "advisor_scoring",
                                 "recommendation_count": len(recommendations)}

        if operation == "sheet_rows":
            if target_page == TOP10_PAGE_NAME:
                return _jsonable_snapshot(await _build_projected_sheet_payload(TOP10_PAGE_NAME, recommendations, meta=meta))
            if target_page == INSIGHTS_PAGE_NAME:
                return _jsonable_snapshot(await _build_insights_sheet_payload(
                    recommendations, criteria=criteria, meta={**meta, "dispatch": "insights_fallback"}))
            if target_page == "AI_Opportunity_Report":
                return _jsonable_snapshot(await _build_projected_sheet_payload(
                    "AI_Opportunity_Report", recommendations,
            ...
            if target_page == "Advisor_Criteria":
                return _jsonable_snapshot(await _build_criteria_sheet_payload(
                    criteria, meta={**meta, "dispatch": "criteria_fallback"}))
            if target_page == "KSA_TADAWUL":
                return _jsonable_snapshot(await _build_projected_sheet_payload(
                    "KSA_TADAWUL", _filter_ksa_rows(candidate_rows),
        ...
        return _jsonable_snapshot(_build_recommendations_payload(recommendations, meta=meta))

    except Exception as e:
        logger.exception("Investment advisor engine execution failed: %s", e)
        return _jsonable_snapshot(_make_error_payload(
            f"{type(e).__name__}: {e}", request=request, page=target_page,
            operation=operation, extra_meta={"dispatch": "engine_exception"}))
...
def _execute_engine_sync(*args: Any, **kwargs: Any) -> Dict[str, Any]:
    try:
        ...
        return _make_error_payload(f"{type(e).__name__}: {e}", request=request, page=page,
                                   operation="run", extra_meta={"dispatch": "sync_wrapper_exception"})
...
class InvestmentAdvisorEngine:
    def run_investment_advisor_engine(self, *a: Any, **k: Any) -> Dict[str, Any]: return _execute_engine_sync(*a, **k)
    def run_investment_advisor(self, *a: Any, **k: Any) -> Dict[str, Any]:        return _execute_engine_sync(*a, **k)
    def run_advisor(self, *a: Any, **k: Any) -> Dict[str, Any]:                   return _execute_engine_sync(*a, **k)
    def execute_investment_advisor(self, *a: Any, **k: Any) -> Dict[str, Any]:    return _execute_engine_sync(*a, **k)
    def execute_advisor(self, *a: Any, **k: Any) -> Dict[str, Any]:               return _execute_engine_sync(*a, **k)
    def recommend(self, *a: Any, **k: Any) -> Dict[str, Any]:                     return _execute_engine_sync(*a, **k)
    def recommend_investments(self, *a: Any, **k: Any) -> Dict[str, Any]:         return _execute_engine_sync(*a, **k)
    def get_recommendations(self, *a: Any, **k: Any) -> Dict[str, Any]:           return _execute_engine_sync(*a, **k)
    def build_recommendations(self, *a: Any, **k: Any) -> Dict[str, Any]:         return _execute_engine_sync(*a, **k)
    def __call__(self, *a: Any, **k: Any) -> Dict[str, Any]:                      return _execute_engine_sync(*a, **k)
...
def create_investment_advisor(*a: Any, **k: Any) -> InvestmentAdvisorEngine: return _SINGLETON_ENGINE
def get_investment_advisor(*a: Any, **k: Any)    -> InvestmentAdvisorEngine: return _SINGLETON_ENGINE
def build_investment_advisor(*a: Any, **k: Any)  -> InvestmentAdvisorEngine: return _SINGLETON_ENGINE
def create_advisor(*a: Any, **k: Any)            -> InvestmentAdvisorEngine: return _SINGLETON_ENGINE
def get_advisor(*a: Any, **k: Any)               -> InvestmentAdvisorEngine: return _SINGLETON_ENGINE
def build_advisor(*a: Any, **k: Any)             -> InvestmentAdvisorEngine: return _SINGLETON_ENGINE
...
class _EngineAdapter:
    ...
    def warm_cache(self, *a: Any, **k: Any) -> Dict[str, Any]:
        return {"status": "ok", "cache_strategy": self.cache_strategy, "cache_ttl": self.cache_ttl, "warmed": False}
    def warm_snapshots(self, *a: Any, **k: Any) -> Dict[str, Any]: return self.warm_cache()
    def preload_snapshots(self, *a: Any, **k: Any) -> Dict[str, Any]: return self.warm_cache()
    def build_snapshot_cache(self, *a: Any, **k: Any) -> Dict[str, Any]: return self.warm_cache()
    def __getattr__(self, name: str) -> Any: return getattr(self.engine, name)

def create_engine_adapter(engine: Any, cache_strategy: str = "memory", cache_ttl: int = 600, **k: Any) -> _EngineAdapter:
...
# =============================================================================
def run_investment_advisor_engine(*a: Any, **k: Any) -> Dict[str, Any]: return _execute_engine_sync(*a, **k)
def run_investment_advisor(*a: Any, **k: Any)        -> Dict[str, Any]: return _execute_engine_sync(*a, **k)
def run_advisor(*a: Any, **k: Any)                   -> Dict[str, Any]: return _execute_engine_sync(*a, **k)
def execute_investment_advisor(*a: Any, **k: Any)    -> Dict[str, Any]: return _execute_engine_sync(*a, **k)
def execute_advisor(*a: Any, **k: Any)               -> Dict[str, Any]: return _execute_engine_sync(*a, **k)
def recommend(*a: Any, **k: Any)                     -> Dict[str, Any]: return _execute_engine_sync(*a, **k)
def recommend_investments(*a: Any, **k: Any)         -> Dict[str, Any]: return _execute_engine_sync(*a, **k)
def get_recommendations(*a: Any, **k: Any)           -> Dict[str, Any]: return _execute_engine_sync(*a, **k)
def build_recommendations(*a: Any, **k: Any)         -> Dict[str, Any]: return _execute_engine_sync(*a, **k)
...
__all__ = [
    ...
    "advisor_service", "investment_advisor_service",
    "advisor_runner", "investment_advisor_runner",
    "advisor", "investment_advisor",
    "create_investment_advisor", "get_investment_advisor", "build_investment_advisor",
    "create_advisor", "get_advisor", "build_advisor",
    "create_engine_adapter",
    "run_investment_advisor_engine", "run_investment_advisor", "run_advisor",
    "execute_investment_advisor", "execute_advisor",
    "recommend", "recommend_investments", "get_recommendations", "build_recommendations",
]
