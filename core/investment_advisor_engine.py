...
================================================================================
INVESTMENT ADVISOR ENGINE — v3.6.0
================================================================================
COMPATIBILITY-FIRST • ROUTE-TOLERANT • ENGINE-AWARE • SHEET-ROWS SAFE
TOP10-BUILDER FIRST • INSIGHTS-BUILDER AWARE • RECOMMENDATION-NORMALIZED
NO-IMPORT-TIME-NETWORK • JSON-SAFE • FAIL-SOFT

Why this revision
-----------------
- FIX: centralizes recommendation normalization so this engine no longer emits a
       drifting mix of "Strong Buy" / "Avoid" / uppercase variants.
- FIX: prefers shared normalization from core.scoring when available, with a
       safe local fallback for Reduce/Sell/Avoid/Accumulate variants.
- FIX: keeps explicit non-Top10 sheet-rows requests pass-through safe while
       still allowing fallback builders when direct payloads are weak/empty.
- FIX: preserves direct symbol order and deterministic ranking behavior.
- FIX: supports row_objects / rows / nested payload / matrix-only shapes.
- ENHANCE: prefers the dedicated Insights builder for Insights_Analysis.
- SAFE: never performs network work at import time.
- SAFE: exports the same compatibility surface expected by older routes.
================================================================================
"""
...
INVESTMENT_ADVISOR_ENGINE_VERSION = "3.6.0"
...
_SPECIAL_SCHEMA_DEFAULTS: Dict[str, Tuple[List[str], List[str]]] = {
    ...
    INSIGHTS_PAGE_NAME: (
        ["Section", "Item", "Metric", "Value", "Notes", "Source Page", "Sort Order"],
        ["section", "item", "metric", "value", "notes", "source_page", "sort_order"],
    ),
    DATA_DICTIONARY_PAGE_NAME: (
        ["Section", "Field", "Header", "Description", "Type", "Format", "Example", "Source", "Notes"],
        ["section", "field", "header", "description", "type", "format", "example", "source", "notes"],
    ),
}
...
# -----------------------------------------------------------------------------
try:
    from core.scoring import normalize_recommendation_label as _normalize_recommendation_label_external  # type: ignore
except Exception:
    _normalize_recommendation_label_external = None  # type: ignore

_LOCAL_RECOMMENDATION_MAP: Dict[str, str] = {
...
def _normalize_recommendation_label(value: Any) -> str:
    raw = _s(value)
    if not raw:
        return ""
    if _normalize_recommendation_label_external is not None:
        try:
            normalized = _normalize_recommendation_label_external(raw)
            if isinstance(normalized, str) and normalized.strip():
                return normalized.strip()
        except Exception:
            pass
    probe = raw.replace("-", "_").replace(" ", "_").strip()
    return _LOCAL_RECOMMENDATION_MAP.get(raw, _LOCAL_RECOMMENDATION_MAP.get(probe, _LOCAL_RECOMMENDATION_MAP.get(probe.upper(), raw)))
...
def _model_to_dict(obj: Any) -> Dict[str, Any]:
    ...
    for meth in ("model_dump", "dict"):
        try:
            ...
            if _callable(fn):
                dumped = fn(mode="python") if meth == "model_dump" else fn()
...
def _extract_columns_from_spec(spec: Any) -> List[Tuple[str, str]]:
    ...
    raw_columns = spec.get("columns") if isinstance(spec, dict) else getattr(spec, "columns", None)
    raw_columns = raw_columns or (spec.get("fields") if isinstance(spec, dict) else getattr(spec, "fields", None))
...
async def _load_schema_defaults(page_name: str = TOP10_PAGE_NAME) -> Tuple[List[str], List[str]]:
    ...
    for mod_name in _SCHEMA_MODULE_CANDIDATES:
        ...
        for fn_name in _SCHEMA_FN_CANDIDATES:
            ...
            if not _callable(fn):
                continue
            attempts = [{"sheet": canonical_page}, {"page": canonical_page}, {"sheet_name": canonical_page}, {"name": canonical_page}]
...
async def _get_engine_from_request(request: Any) -> Any:
    try:
        ...
        if st is not None and getattr(st, "engine", None) is not None:
            return st.engine
        if st is not None and getattr(st, "data_engine", None) is not None:
            return st.data_engine
    except Exception:
        pass

    try:
        from core.engine_resolver import resolve_engine  # type: ignore
        eng = resolve_engine(request=request)
...
async def _extract_context(*args: Any, **kwargs: Any) -> Tuple[Any, Any, Any, Dict[str, Any]]:
    ...
    for key in ("payload", "body", "request_data", "params"):
        ...
        if isinstance(val, dict):
            payload.update({k: v for k, v in val.items() if v is not None})
    criteria = kwargs.get("criteria")
    if isinstance(criteria, dict):
        payload["criteria"] = {**(payload.get("criteria") if isinstance(payload.get("criteria"), dict) else {}), **criteria}
    for arg in args:
        if request is None and hasattr(arg, "headers") and hasattr(arg, "url"):
            request = arg
            continue
        if engine is None and any(_callable(getattr(arg, name, None)) for name in _ENGINE_SHEET_METHOD_CANDIDATES):
            engine = arg
            continue
        if isinstance(arg, dict):
            if not payload:
                ...
                payload["criteria"] = {**(payload.get("criteria") if isinstance(payload.get("criteria"), dict) else {}), **arg}
...
def _flatten_criteria(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in (
        ...
        "mode", "advisor_data_mode", "debug", "schema_only", "headers_only",
        "include_matrix", "format", "invest_amount", "allocation_strategy",
    ):
    ...
    for key in ("page", "sheet", "sheet_name"):
        if key in out:
            ...
            if normalized:
                out[key] = normalized
    normalized_pages = []
    for item in (_normalize_list(out.get("pages_selected")) or _normalize_list(out.get("pages")) or _normalize_list(out.get("selected_pages")) or _normalize_list(out.get("sources"))):
...
def _normalize_direct_symbols(criteria: Dict[str, Any]) -> List[str]:
    return _normalize_list(criteria.get("direct_symbols")) or _normalize_list(criteria.get("symbols")) or _normalize_list(criteria.get("tickers")) or _normalize_list(criteria.get("symbol"))
...
def _infer_operation(request: Any, criteria: Dict[str, Any]) -> str:
    ...
    if path.endswith("/run") or path.endswith("/advisor") or path.endswith("/investment-advisor"):
        return "run"
    if _s(criteria.get("format")).lower() == "rows":
        return "sheet_rows"
    return "run"
...
# Engine page access
# -----------------------------------------------------------------------------
async def _call_engine_sheet_rows(engine: Any, *, page: str, limit: int, mode: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    page = _normalize_page_name(page) or page
    if engine is None:
        return _make_error_payload("Data engine unavailable", page=page, operation="sheet_rows", extra_meta={"dispatch": "engine.missing"})
    fn = None
    fn_name = ""
    for name in _ENGINE_SHEET_METHOD_CANDIDATES:
        ...
        if _callable(candidate):
            ...
            break
    if fn is None:
        return _make_error_payload("Engine sheet-row handler not found", page=page, operation="sheet_rows", extra_meta={"dispatch": "engine.sheet_rows_missing"})
    attempts = [
    ...
    for kwargs in attempts:
        try:
            ...
            if _is_signature_mismatch_typeerror(exc):
                continue
            return _make_error_payload(f"Engine page call failed: {type(exc).__name__}: {exc}", page=page, operation="sheet_rows", extra_meta={"dispatch": f"engine.{fn_name}"})
        except Exception as e:
            return _make_error_payload(f"Engine page call failed: {type(e).__name__}: {e}", page=page, operation="sheet_rows", extra_meta={"dispatch": f"engine.{fn_name}"})
    if out is None:
        for args in ((page, limit), (page,), tuple()):
            try:
                ...
                return _make_error_payload(f"Engine page call failed: {type(e).__name__}: {e}", page=page, operation="sheet_rows", extra_meta={"dispatch": f"engine.{fn_name}"})
    result = dict(out) if isinstance(out, dict) else _model_to_dict(out)
...
def _normalize_engine_page_payload(result: Dict[str, Any], *, page: str) -> Dict[str, Any]:
    ...
    if matrix is None:
        matrix = []
    for row in rows:
        if isinstance(row, dict) and not _is_blank(row.get("recommendation")):
...
def _symbol_of(row: Dict[str, Any]) -> str:
    return _s(row.get("symbol")) or _s(row.get("ticker")) or _s(row.get("code")) or _s(row.get("symbol_code")) or _s(row.get("instrument"))

def _name_of(row: Dict[str, Any]) -> str:
    return _s(row.get("name")) or _s(row.get("company_name")) or _s(row.get("description")) or _s(row.get("short_name"))

# -----------------------------------------------------------------------------
# Candidate collection
# -----------------------------------------------------------------------------
async def _collect_candidate_rows(*, engine: Any, criteria: Dict[str, Any], mode: str) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    for page in pages:
        page_result = await _call_engine_sheet_rows(engine, page=page, limit=fetch_limit, mode=mode, payload=criteria)
...
def _canonical_selection_reason(row: Dict[str, Any]) -> Optional[str]:
    recommendation = _normalize_recommendation_label(row.get("recommendation"))
    confidence_bucket = _s(row.get("confidence_bucket"))
    risk_bucket = _s(row.get("risk_bucket"))
    score_parts: List[str] = []
    for label, key in (("overall", "overall_score"), ("opportunity", "opportunity_score"), ("value", "value_score"), ("quality", "quality_score"), ("momentum", "momentum_score"), ("growth", "growth_score")):
        val = row.get(key)
...
def _score_and_rank_rows(rows: List[Dict[str, Any]], *, criteria: Dict[str, Any], limit: int, offset: int = 0) -> List[Dict[str, Any]]:
    ...
    for row in rows:
        ...
        r["recommendation"] = _normalize_recommendation_label(r.get("recommendation"))
        ...
        scored.append(r)
    scored.sort(key=lambda x: (float(x.get("overall_score") or 0.0), float(x.get("expected_roi_3m") or 0.0), float(x.get("forecast_confidence") or 0.0)), reverse=True)
...
def _normalize_top10_builder_rows(value: Any) -> List[Dict[str, Any]]:
    ...
    if isinstance(value, list):
        ...
        if value and isinstance(value[0], (list, tuple)):
            ...
            return _matrix_to_rows([list(x) for x in value], list(fallback_keys)[:len(value[0])])
    return []
...
async def _resolve_top10_builder(request: Any, engine: Any) -> Tuple[Any, str]:
    try:
        ...
        st = getattr(app, "state", None)
        if st is not None:
            for name in ("top10_selector", "top10_builder", "investment_top10_selector", "investment_top10_builder"):
                obj = getattr(st, name, None)
                if obj is not None:
                    for fn_name in _TOP10_BUILDER_FN_CANDIDATES:
                        fn = getattr(obj, fn_name, None)
                        if _callable(fn):
                            return fn, f"app.state.{type(obj).__name__}.{fn_name}"
                    if _callable(obj):
                        return obj, f"app.state.{type(obj).__name__}"
    except Exception:
        pass
...
async def _build_top10_via_builder(*, request: Any, engine: Any, criteria: Dict[str, Any], limit: int, offset: int, mode: str) -> Dict[str, Any]:
    builder, dispatch = await _resolve_top10_builder(request, engine)
    if builder is None:
        return {}
    attempts = [
        {"request": request, "engine": engine, "criteria": criteria, "limit": limit, "offset": offset, "top_n": limit + offset, "mode": mode, "page": TOP10_PAGE_NAME},
        {"request": request, "engine": engine, "criteria": criteria, "limit": limit + offset, "page": TOP10_PAGE_NAME},
        {"request": request, "engine": engine, "criteria": criteria, "top_n": limit + offset, "page": TOP10_PAGE_NAME},
        {"request": request, "engine": engine, "body": criteria, "limit": limit + offset, "page": TOP10_PAGE_NAME},
        {"request": request, "engine": engine, "payload": criteria, "limit": limit + offset, "page": TOP10_PAGE_NAME},
        {"request": request, "engine": engine, "symbols": _normalize_direct_symbols(criteria), "criteria": criteria, "limit": limit + offset},
    ...
    for kwargs in attempts:
        try:
            ...
            if isinstance(out, dict) and _has_meaningful_tabular_content(out):
            ...
            last_error = f"{type(exc).__name__}: {exc}"
    if last_error:
        return {"status": "error", "detail": f"Top10 builder failed: {last_error}", "meta": {"dispatch": dispatch, "builder_used": True}}
    return {}

# -----------------------------------------------------------------------------
# Output builders
# -----------------------------------------------------------------------------
async def _build_projected_sheet_payload(page_name: str, rows: List[Dict[str, Any]], *, meta: Dict[str, Any], detail: str = "") -> Dict[str, Any]:
    ...
    if keys:
        for row in rows:
            item = {k: row.get(k, None) for k in keys}
            if not _is_blank(item.get("recommendation")):
                item["recommendation"] = _normalize_recommendation_label(item.get("recommendation"))
...
def _build_recommendations_payload(recommendations: List[Dict[str, Any]], *, meta: Dict[str, Any]) -> Dict[str, Any]:
    normalized = []
    ...
    return {
        ...
        "rows": normalized,
        "row_objects": normalized,
        "data": normalized,
        "items": normalized,
        "count": len(normalized),
        "detail": "" if normalized else "No advisor recommendations produced",
        "meta": meta,
    }
...
async def _build_criteria_sheet_payload(criteria: Dict[str, Any], *, meta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return await _build_projected_sheet_payload("Advisor_Criteria", [row], meta=meta, detail="Criteria fallback payload built by advisor engine")
...
async def _build_insights_sheet_payload(recommendations: List[Dict[str, Any]], *, engine: Any, criteria: Dict[str, Any], meta: Dict[str, Any]) -> Dict[str, Any]:
    try:
        ...
        payload = await build_insights_analysis_rows(
            engine=engine,
            criteria=criteria,
            symbols=[_symbol_of(r) for r in recommendations if _symbol_of(r)],
            mode=_normalize_mode(criteria),
            include_top10_section=True,
            include_portfolio_kpis=True,
        )
        if isinstance(payload, dict) and _has_meaningful_tabular_content(payload):
            normalized = _normalize_engine_page_payload(payload, page=INSIGHTS_PAGE_NAME)
            normalized_meta = normalized.get("meta") if isinstance(normalized.get("meta"), dict) else {}
            normalized["meta"] = {**meta, **normalized_meta, "dispatch": normalized_meta.get("dispatch") or "insights_builder"}
            return normalized
    except Exception:
        pass
    ...
    avg_conf = 0.0
    if recommendations:
        avg_roi = sum(float(r.get("expected_roi_3m") or 0.0) for r in recommendations) / len(recommendations)
        avg_conf = sum(float(r.get("forecast_confidence") or 0.0) for r in recommendations) / len(recommendations)
    ...
    rows.extend([
        {"section": "Summary", "item": "Coverage", "metric": "Candidate Count", "value": len(recommendations), "notes": f"Sources={','.join(_normalize_sources(criteria))}", "source_page": "advisor_engine", "sort_order": 1},
        {"section": "Summary", "item": "Expected Return", "metric": "Average ROI 3M", "value": round(avg_roi, 6), "notes": "Average across ranked recommendations", "source_page": "advisor_engine", "sort_order": 2},
        {"section": "Summary", "item": "Confidence", "metric": "Average Forecast Confidence", "value": round(avg_conf, 6), "notes": "Average confidence across ranked recommendations", "source_page": "advisor_engine", "sort_order": 3},
        {"section": "Risk", "item": "Distribution", "metric": "Low Risk Count", "value": low_risk, "notes": "Ranked recommendation distribution", "source_page": "advisor_engine", "sort_order": 4},
        {"section": "Risk", "item": "Distribution", "metric": "Moderate Risk Count", "value": med_risk, "notes": "Ranked recommendation distribution", "source_page": "advisor_engine", "sort_order": 5},
        {"section": "Risk", "item": "Distribution", "metric": "High Risk Count", "value": high_risk, "notes": "Ranked recommendation distribution", "source_page": "advisor_engine", "sort_order": 6},
    ])
    for idx, row in enumerate(recommendations[:3], start=1):
        rows.append({
            ...
            "value": _normalize_recommendation_label(row.get("recommendation")) or "n/a",
            "notes": _s(row.get("selection_reason")) or f"ROI3M={round(float(row.get('expected_roi_3m') or 0.0) * 100.0, 2)}%",
            "source_page": _s(row.get("source_page")) or "advisor_engine",
            "sort_order": 10 + idx,
        })
    return await _build_projected_sheet_payload(INSIGHTS_PAGE_NAME, rows, meta=meta, detail="Insights fallback built from advisor scoring")
...
async def _execute_engine_async(*args: Any, **kwargs: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if operation == "sheet_rows" and target_page != TOP10_PAGE_NAME:
            direct_result = await _call_engine_sheet_rows(engine, page=target_page, limit=limit + offset, mode=mode, payload=criteria)
            ...
            direct_payload["meta"] = {**direct_meta, **base_meta, "dispatch": direct_meta.get("dispatch") or "engine_direct_page"}
        ...
        if target_page == TOP10_PAGE_NAME or operation in {"recommendations", "run"}:
            builder_result = await _build_top10_via_builder(request=request, engine=engine, criteria=criteria, limit=limit, offset=offset, mode=mode)
            if isinstance(builder_result, dict):
                ...
                if builder_rows:
                    ...
                    if operation == "sheet_rows":
                        return _jsonable_snapshot(await _build_projected_sheet_payload(TOP10_PAGE_NAME, builder_rows, meta=builder_meta))
                    return _jsonable_snapshot(_build_recommendations_payload(builder_rows, meta=builder_meta))
                if _has_meaningful_tabular_content(builder_result):
                    ...
                    payload_meta = payload.get("meta") if isinstance(payload.get("meta"), dict) else {}
                    payload["meta"] = {**base_meta, **payload_meta}
                    if operation == "sheet_rows":
        ...
        if not candidate_rows:
            if operation == "sheet_rows" and target_page == "Advisor_Criteria":
                return _jsonable_snapshot(await _build_criteria_sheet_payload(criteria, meta={**base_meta, "dispatch": "criteria_fallback"}))
            if operation == "sheet_rows" and target_page == DATA_DICTIONARY_PAGE_NAME:
                return _jsonable_snapshot(await _build_empty_schema_payload(DATA_DICTIONARY_PAGE_NAME, meta={**base_meta, "dispatch": "data_dictionary_schema_fallback"}, detail="Data_Dictionary schema fallback built by advisor engine"))
            return _jsonable_snapshot(_make_error_payload("No candidate rows found for advisor execution", request=request, page=(target_page if operation == "sheet_rows" else TOP10_PAGE_NAME), operation=operation, extra_meta=base_meta))
        ...
        if operation == "sheet_rows":
            ...
            if target_page == INSIGHTS_PAGE_NAME:
                return _jsonable_snapshot(await _build_insights_sheet_payload(recommendations, engine=engine, criteria=criteria, meta={**meta, "dispatch": "insights_fallback"}))
            if target_page == "AI_Opportunity_Report":
                return _jsonable_snapshot(await _build_projected_sheet_payload("AI_Opportunity_Report", recommendations, meta={**meta, "dispatch": "ai_opportunity_fallback"}, detail="AI Opportunity fallback built from advisor scoring"))
            if target_page == "Advisor_Criteria":
                return _jsonable_snapshot(await _build_criteria_sheet_payload(criteria, meta={**meta, "dispatch": "criteria_fallback"}))
            if target_page == "KSA_TADAWUL":
                return _jsonable_snapshot(await _build_projected_sheet_payload("KSA_TADAWUL", _filter_ksa_rows(candidate_rows), meta={**meta, "dispatch": "ksa_fallback"}, detail="KSA_TADAWUL fallback built from advisor source rows"))
            if target_page == DATA_DICTIONARY_PAGE_NAME:
                return _jsonable_snapshot(await _build_empty_schema_payload(DATA_DICTIONARY_PAGE_NAME, meta={**meta, "dispatch": "data_dictionary_schema_fallback"}, detail="Data_Dictionary schema fallback built by advisor engine"))
            return _jsonable_snapshot(await _build_projected_sheet_payload(target_page, recommendations, meta={**meta, "dispatch": "generic_sheet_fallback"}, detail=f"{target_page} fallback built from advisor scoring"))
        ...
        return _jsonable_snapshot(_make_error_payload(f"{type(e).__name__}: {e}", request=request, page=target_page, operation=operation, extra_meta={"dispatch": "engine_exception"}))
...
def _execute_engine_sync(*args: Any, **kwargs: Any) -> Dict[str, Any]:
    try:
        ...
        return _make_error_payload(f"{type(e).__name__}: {e}", request=request, page=page, operation="run", extra_meta={"dispatch": "sync_wrapper_exception"})
