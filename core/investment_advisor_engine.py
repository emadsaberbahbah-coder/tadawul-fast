...
What v3.6.0 revises
-------------------
- FIX CRITICAL: _recommendation_from_scores() returned non-canonical labels
  ("Strong Buy", "Buy", "Hold", "Avoid") that no downstream code recognized.
  "Avoid" was the most damaging: it was never mapped to SELL or REDUCE in the
  display pipeline, appearing verbatim as "Avoid" in Google Sheets.
  Fixed to use canonical 5-value vocabulary: STRONG_BUY/BUY/HOLD/REDUCE/SELL.
- FIX: Incoming rows that already have a recommendation field are now normalized
  through the canonical vocabulary before being used. Prevents non-canonical
  labels from providers ("Accumulate", "Outperform", etc.) persisting in output.
- ENH: Added reco_normalize import with fallback constants so this module stays
  import-safe even if core.reco_normalize is unavailable.
...
- FIX: tolerant calling only skips real signature-mismatch TypeErrors; real
...
# FIX v3.6.0: _recommendation_from_scores() was using non-canonical strings
# ("Strong Buy", "Avoid"). Now uses these constants everywhere so labels are
# always in the 5-value canonical vocabulary expected by Google Sheets display.
# ---------------------------------------------------------------------------
try:
    ...
    def _normalize_reco(x: Any) -> str:  # type: ignore[misc]
        """Minimal fallback when core.reco_normalize is unavailable."""
        _map = {
...
def _recommendation_from_scores(overall_score: float, roi_3m: float, confidence: float, risk_score: float) -> str:
    """
    Derive a canonical recommendation from computed scores.
    ...
    "Avoid" → SELL (overall < 45 means the asset failed all criteria).
    "Strong Buy" → STRONG_BUY (aligned with scoring.py v2.3.0 threshold).
    """
    # STRONG_BUY: high conviction — all signals green, risk under control
    if overall_score >= 75.0 and roi_3m >= 0.05 and confidence >= 0.70 and risk_score <= 60.0:
        return RECO_STRONG_BUY
    # BUY: positive signals, reasonable confidence
    if overall_score >= 60.0 and roi_3m > 0.0 and confidence >= 0.55:
        return RECO_BUY
    # HOLD: borderline — acceptable score but no strong buy signal
    if overall_score >= 45.0:
        return RECO_HOLD
    # REDUCE: weak score — suggest lightening rather than hard exit
    if overall_score >= 30.0:
        return RECO_REDUCE
    # SELL: failed all criteria — explicit exit signal
    return RECO_SELL
...
def _score_and_rank_rows(
    ...
    *,
    ...
    for row in rows:
        ...
        # Previously only set if blank — non-canonical values from providers
        # ("Accumulate", "Outperform", "Watch") persisted verbatim into output.
        existing_reco = _s(r.get("recommendation"))
        if existing_reco:
            normalized = _normalize_reco(existing_reco)
    ...
    rows.append(
        {
            ...
            "metric": "Average ROI 3M",
            "value": round(avg_roi, 6),
    ...
    rows.append(
        {
            "section": "Summary",
            "item": "Confidence",
            "metric": "Average Forecast Confidence",
            "value": round(avg_conf, 6),
