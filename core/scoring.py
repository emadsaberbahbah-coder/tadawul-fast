...
"""
================================================================================
Scoring Module — v1.1.0 (PHASE 4 / SCHEMA-ALIGNED / SAFE / TOP10-READY)
================================================================================
Purpose:
...
- Startup-safe: no network, no heavy optional ML deps.

v1.1.0 changes (alignment & robustness)
- ✅ Stronger schema alignment for common key variants (week_52_position_pct, change_pct, etc.)
- ✅ Deterministic scoring even with sparse data (more graceful fallbacks)
- ✅ Adds optional: overall_score_raw, overall_penalty_factor (debug helpers)
- ✅ Keeps backward-compatible score keys used by dashboards
================================================================================
"""
...
def _weights_from_settings(settings: Any) -> ScoreWeights:
    ...
    Supported setting names:
      score_w_valuation, score_w_momentum, score_w_quality, score_w_opportunity
...
def _valuation_score(row: Dict[str, Any]) -> Optional[float]:
    ...
    Uses (best effort):
      - upside from fair_value/target/forecast prices vs current price
      - expected ROI (3m/12m) as a direct value proxy
      - PE/PB/EV-EBITDA as weak anchors (lower is better)
    """
    price = _getf(row, "current_price", "price", "last_price")
    if price is None or price <= 0:
        return None

    fair = _getf(row, "fair_value", "target_price", "forecast_price_3m", "forecast_price_12m", "forecast_price_1m")
    upside = None
...
def _momentum_score(row: Dict[str, Any]) -> Optional[float]:
    ...
    Uses:
      - percent_change / change_pct (fraction) if available
      - RSI 14 (target ~55-65)
...
def _risk_score(row: Dict[str, Any]) -> Optional[float]:
    ...
    if vol is not None:
        v = _clamp((vol - 0.10) / (0.60 - 0.10), 0.0, 1.0)  # 10%..60%
        parts.append((0.50, v))

    if beta is not None:
        b = _clamp((beta - 0.60) / (2.00 - 0.60), 0.0, 1.0)  # 0.6..2.0
        parts.append((0.30, b))

    if dd is not None:
        d = _clamp(dd / 0.50, 0.0, 1.0)  # 0..50%
        parts.append((0.20, d))
...
def compute_scores(row: Dict[str, Any], *, settings: Any = None) -> Dict[str, Any]:
    ...
    if base_parts:
        ...
        conf_pen = (1.0 - w.confidence_penalty_strength * ((1.0 - conf01) * 0.80))
