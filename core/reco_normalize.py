...
------------------------------------------------------------
تسوية التوصيات — الإصدار 3.0.0 (مستوى المؤسسات المتقدم)
------------------------------------------------------------
...
- ✅ آمن للاستخدام مع خيوط المعالجة المتعددة (Thread-safe) وبدون اعتمادات خارجية معقدة.
...
if __name__ == "__main__":
    ...
    print(f"أداة تسوية التوصيات الإصدار {VERSION} - اختبار ذاتي")
    print("-" * 50)
    ...
    for inp, expected in test_cases:
        ...
        if result == expected:
            ...
            failed += 1
        print(f"{status:<8} | المُدخل: {str(inp):<20} -> النتيجة: {result:6} (المتوقع: {expected})")
        
    print("-" * 50)
    print(f"النجاح: {passed}، الفشل: {failed}")
