...
(ROOT-SHEET-ROWS OWNER GUARANTEED / SHARED-PIPELINE HARDENED /
 ...
 DATA-DICTIONARY SAFE / SCHEMA-FLAT SHEET-SPEC / TOP10-REHYDRATE / JSON-SAFE / LIMIT+OFFSET CONSISTENT / ASYNC-SAFE)
...
- ✅ FIX: Data_Dictionary follows the same normalized payload contract as the
         other pages.
- ✅ FIX: single-sheet schema endpoints now expose flat top-level `headers` /
         `keys` in addition to the nested `spec` payload, so schema tests can
         read the contract directly.
- ✅ FIX: weak Top10 selector/shared payloads no longer short-circuit the full
         instrument rehydration path when they only contain the 3 Top10 extras.
--------------------------------------------------------------------------------
"""
...
def _top10_payload_needs_rehydrate(payload: Any) -> bool:
    rows = _extract_rows_like(payload)
    if not rows:
        return True

    base_fields = {
        "symbol", "ticker", "name", "current_price", "exchange", "currency",
        "country", "sector", "industry", "recommendation", "overall_score",
        "opportunity_score", "forecast_confidence", "expected_roi_1m",
        "expected_roi_3m", "expected_roi_12m",
    }
    ...
    for row in rows:
        if not isinstance(row, Mapping):
            continue
        if _strip(row.get("symbol") or row.get("ticker")):
            symbol_rows += 1
        if any(row.get(field) not in (None, "", [], {}) for field in base_fields):
            populated += 1

    if populated == 0:
        return True
    if symbol_rows == 0 and _extract_symbol_list_like(payload):
        return True
    return False
...
def _shared_payload_acceptable(payload: Any, page: str) -> bool:
    ...
    if page == "Top_10_Investments":
        return _payload_has_real_rows(payload) and not _top10_payload_needs_rehydrate(payload)
...
async def _call_builder_best_effort(
    *,
    ...
    if allow_symbol_rehydrate and (not _payload_has_real_rows(result) or (page == "Top_10_Investments" and _top10_payload_needs_rehydrate(result))):
        selected_symbols = _extract_symbol_list_like(result)
        if not selected_symbols:
            selected_symbols = []
            for row in _extract_rows_like(result):
                if isinstance(row, Mapping):
                    sym = _normalize_symbol_token(row.get("symbol") or row.get("ticker") or row.get("requested_symbol"))
        ...
        if selected_symbols and engine is not None:
            try:
                ...
                source_rows = _extract_rows_like(result)
                source_by_symbol: Dict[str, Dict[str, Any]] = {}
                for row in source_rows:
                    if isinstance(row, Mapping):
                        sym = _normalize_symbol_token(row.get("symbol") or row.get("ticker") or row.get("requested_symbol"))
                        if sym and sym not in source_by_symbol:
                            source_by_symbol[sym] = dict(row)
                ...
                for idx, sym in enumerate(selected_symbols[:limit], start=offset + 1):
                    ...
                    merged = dict(source_by_symbol.get(sym, {}))
                    merged.update(raw)
                    if not merged.get("symbol") and not merged.get("ticker"):
                        merged["symbol"] = sym
                        merged["ticker"] = sym
                    rr = _normalize_to_schema_keys(schema_keys=schema_keys, schema_headers=schema_headers, raw=merged)
                    if page == "Top_10_Investments":
                        ...
                        if not rr.get("selection_reason") and "selection_reason" in schema_keys:
                            rr["selection_reason"] = _strip(merged.get("selection_reason") or merged.get("reason")) or "Selected by Top10 builder."
                        if not rr.get("criteria_snapshot") and "criteria_snapshot" in schema_keys:
                            rr["criteria_snapshot"] = _strip(merged.get("criteria_snapshot") or merged.get("criteria")) or "{}"
                    rebuilt.append(_project_row(schema_keys, rr))
                result = {"status": "partial", "row_objects": rebuilt, "meta": {"symbols_rehydrated": True}}
