...
def _protected_prefixes() -> List[str]:
    raw = _parse_csv(
        _env_str(
            ...
            "/v1/advisor,/v1/advanced,/v1/analysis,/v1/schema,/schema,"
            "/v1/enriched,/v1/enriched_quote,/v1/enriched-quote,"
            "/quote,/quotes,/sheet-rows",
        )
    )
...
def _canonical_owner_map() -> Dict[str, str]:
    ...
    Value is the logical RouterSpec.key (not the module name).
    """
    return {
        "/v1/advisor":        "advisor",
...
def get_mount_plan() -> List[str]:
    resolved, _missing = _resolve_catalog(_default_catalog())
...
def _append_router_routes_filtered(
    app: Any, router_obj: Any, module_name: str
) -> Dict[str, Any]:
    ...
    for route in list(getattr(router_obj, "routes", []) or []):
        ...
        if overlap:
            ...
            logger.warning(
                ...
                module_name, path, sorted(list(overlap)),
            )
            continue
        ...
        if callable(mount_fn):
            ...
            if not added_routes:
                return True, None, "mount_fn", {
                    "route_count": 0, "claimed_prefixes": [],
                    "policy_filtered_routes": [], "duplicate_skips": 0,
                    "partial_duplicate_skips": 0, "filtered_out": 0,
                }

            disallowed_routes = [
                r for r in added_routes
                if not _path_allowed_for_module(module_name, str(getattr(r, "path", "") or ""))
            ]
...
def mount_all_routers(app: Any) -> Dict[str, Any]:
    ...
    logger.info(
        "Routes mount summary: mounted=%s duplicate_skips=%s partial_duplicate_skips=%s "
        "conflict_skips=%s policy_skips=%s missing=%s failed=%s strict=%s",
        len(mounted), len(duplicate_skips), len(partial_duplicate_skips),
        len(conflict_skips), len(policy_skips), len(missing), failed_count, strict,
    )
