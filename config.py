...
"""
TADAWUL FAST BRIDGE – MAIN CONFIG (v2.6.0) – PROD SAFE / NO HEAVY IMPORTS
...
def _maybe_decode_b64_json(s: str) -> Optional[str]:
    ...
    return raw  # return original if not decodable
...
class Settings:
    ...
    google_sheets_credentials: Optional[str] = None  # JSON or base64 JSON
    ...
    def from_env() -> "Settings":
        ...
        gsc = _maybe_decode_b64_json(_strip(os.getenv("GOOGLE_SHEETS_CREDENTIALS"))) or None
        ...
        enable_yahoo_raw = _strip(os.getenv("ENABLE_YAHOO"))
        enable_yahoo = True
        if enable_yahoo_raw:
            enable_yahoo = enable_yahoo_raw.strip().lower() not in ("0", "false", "no", "off")
...
def mask_settings_dict() -> Dict[str, Any]:
    ...
    # Never return full service-account JSON (presence only)
    if s.google_sheets_credentials:
        d["google_sheets_credentials"] = "***present***"
    else:
        d["google_sheets_credentials"] = None

    # keep path but do not expose if you prefer; for now mask it lightly
    if s.google_application_credentials:
        d["google_application_credentials"] = "***present***"
    else:
        d["google_application_credentials"] = None
