...
## Addendum 2 — v6.56.2 HOTFIX (supersedes the v6.56.1 script file) — SHA256 `0190c0353bce6808…` · 10,914 lines
**Defect (mine, run 33640922781):** v6.56.1 inserted the ENV-ECHO `try/except` between the self-test `if` and its `else`; the `else` bound to the `try`, so `::error::[SELFTEST] … a guard fixture failed; FW-4 quarantine disabled` printed on every leg after a *successful* echo (3 false error annotations). FW-4 itself stayed ON (it quarantined ZL=F); the print is log-only. `py_compile` cannot catch a valid `try/except/else`; the deterministic gate does not exercise the self-test log path; my smoke test called `_env_echo_line()` directly. **Fix:** echo moved above the if/else, which is restored verbatim. **New harness H-SELFTEST:** runs the real `_idfw_selftest_()` with stdout captured and asserts it returns True with no `::error::`. Deterministic gate 84/84.
**Inference error (mine, runs #3835/#3849):** I reported "fill guard OFF — no `[OHLC-FILLGUARD]` line". That line is a `_Run_Log` (sheet) append only and never reaches the local log file, so its absence proved nothing. ENV-ECHO on run 33640922781 is the first valid evidence: `fill_guard=True('1') fill_mode=enforce('enforce')` — the guard Variables were reaching the job. The only real gap is `rb_tol_rows='0' rb_tol_pct='0'`: the two tolerance Variables are unset.
**Register correction:** P-55 root cause = tolerance Variables unset (operator) + invalid inference (Claude). Not a mapping problem.
