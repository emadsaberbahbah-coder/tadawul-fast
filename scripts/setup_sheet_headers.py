...
def resolve_percent_mode(cli_value: Optional[str]) -> PercentMode:
    """
    FIX v6.3.0: Logs clearly what mode is in use + warns if TFB_PERCENT_MODE
    is unset (ops visibility, avoids silent misconfiguration).
    """
...
def infer_field_type(header: str) -> FieldType:
    ...
    # Previously ROE, ROA, Volatility, Drawdown, ROI, Payout, Beta, Sharpe, VaR
    # were all falling through to STRING with no number format.
    if (
        "%" in h
        or "percent" in h
        or "yield" in h
...
DEFAULT_TABS = [
    # 5 source pages — live enriched data from providers
    "Market_Leaders",
    "Global_Markets",
    "Mutual_Funds",
    "Commodities_FX",
    "My_Portfolio",
    # 3 derived output pages — analysis/advisor/data-dictionary builders
    "Insights_Analysis",
    "Top_10_Investments",
    "Data_Dictionary",
