    ...
    *,
    ...
    async def _do_batch(batch: List[str]) -> None:
        async with sem:
            ...
            # take down its 8-symbol cohort. This is the fix for the
            # group-2 (BBVA, PNC, GSK, FITB, MRK, NVS, AER, TTE) scenario
            # where one batch's failure produced 8 rows of empty defaults.
            logger.warning(f"Batch failed ({msg}); falling back to per-symbol for {len(batch)} symbols")
            per_results = await asyncio.gather(
                *[
                    _refresh_symbol(engine, s, backoff, timeout_sec=timeout_sec, refresh=refresh)
                    for s in batch
                ],
