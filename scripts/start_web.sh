...
===========================================================
TADAWUL FAST BRIDGE – ENTERPRISE WEB LAUNCHER (v6.1.1)
===========================================================
QUANTUM EDITION | AUTO-SCALING | HIGH-PERFORMANCE ASGI | NON-BLOCKING

What’s new in v6.1.1 (patch)
- ✅ Fixed OpenTelemetry TraceContext: start_as_current_span() is a context manager (no .set_attributes / no double-end)
- ✅ Fixed DummyTracer/DummySpan compatibility (set_attribute(s), end, context manager)
- ✅ Fixed Gunicorn BaseApplication usage: now loads the actual ASGI app object (not a "main:app" string)
- ✅ Gunicorn worker class auto-detect: prefers `uvicorn_worker.UvicornWorker` when installed
...
# Prefer uvicorn-worker package when installed (uvicorn-worker >= 0.2)
try:
    import uvicorn_worker  # type: ignore

    UVICORN_WORKER_AVAILABLE = True
except ImportError:
    UVICORN_WORKER_AVAILABLE = False
...
class TraceContext:
    ...
    - enter it to get the Span (if any)
    - exit the context manager to end the span
    """
    ...
    def __enter__(self):
        if self.tracer:
            ...
            # _cm is a context manager; entering yields span (OTel) or DummySpan
            self.span = self._cm.__enter__()
            if self.attributes and self.span:
                # OpenTelemetry Span API uses set_attribute (singular)
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self._cm and self.span:
            try:
                if exc_val and OTEL_AVAILABLE:
                    self.span.record_exception(exc_val)
                    self.span.set_status(Status(StatusCode.ERROR, str(exc_val)))
            except Exception:
                pass
            # Let the context manager handle end()
...
if PYDANTIC_AVAILABLE:
    if PYDANTIC_V2:
        ...
        class ServerConfig(BaseModel):
            ...
            def _auto_detect_workers() -> int:
                ...
                if PSUTIL_AVAILABLE:
                    ...
                    mem_cap = int(max(1, available_gb * 2))
                else:
                    mem_cap = 8
        ...
        class ServerConfig(BaseModel):
            ...
            @validator("workers", always=True)  # type: ignore
...
class StructuredLogger:
    ...
    def _get_formatter(self) -> logging.Formatter:
        ...
        if fmt == LogFormat.COLOR:
            return logging.Formatter(
                "\033[36m%(asctime)s\033[0m | \033[1m%(levelname)-8s\033[0m | \033[35m%(name)s\033[0m | %(message)s"
            )
        ...
        def wrapper(*args, **kwargs):
            with self._lock:
                if self.state == CircuitState.OPEN:
                    cooldown = self.config.circuit_breaker_timeout * min(
                        (1.5 ** (self.failures - self.config.circuit_breaker_failures)), 10
                    )
...
class Profiler:
        ...
        while True:
            try:
                if PSUTIL_AVAILABLE:
                    ...
                    # some platforms may not support num_fds
                    with suppress(Exception):
                        self.record_metric("num_fds", float(proc.num_fds()))
                    self.record_metric("num_threads", float(proc.num_threads()))
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report: Dict[str, Any] = {"uptime_seconds": uptime, "metrics": {}}
...
"""
        if _RICH_AVAILABLE and console:
            panel = Panel(Text.from_ansi(banner_text), title="🚀 System Boot", border_style="cyan")
            console.print(panel)
        else:
    ...
    def _run_with_uvicorn(self):
        with TraceContext("startup_uvicorn"):
            if not UVICORN_AVAILABLE:
                self.logger.error("Uvicorn is not installed. Cannot start server.")
                return
    ...
    def _run_with_gunicorn(self):
        ...
        # Load the ASGI app object (BaseApplication expects a callable/object, not "main:app" string)
        try:
            module = importlib.import_module(self.config.app_module)
            app_obj = getattr(module, self.config.app_var)
            if getattr(self.config, "factory", False) and callable(app_obj):
                app_obj = app_obj()
        except Exception as e:
            self.logger.error(f"❌ Failed to import ASGI app for Gunicorn: {e}. Falling back to Uvicorn.")
            return self._run_with_uvicorn()

        gunicorn_uvicorn_worker = (
            os.getenv("WORKER_CLASS", "").strip()
            or ("uvicorn_worker.UvicornWorker" if UVICORN_WORKER_AVAILABLE else "uvicorn.workers.UvicornWorker")
        )
        ...
        options = {
            ...
            "worker_class": gunicorn_uvicorn_worker,
        ...
        self.logger.info(f"🚀 Starting Gunicorn server on {self.config.host}:{self.config.port} ({gunicorn_uvicorn_worker})")
        try:
    ...
    def run(self) -> int:
        ...
        elif GUNICORN_AVAILABLE and getattr(self.config, "workers", 1) and getattr(self.config, "workers", 1) > 1:
...
def main() -> int:
    ...
    parser.add_argument("--worker-class", default="uvicorn", help="Worker class (informational)")
    ...
    kwargs = {"host": args.host, "port": args.port, "app_module": args.app_module, "app_var": args.app_var}
    if args.workers:
