...
✅ Adds derived "Technicals" to fundamentals (previous_close, change%, 52W high/low, avg vol, etc.)
✅ Prophet forecasting improved + adds 90D forecast + confidence score (0-100)
✅ Robust handling of missing/invalid data + fallback forecast if Prophet fails
✅ Stronger Shariah screening (latest quarter detection + safer ratios)
✅ Score model upgraded (fundamental + momentum + risk + AI) and returns 0–100
...
def _compute_technicals_from_hist(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # Day high/low from last row if present, else use close
    ...
    # 52W range uses ~252 trading days
...
def _linear_fallback_forecast(df: pd.DataFrame, horizon_days: int) -> Tuple[Optional[float], Optional[float]]:
    """
    Very light fallback if Prophet fails:
    Fit a simple linear trend on close vs time index.
    Returns (predicted_price, confidence_score_0_100).
    """
    try:
        ...
        # crude confidence: lower residual std -> higher confidence
...
class FinanceEngine:
    ...
    def _get_json(self, url: str) -> Any:
        ...
        # Some errors still return JSON; try to parse
        try:
            data = r.json()
        except Exception:
            raise RuntimeError(f"Non-JSON response ({r.status_code}) from EODHD")

        # Handle EODHD error format
        ...
        # Sometimes EODHD returns {"code":..., "message":...}
    ...
    def get_stock_data(self, ticker: str):
        """
        Fetches fundamentals + historical EOD data.
        Enhancement: injects derived Technicals into the fundamentals dict.
        """
        ...
        # 1) Fundamentals
        fund_url = f"{self.base_url}/fundamentals/{t}?api_token={self.api_key}&fmt=json"
        fund_data = self._get_json(fund_url)

        # 2) Historical EOD (2 years)
        start = dt.date.today() - dt.timedelta(days=365 * 2)
        hist_url = f"{self.base_url}/eod/{t}?api_token={self.api_key}&fmt=json&from={start.isoformat()}"
        hist_data = self._get_json(hist_url)

        # Inject derived technicals into fundamentals for easier consumption in app.py
        ...
        if isinstance(fund_data, dict):
            ...
            if isinstance(fund_data["Technicals"], dict):
                # do not overwrite existing provider fields unless missing
                for k, v in tech.items():
                    if fund_data["Technicals"].get(k) is None:
                        fund_data["Technicals"][k] = v

            # Add some common convenience fields (optional)
    ...
    def check_shariah_compliance(self, fundamentals: Dict[str, Any]) -> Dict[str, Any]:
        ...
        - Cash / MarketCap < 33%
        Notes:
        - Uses latest quarterly balance sheet if available.
        - If MarketCap missing, attempts fallback from Highlights/Valuation.
        """
        try:
            ...
            if not last_q:
                return {"compliant": False, "note": "Insufficient Balance Sheet Data"}
            ...
            market_cap = (
                ...
                or _safe_float(highlights.get("MarketCapitalizationMln"))  # sometimes in mln
            )

            # Convert mln if clearly too small
            ...
            if market_cap is None or market_cap <= 0:
                return {"compliant": False, "note": "Missing Market Cap"}

            debt_ratio = (total_debt / market_cap) * 100.0
            cash_ratio = (cash / market_cap) * 100.0

            # Standard: pass if both < 33%
    ...
    def run_ai_forecasting(self, hist_data: Any) -> Dict[str, Any]:
        ...
        # Add extra technical outputs (used by app.py if you map them)
        tech = _compute_technicals_from_hist(df)
        ...
        if _PROPHET_AVAILABLE and Prophet is not None and len(df) >= 120:
            try:
                ...
                # 90 days future to get both 30 and 90
                future = m.make_future_dataframe(periods=90, freq="D")
                forecast = m.predict(future)

                # Last row corresponds to 90D horizon
                ...
                # Confidence derived from interval width at 30D
                ...
                return {
                    ...
                    "trend": trend,
                    # extra technicals (helpful for sheet)
    ...
    def generate_score(self, fundamentals: Dict[str, Any], ai_data: Dict[str, Any]) -> float:
        ...
        - Momentum: AI ROI30 + trend
        - Risk: volatility, drawdown (from injected Technicals if available)
        ...
        if div_y is not None:
            # EODHD may provide dividend yield as percent
            if div_y >= 4:
                score += 6
            elif 2 <= div_y < 4:
                score += 3
        ...
        if dd90 is not None:
            # dd90 is negative; large drawdown -> penalty
            if dd90 <= -0.30:
                score -= 10
            elif dd90 <= -0.20:
                score -= 6
