...
v2.0.0 (2026-07-23) - COLLECTABLE RESTRUCTURE
--------------------------------------------------------------------------
WHY: this file ran its whole suite at IMPORT time and reported via
sys.exit(1) - a shape pytest collects as ZERO tests (exit code 5, 'no
tests ran'), so registering it in tests.yml as-is would have failed CI on
a perfectly green day. It was therefore never registered and, like
test_portfolio_actions before it, could rot unseen. The assertions are
PRESERVED VERBATIM inside _run_trend_suite(); a unittest.TestCase wraps
them so pytest collects one real test and CI can finally see this module.
Env keys touched by the suite are saved and restored around the run.
"""
import os
import sys
sys.path.insert(0, '.')
...
_TREND_ENV_KEYS = ("TFB_TREND_SIGNALS",)


def _run_trend_suite():
    """The original assertions, verbatim, with env save/restore."""
    del FAILS[:]
    _saved = {k: os.environ.get(k) for k in _TREND_ENV_KEYS}
    try:
...
class TestTrendSignalsContract(unittest.TestCase):
    """v1.0.0 trend-signals behavioral contract."""

    def test_trend_signals_suite(self):
        fails = _run_trend_suite()
        self.assertEqual(
            fails, [],
            "trend-signal groups failed:\n  " + "\n  ".join(fails))
