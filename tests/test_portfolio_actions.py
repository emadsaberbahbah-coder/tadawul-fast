...
ADD" fixture uses 1050.SR — a symbol the Q1-2026 Al-Rajhi authority list
now marks FAIL — so portfolio_actions v1.4.0's Rule 1a correctly answers
EXIT-BY-RULE, t1's `suggested_delta_sar > 0` compares None, and the whole
module (which executed its 22 groups at IMPORT time) crashed at pytest
...
list that never included this file. Two structural fixes:
  ...
  (1) HERMETIC: the 22 legacy groups are preserved VERBATIM but now run
      ...
      list says about any real symbol) and TFB_PF_ADD_CONFIRM_DAYS=0 (the
      v1.1.0 two-day ADD-confirmation gate renders every day-1 ADD as an
      unsized "pending confirmation" HOLD, which was the SECOND
      independent cause of the collection crash: t1 asserted sizing on a
      day-1 ADD). Each pinned gate gets its own dedicated test below,
      using FICTIONAL symbols via TFB_EXIT_BY_RULE_EXTRA where a rule set
      is needed, so nothing here ever depends on compliance list
      contents.
  (2) COLLECTABLE: everything is wrapped in a unittest.TestCase so pytest
      discovers real tests (a passing module-level script collects zero
      tests -> pytest exit 5 -> a green suite would have failed CI).

NEW COVERAGE (v1.4.0 Rule 1a + v1.5.0 Rule 1b):
  - Rule 1a: a rule-set symbol EXITs as EXIT-BY-RULE even at reliability
    20 (uncappable), proceeds = full market value.
  - Rule 1b: a Low-reliability position-cap breach TRIMs as TRIM-BY-RULE
    sized exactly to the cap, capped_from None.
  - Rule 1b kill-switch: TFB_TRIM_BY_RULE_GATE=0 restores the v1.4.0
    capped-HOLD byte-for-byte semantics.
  - Rule 1b scope: a Low-reliability VALUATION trim (signal-driven) is
    still capped to HOLD with the gate ON.

Every test saves and restores the env keys it touches. Direct run:
`python tests/test_portfolio_actions.py` (unittest.main), or via pytest.
"""
import os
import sys
import unittest
...
class TestPortfolioActionsContract(unittest.TestCase):
    """Legacy v1.0.x contract (rulebook pinned off) + Rule 1a/1b coverage."""

    def setUp(self):
        self._saved = {k: os.environ.get(k) for k in _RULE_ENV_KEYS}
    ...
    # -- the preserved 22-group suite ---------------------------------------

    def test_legacy_contract_suite(self):
        fails = _run_legacy_suite()
        self.assertEqual(fails, [],
                         "legacy groups failed:\n  " + "\n  ".join(fails))

    # -- Rule 1a (v1.4.0): compliance EXIT outranks the confidence cap ------

    def test_add_confirmation_day1_holds_unsized(self):
        """v1.1.0 gate at its DEFAULT depth (2): a first-day qualifying ADD
        renders HOLD 'pending confirmation (day 1/2)' with no sizing —
        ...
        self.assertIsNone(a["suggested_delta_sar"])
    ...
    def test_rule_1a_exit_by_rule_uncappable(self):
        ...
        self.assertIn("EXIT-BY-RULE", a["action_reason"])
        self.assertIn("not confidence-cappable", a["action_reason"])
        self.assertIsNone(a["detail"]["capped_from"])
        self.assertAlmostEqual(a["proceeds_sar"], 3450, delta=1)

    # -- Rule 1b (v1.5.0): risk-budget TRIM outranks the confidence cap -----

    def test_rule_1b_trim_by_rule_on_cap_breach(self):
        os.environ.pop("TFB_TRIM_BY_RULE_GATE", None)      # default ON
        os.environ.pop("TFB_EXIT_BY_RULE_EXTRA", None)
        ctl = dict(CTL)
        ...
        self.assertIn("not confidence-cappable", a["action_reason"])
        self.assertIsNone(a["detail"]["capped_from"])
        expected = 34500 - 0.15 * 44500.0
        self.assertAlmostEqual(a["proceeds_sar"], round(expected), delta=1)

    def test_rule_1b_kill_switch_restores_v140(self):
    ...
    def test_rule_1b_valuation_trim_still_cappable(self):
        ...
        self.assertEqual(a["action"], "HOLD")
        self.assertEqual(a["detail"]["capped_from"], "TRIM")
        self.assertNotIn("TRIM-BY-RULE", a["action_reason"])
