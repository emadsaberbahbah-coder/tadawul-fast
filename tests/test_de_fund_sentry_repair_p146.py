...
def test_t9_mode_disclosure():
    ...
    # v5.147.0: the boot-line literal grew a scoring_settle=%s leg and the exact
    # version pin (already red at HEAD 5.145.0) is loosened to a floor.
    assert src.count("fund_unit_sentry=%s") == 1
    assert src.count("_fund_unit_sentry_mode(),   # v5.143.0") == 1
    assert tuple(int(x) for x in de.__version__.split(".")[:3]) >= (5, 143, 0)
