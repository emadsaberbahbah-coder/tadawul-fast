...
def test_market_and_top10_tail_is_the_governed_decision_block() -> None:
    # In the accepted v2.15 physical layout Block Reason is the last market
    # contract field; Top_10 appends its three selector fields after that block.
    for page in MARKET_PAGES:
        assert _keys(page)[-1] == "block_reason", page
        assert _headers(page)[-1] == "Block Reason", page

    top10_keys = _keys("Top_10_Investments")
    top10_headers = _headers("Top_10_Investments")
    assert top10_keys[-3:] == ["top10_rank", "selection_reason", "criteria_snapshot"]
    assert top10_headers[-3:] == ["Top 10 Rank", "Selection Reason", "Criteria Snapshot"]
