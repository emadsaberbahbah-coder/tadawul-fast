...
===============================================================
Argaam Router Shim – v1.5.0 (PROD SAFE + QUIET BOOT)
...
- If DEBUG_ERRORS=1, logs become verbose.
...
def _import_delegate() -> Tuple[Optional[Any], Optional[str], Optional[str]]:
    ...
    Quiet by default; verbose only when DEBUG_ERRORS=1.
    """
    dbg = _debug_enabled()
    ...
    # Quiet boot: do NOT log warnings unless debug is enabled
    if dbg:
...
if _delegate_router is not None:
    ...
    def _token_required() -> bool:
        # If REQUIRE_AUTH=true, require token even if APP_TOKEN empty (rare, but explicit)
        if _truthy(os.getenv("REQUIRE_AUTH", "0")):
            return True
        # Otherwise require only if a token exists
    ...
    def _auth_ok(x_app_token: Optional[str]) -> bool:
        ...
        if not required_primary and not required_backup and not _truthy(os.getenv("REQUIRE_AUTH", "0")):
            return True
        ...
        if required_primary and provided == required_primary:
            return True
        if required_backup and provided == required_backup:
            return True
        return False
    ...
    async def quote(
        ...
        if _token_required() and not _auth_ok(x_app_token):
