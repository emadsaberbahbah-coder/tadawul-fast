...
Version: 4.1.0 - Full async with structured configuration and monitoring
...
class SheetsConfig(BaseModel):
    ...
    def validate_credentials(cls, v, values):
        """Validate or load credentials from environment or path."""
        if v:
            # If provided directly, just return
            return v

        # Try environment JSON
...
class FinancialData(BaseModel):
    ...
    def is_valid(self) -> bool:
        ...
        return bool(self.ticker) and self.last_price > 0 and self.data_quality != DataQuality.LOW
...
class AsyncGoogleSheetsService:
    ...
    def __init__(self, config: Optional[SheetsConfig] = None):
        ...
        # Google Sheets service (sync, used via executor)
        self._service = None
        ...
        logger.info(
            "AsyncGoogleSheetsService initialized: "
    ...
    async def _enforce_rate_limit(self) -> None:
        ...
        async with self._rate_limit_lock:
            ...
            if len(self.request_timestamps) >= self.config.rate_limit_rpm:
                ...
                if sleep_time > 0:
                    logger.warning(f"Sheets rate limit exceeded, sleeping for {sleep_time:.2f}s")
    ...
    def _get_cache_key(self, sheet_id: str, range_name: str, **kwargs) -> str:
        ...
        Format:
            <sheet_prefix>_<hash_of_range+params>

        This is designed so that per-sheet cache clearing can use the prefix.
        """
        params = sorted([f"{k}={v}" for k, v in kwargs.items()])
        sheet_prefix = hashlib.md5(sheet_id.encode()).hexdigest()[:16]
        key_payload = f"{range_name}:{'|'.join(params)}"
        payload_hash = hashlib.md5(key_payload.encode()).hexdigest()
        return f"{sheet_prefix}_{payload_hash}"
    ...
    async def request_context(self, sheet_id: str, range_name: str):
        ...
        try:
            ...
            duration_ms = (metrics.duration or 0) * 1000
            logger.debug(f"[{metrics.request_id}] Request completed: {duration_ms:.2f}ms")
    ...
    async def read_sheet_data(
        ...
        async with self.request_context(sheet_id, range_name) as metrics:
            try:
                ...
                metrics.rows_fetched = max(len(values) - 1, 0)
    ...
    async def search_by_sector(
        ...
        if case_sensitive:
            return [item for item in data if sector in item.get("sector", "")]
        return [item for item in data if sector.lower() in item.get("sector", "").lower()]
    ...
    async def batch_read_sheets(
        ...
        results: Dict[str, Union[List[Dict[str, Any]], pd.DataFrame]] = {}
        ...
        for sheet_type, task in tasks:
            try:
                ...
                if isinstance(data, list):
                    logger.info(f"Read {len(data)} rows from {sheet_type}")
                else:
                    logger.info(f"Read DataFrame from {sheet_type} with shape {data.shape}")
            except Exception as e:
    ...
    async def export_to_dataframe(
        ...
        Args:
            sheet_type: Sheet type key (e.g., "KSA_TADAWUL")
            use_cache: Use cache layer
    ...
    async def clear_cache(
        ...
        Uses the first 16 hex chars of md5(sheet_id) as a prefix
        (aligned with _get_cache_key).
        """
        cleared_count = 0
        ...
        if clear_memory:
            async with self._cache_lock:
                if sheet_type:
                    sheet_id = self.sheet_ids.get(sheet_type)
                    if sheet_id:
                        prefix = hashlib.md5(sheet_id.encode()).hexdigest()[:16] + "_"
                        keys_to_remove = [key for key in list(self._memory_cache.keys()) if key.startswith(prefix)]
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache": {
                ...
                "total_size_bytes": cache_size_bytes,
            ...
            "sheets_configured": {
                sheet_type: bool(sheet_id) for sheet_type, sheet_id in self.sheet_ids.items()
            },
            "timestamp": datetime.now(timezone.utc).isoformat(),
        }
...
async def test_service():
    ...
    try:
        ...
        print("=" * 60)
        print("Testing Async Google Sheets Service v4.1.0")
        print("=" * 60)
        ...
        # DataFrame export
        print("\n=== DataFrame Export ===")
        df = await service.export_to_dataframe(sheet_type="KSA_TADAWUL", limit=10)
